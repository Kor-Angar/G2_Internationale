...
func void dia_strf_1_standard_info()
{
	...
	if(randy == 0)
	{
		B_AI_Output(self,other,"DIA_STRF_1_STANDARD_01_00");	//Если бы мне удалось вырваться отсюда, когда у меня был шанс, мне бы сейчас не пришлось гнуть спину на эту свинью!
	};
	if(randy == 1)
	{
		B_AI_Output(self,other,"DIA_STRF_1_STANDARD_01_01");	//Это все из-за этой руды. Всегда все одно и то же. Всем позарез нужна эта проклятая руда.
	};
	if(randy == 2)
	{
		B_AI_Output(self,other,"DIA_STRF_1_STANDARD_01_02");	//Если бы ты попробовал это дерьмо, которым нас снабжают, ты бы тоже стал подумывать о бунте!
	};
	AI_StopProcessInfos(self);
};
