...
func void dia_out_13_standard_info()
{
	...
	if(randy == 0)
	{
		B_AI_Output(self,other,"DIA_OUT_1_STANDARD_13_00");	//Раньше нам удавалось не подпускать орков близко. Но последнее время их становится все больше и больше.
	};
	if(randy == 1)
	{
		B_AI_Output(self,other,"DIA_OUT_1_STANDARD_13_01");	//От ополчения тоже мало толку. Они приходят из города только, чтобы забрать наш урожай и скот.
	};
	if(randy == 2)
	{
		B_AI_Output(self,other,"DIA_OUT_1_STANDARD_13_02");	//Онар, наверное, выжил из ума, раз решился на конфликт с городом как раз тогда, когда прибыли паладины.
	};
	AI_StopProcessInfos(self);
};
