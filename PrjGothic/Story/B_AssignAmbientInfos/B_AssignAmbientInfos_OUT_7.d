...
func void dia_out_7_standard_info()
{
	...
	if(randy == 0)
	{
		B_AI_Output(self,other,"DIA_OUT_7_STANDARD_07_00");	//Последнее время жить здесь стало значительно опаснее. А полевые хищники так вообще житья не дают.
	};
	if(randy == 1)
	{
		B_AI_Output(self,other,"DIA_OUT_7_STANDARD_07_01");	//Мы должны отдавать долю городу. Если мы не буде делать этого, у нас будут проблемы с паладинами.
	};
	if(randy == 2)
	{
		B_AI_Output(self,other,"DIA_OUT_7_STANDARD_07_02");	//Бунт Онара будет иметь серьезные последствия. Я лучше не буду принимать в нем участия.
	};
	AI_StopProcessInfos(self);
};
