...
{
        B_AI_Output(self,other,"DIA_Carl_Hallo_05_00");   //ѕохоже, в городе есть несколько воров, которые крадут у зажиточных горожан.
        B_AI_Output(self,other,"DIA_Carl_Hallo_05_01");   //√ородска€ стража недавно перевернула портовый квартал вверх дном, но они ничего не нашли.
};
...
{
        B_AI_Output(self,other,"DIA_Carl_Hallo_05_02");   //„то привело теб€ в этот бедный район? „то ты ищешь здесь?
        Info_ClearChoices(dia_carl_hallo);
        Info_AddChoice(dia_carl_hallo,dia_carl_hallo_verlaufen_description,dia_carl_hallo_verlaufen); // "я заблудилс€."
        Info_AddChoice(dia_carl_hallo,dia_carl_hallo_umsehen_description,dia_carl_hallo_umsehen); // "я просто брожу осматриваю окрестности."
};
func void dia_carl_hallo_verlaufen()
{
        B_AI_Output(other,self,"DIA_Carl_Hallo_verlaufen_15_00"); //я заблудилс€.
        B_AI_Output(self,other,"DIA_Carl_Hallo_verlaufen_05_01"); //“огда смотри, чтобы теб€ не ограбили.
        b_carlsayhallo();
        Info_ClearChoices(dia_carl_hallo);
};
func void dia_carl_hallo_umsehen()
{
        B_AI_Output(other,self,"DIA_Carl_Hallo_umsehen_15_00");   //я просто брожу, осматриваю окрестности.
        B_AI_Output(self,other,"DIA_Carl_Hallo_umsehen_05_01");   //јга. —мотри, зазеваешьс€ и лишишьс€ кошелька.
        b_carlsayhallo();
        Info_ClearChoices(dia_carl_hallo);
};
...
{
        B_AI_Output(other,self,"DIA_Carl_Diebe_15_00");   //„то ты знаешь о ворах?
        B_AI_Output(self,other,"DIA_Carl_Diebe_05_01");   //Ќичего. Ќо все горожане напуганы и станов€тс€ недоверчивыми - особенно к чужакам.
        B_AI_Output(self,other,"DIA_Carl_Diebe_05_02");   //—мотри, чтобы теб€ не застали в чужом доме. ” нас этого не люб€т.
        B_AI_Output(self,other,"DIA_Carl_Diebe_05_03");   //ƒа, ты должен сам защищатьс€ от воров. ’ороша€ дубинка - лучше всего.
};
...
{
        B_AI_Output(other,self,"DIA_Carl_Lernen_15_00");  //“ы можешь научить мен€ чему-нибудь?
        B_AI_Output(self,other,"DIA_Carl_Lernen_05_01");  //ќх, € всего лишь кую скоб€ные издели€ и гвозди и чиню различные желез€ки.
        B_AI_Output(self,other,"DIA_Carl_Lernen_05_02");  //Ќо € мало что знаю об изготовлении оружи€, чтобы учить теб€ этому.
        B_AI_Output(self,other,"DIA_Carl_Lernen_05_03");  //≈сли ты хочешь обучитьс€ этому ремеслу, поговори с √арадом. ќн-то уж точно знает, как ковать оружие!
        B_AI_Output(self,other,"DIA_Carl_Lernen_05_04");  //Ќо если ты хочешь немного потренировать свои мускулы, тут € могу помочь.
        Log_CreateTopic(TOPIC_CITYTEACHER,LOG_NOTE);
        b_logentry(TOPIC_CITYTEACHER,TOPIC_CITYTEACHER_description_707); // " арл кузнец в портовом квартале может сделать мен€ сильнее."
};
...
{
        B_AI_Output(other,self,"DIA_Carl_Wieviel_15_00"); //—колько ты берешь за обучение?
        if(Npc_KnowsInfo(other,dia_edda_statue))
        {
                B_AI_Output(self,other,"DIA_Carl_Wieviel_05_01"); //я слышал, что ты сделал дл€ Ёдды. я буду тренировать теб€ бесплатно.
                CARL_TEACHSTR = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Carl_Wieviel_05_02"); //50 золотых монет - и € помогу тебе стать сильнее.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Carl_bezahlen_15_00");        //я хочу потренироватьс€ с тобой.
        if(Npc_KnowsInfo(other,dia_edda_statue))
        {
                B_AI_Output(self,other,"DIA_Carl_bezahlen_05_01");        //я слышал, что ты сделал дл€ Ёдды. я буду тренировать теб€ бесплатно.
                CARL_TEACHSTR = TRUE;
        }
        else if(b_giveinvitems(other,self,itmi_gold,50))
        {
                B_AI_Output(self,other,"DIA_Carl_bezahlen_05_02");        //’орошо, мы можем начать, как только ты будешь готов.
                CARL_TEACHSTR = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Carl_bezahlen_05_03");        //ѕринеси золото, и € потренирую теб€.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Carl_Teach_15_00");   //я хочу стать сильнее.
