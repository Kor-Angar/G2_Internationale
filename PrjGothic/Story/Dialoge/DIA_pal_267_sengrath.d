...
{
        B_AI_Output(self,other,"DIA_Sengrath_Hello_03_00");       //Я знал! Я знал, что кому-нибудь это удастся!
        B_AI_Output(self,other,"DIA_Sengrath_Hello_03_01");       //Ты прошел через Проход? Наш посланник смог пробиться, да?
        B_AI_Output(other,self,"DIA_Sengrath_Hello_15_02");       //Нет, ваш посланник не смог пересечь Проход. Я пришел по приказу лорда Хагена.
        B_AI_Output(self,other,"DIA_Sengrath_Hello_03_03");       //(рычит) Проклятые орки...
        B_AI_Output(self,other,"DIA_Sengrath_Hello_03_04");       //Ну, командующий Гаронд наверняка захочет поговорить с тобой. Ты найдешь его в этом большом здании, охраняемом двумя рыцарями.
};
...
{
        B_AI_Output(other,self,"DIA_Sengrath_Equipment_15_00");   //Где я могу найти снаряжение здесь?
        B_AI_Output(self,other,"DIA_Sengrath_Equipment_03_01");   //Тандор распоряжается оружием. Управляющий Энгор отвечает за все остальное.
        B_AI_Output(other,self,"DIA_Sengrath_Equipment_15_02");   //А что насчет магии?
        B_AI_Output(self,other,"DIA_Sengrath_Equipment_03_03");   //У нас есть свитки с заклинаниями. Если тебе понадобится один из них, дай мне знать.
        Log_CreateTopic(TOPIC_TRADER_OC,LOG_NOTE);
        b_logentry(TOPIC_TRADER_OC,TOPIC_TRADER_OC_description_380); // "Сенграт продает свитки в замке."
};
...
{
        B_AI_Output(other,self,"DIA_Sengrath_Perm_15_00");        //Кто здесь может научить меня чему-нибудь?
        if((other.guild == GIL_KDF) && (KAPITEL == 2))
        {
                B_AI_Output(self,other,"DIA_Sengrath_Perm_03_01");        //Поговори с Милтеном - он здесь единственный маг.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Sengrath_Perm_03_02");        //Спроси Керолота. Он обучает парней обращению с мечом. Может, и тебя он сможет чему-нибудь научить.
                Log_CreateTopic(TOPIC_TEACHER_OC,LOG_NOTE);
                b_logentry(TOPIC_TEACHER_OC,TOPIC_TEACHER_OC_description_381); // "Керолот тренирует мечников в замке."
        };
};
...
{
        b_givetradeinv(self);
        B_AI_Output(other,self,"DIA_Sengrath_Scrolls_15_00");     //Покажи мне свои свитки.
};
