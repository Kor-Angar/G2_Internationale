...
{
        B_AI_Output(other,self,"DIA_Bromor_Pay_15_00");   //Я хочу развлечься.
        B_AI_Output(self,other,"DIA_Bromor_GIRLS_07_01"); //Ну да, все сюда за этим приходят.
        B_AI_Output(self,other,"DIA_Bromor_GIRLS_07_02"); //Я Бромор. Это мой дом, а это мои девочки. Я люблю моих девочек.
        B_AI_Output(self,other,"DIA_Bromor_GIRLS_07_03"); //А если ты любишь моих девочек тоже, ты должен заплатить за это 50 золотых монет.
        B_AI_Output(self,other,"DIA_Bromor_GIRLS_07_04"); //Смотри только, чтобы никаких проблем.
};
...
func void dia_bromor_pay_info()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Bromor_Pay_07_01");   //Отлично. (ухмыляется) Ты долго не забудешь следующие несколько часов твоей жизни.
                B_AI_Output(self,other,"DIA_Bromor_Pay_07_02");   //Иди наверх с Надей. Удачи.
        ...
        {
                B_AI_Output(self,other,"DIA_Bromor_Pay_07_03");   //Я не выношу, когда кто-нибудь пытается надуть меня. Убирайся отсюда, если не можешь заплатить.
        };
        b_npcclearobsessionbydmt(self);
};
...
{
        B_AI_Output(other,self,"DIA_Bromor_DOPE_15_00");  //А могу я рассчитывать на 'особые' услуги, а?
        B_AI_Output(self,other,"DIA_Bromor_DOPE_07_01");  //Конечно, все мои девочки особенные. (ухмыляется)
        B_AI_Output(self,other,"DIA_Bromor_DOPE_07_02");  //Если у тебя есть деньги, ты можешь пойти наверх с Надей.
};
...
{
        B_AI_Output(other,self,"DIA_Bromor_Heilung_15_00");       //Ты одержим.
        B_AI_Output(self,other,"DIA_Bromor_Heilung_07_01");       //Что? О чем это ты? Убирайся отсюда.
        b_npcclearobsessionbydmt(self);
};
