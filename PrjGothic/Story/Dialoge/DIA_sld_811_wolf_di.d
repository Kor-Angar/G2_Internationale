...
{
        B_AI_Output(other,self,"DIA_Wolf_DI_HALLO_15_00");        //Как дела?
        if((BENNET_ISONBOARD == LOG_SUCCESS) && (Npc_IsDead(bennet_di) == FALSE))
        {
                B_AI_Output(self,other,"DIA_Wolf_DI_HALLO_08_01");        //Тебе ОЧЕНЬ нужно было брать с собой Беннета?
                B_AI_Output(other,self,"DIA_Wolf_DI_HALLO_15_02");        //В чем проблема?
                B_AI_Output(self,other,"DIA_Wolf_DI_HALLO_08_03");        //А, ничего. Все в порядке. Делай, что считаешь нужным.
        }
        else if(DIA_WOLF_DI_HALLO_ONETIME == FALSE)
        {
                B_AI_Output(self,other,"DIA_Wolf_DI_HALLO_08_04");        //Эта кузница не в самом лучшем состоянии, но я думаю, у меня что-нибудь получится.
        ...
        {
                B_AI_Output(other,self,"DIA_Wolf_DI_HALLO_15_05");        //Ты можешь изготовить для меня доспехи?
                B_AI_Output(self,other,"DIA_Wolf_DI_HALLO_08_06");        //Извини, приятель. Но не с этими инструментами. Придется подождать, пока мы не прибудем на материк.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Wolf_DI_Training_15_00");     //Обучи меня стрельбе.
        B_AI_Output(self,other,"DIA_Wolf_DI_Training_08_01");     //Из чего?
...
func void dia_wolf_di_training_bow_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Wolf_DI_Training_BOW_1_08_00");       //В отличие от арбалета, лук очень громоздкий и требует много места. Не забывай об этом в бою.
        };
        Info_ClearChoices(dia_wolf_di_training);
        Info_AddChoice(dia_wolf_di_training,DIALOG_BACK,dia_wolf_di_training_back);
        Info_AddChoice(dia_wolf_di_training,b_buildlearnstring(PRINT_LEARNBOW1,b_getlearncosttalent(other,NPC_TALENT_BOW)),dia_wolf_di_training_bow_1);
...
func void dia_wolf_di_training_bow_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Wolf_DI_Training_BOW_5_08_00");       //Тетива при стрельбе должна свободно скользить по пальцам. Скрюченный палец испортит траекторию стрелы.
        };
        Info_ClearChoices(dia_wolf_di_training);
        Info_AddChoice(dia_wolf_di_training,DIALOG_BACK,dia_wolf_di_training_back);
        Info_AddChoice(dia_wolf_di_training,b_buildlearnstring(PRINT_LEARNBOW1,b_getlearncosttalent(other,NPC_TALENT_BOW)),dia_wolf_di_training_bow_1);
...
func void dia_wolf_di_training_crossbow_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Wolf_DI_Training_CROSSBOW_1_08_00");  //Постарайся, чтобы при выстреле из арбалета у тебя не дрогнула рука. Вот почему спусковой крючок нужно нажимать очень нежно.
        };
        Info_ClearChoices(dia_wolf_di_training);
        Info_AddChoice(dia_wolf_di_training,DIALOG_BACK,dia_wolf_di_training_back);
        Info_AddChoice(dia_wolf_di_training,b_buildlearnstring(PRINT_LEARNBOW1,b_getlearncosttalent(other,NPC_TALENT_BOW)),dia_wolf_di_training_bow_1);
...
func void dia_wolf_di_training_crossbow_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Wolf_DI_Training_CROSSBOW_5_08_00");  //Опытный стрелок всегда учитывает направление ветра и никогда не стреляет против него.
        };
        Info_ClearChoices(dia_wolf_di_training);
        Info_AddChoice(dia_wolf_di_training,DIALOG_BACK,dia_wolf_di_training_back);
        Info_AddChoice(dia_wolf_di_training,b_buildlearnstring(PRINT_LEARNBOW1,b_getlearncosttalent(other,NPC_TALENT_BOW)),dia_wolf_di_training_bow_1);
...
{
        B_AI_Output(other,self,"DIA_Wolf_DI_UndeadDragonDead_15_00");     //Все в порядке?
        B_AI_Output(self,other,"DIA_Wolf_DI_UndeadDragonDead_08_01");     //Конечно. А у тебя? Нелегко тебе пришлось, да?
        if(DIA_WOLF_DI_UNDEADDRAGONDEAD_ONETIME == FALSE)
        {
                B_AI_Output(other,self,"DIA_Wolf_DI_UndeadDragonDead_15_02");     //Куда ты теперь?
                B_AI_Output(self,other,"DIA_Wolf_DI_UndeadDragonDead_08_03");     //Куда угодно, только не назад. Я по горло сыт Хоринисом.
                B_AI_Output(self,other,"DIA_Wolf_DI_UndeadDragonDead_08_04");     //Мне все равно, куда плыть. Лишь бы подальше отсюда.
