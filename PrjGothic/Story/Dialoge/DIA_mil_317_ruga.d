...
{
        B_AI_Output(other,self,"DIA_Ruga_Hallo_15_00");   //Что ты делаешь здесь?
        B_AI_Output(self,other,"DIA_Ruga_Hallo_11_01");   //Я обучаю парней стрельбе из арбалета и помогаю им стать более ловкими.
        Log_CreateTopic(TOPIC_CITYTEACHER,LOG_NOTE);
        b_logentry(TOPIC_CITYTEACHER,TOPIC_CITYTEACHER_description_260); // "Руга городской гвардеец может помочь мне повысить мою ловкость и научить меня пользоваться арбалетом. Но для этого я должен служить королю."
};
...
{
        B_AI_Output(other,self,"DIA_Ruga_Train_15_00");   //Ты можешь потренировать меня?
        if((hero.guild == GIL_MIL) || (hero.guild == GIL_PAL) || (hero.guild == GIL_KDF))
        {
                B_AI_Output(self,other,"DIA_Ruga_Train_11_01");   //Конечно. Если у тебя достаточно опыта, я готов помочь тебе.
                B_AI_Output(self,other,"DIA_Ruga_Train_11_02");   //Но нужно понимать, что ловкость и стрельба неотделимы друг от друга, как арбалет и стрела. Одно...
                B_AI_Output(other,self,"DIA_Ruga_Train_15_03");   //... ничего не стоит без другого. Я понял.
                RUGA_TEACHCROSSBOW = TRUE;
                RUGA_TEACHDEX = TRUE;
        }
        else if((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
        {
                B_AI_Output(self,other,"DIA_Ruga_Train_11_04");   //Убирайся с глаз моих, (презрительно) наемник.
                AI_StopProcessInfos(self);
        }
        else
        {
                B_AI_Output(self,other,"DIA_Ruga_Train_11_05");   //Нет. Я тренирую только людей, состоящих на службе у короля. Больше никого.
                B_AI_Output(self,other,"DIA_Ruga_Train_11_06");   //Но нам всегда нужны хорошие люди. Так что, если хочешь поступить в ополчение, поговори с лордом Андрэ.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Ruga_Teach_15_00");   //Покажи мне, как стрелять из арбалета.
...
func void dia_ruga_teach_back()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Ruga_Teach_11_00");   //Мне больше нечему учить тебя. Тебе лучше поискать другого учителя.
...
{
        B_AI_Output(other,self,"DIA_Ruga_TEACHDEX_15_00");        //Я хочу стать более ловким.
...
func void dia_ruga_teachdex_back()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Ruga_TEACHDEX_11_00");        //Это все, чему я мог обучить тебя. Если ты хочешь стать еще более ловким, тебе лучше поискать другого учителя.
