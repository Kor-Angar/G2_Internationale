...
{
        B_AI_Output(other,self,"DIA_Lares_DI_Hallo_15_00");       //Как дела?
        if(ORKSTURMDI == TRUE)
        {
                B_AI_Output(self,other,"DIA_Lares_DI_Hallo_09_01");       //Поторопись. Орки, наверняка, вернутся опять.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Lares_DI_Hallo_09_02");       //Хорошо. Хотя этот грот немного пугает меня.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Lares_DI_Training_15_00");    //Научи меня тому, что знаешь сам.
        B_AI_Output(self,other,"DIA_Lares_DI_Training_09_01");    //Нет проблем.
        ...
        {
                Info_AddChoice(dia_lares_di_training,dia_lares_di_training_sneak_description,dia_lares_di_training_sneak); // "Научиться красться"
        };
...
func void dia_lares_di_training_1h_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lares_DI_Training_1H_1_09_00");       //Тебе нужно согнуть немного переднюю ногу и выпрямить спину - так ты займешь правильную стойку.
        };
        Info_ClearChoices(dia_lares_di_training);
        Info_AddChoice(dia_lares_di_training,DIALOG_BACK,dia_lares_di_training_back);
        if(Npc_GetTalentSkill(hero,NPC_TALENT_SNEAK) == FALSE)
...
func void dia_lares_di_training_1h_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lares_DI_Training_1H_5_09_00");       //Не напрягай бедра, это позволит тебе вовремя уклониться от контратаки.
        };
        Info_ClearChoices(dia_lares_di_training);
        Info_AddChoice(dia_lares_di_training,DIALOG_BACK,dia_lares_di_training_back);
        if(Npc_GetTalentSkill(hero,NPC_TALENT_SNEAK) == FALSE)
...
func void dia_lares_di_training_dex_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lares_DI_Training_DEX_1_09_00");      //Верхняя часть твоего туловища должна действовать согласованно с нижней.
        };
        Info_ClearChoices(dia_lares_di_training);
        Info_AddChoice(dia_lares_di_training,DIALOG_BACK,dia_lares_di_training_back);
        if(Npc_GetTalentSkill(hero,NPC_TALENT_SNEAK) == FALSE)
...
func void dia_lares_di_training_dex_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lares_DI_Training_DEX_5_09_00");      //Всегда следи за своими ногами.
        };
        Info_ClearChoices(dia_lares_di_training);
        Info_AddChoice(dia_lares_di_training,DIALOG_BACK,dia_lares_di_training_back);
        if(Npc_GetTalentSkill(hero,NPC_TALENT_SNEAK) == FALSE)
...
func void dia_lares_di_training_sneak()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lares_DI_Training_SNEAK_09_00");      //Всегда используй всю площадь подошвы, когда крадешься.
        };
        Info_ClearChoices(dia_lares_di_training);
        Info_AddChoice(dia_lares_di_training,DIALOG_BACK,dia_lares_di_training_back);
        if(Npc_GetTalentSkill(hero,NPC_TALENT_SNEAK) == FALSE)
...
{
        B_AI_Output(other,self,"DIA_Lares_DI_UndeadDragonDead_15_00");    //Похоже, все это тебя не особенно волнует.
        B_AI_Output(self,other,"DIA_Lares_DI_UndeadDragonDead_09_01");    //Ах, знаешь, я был уверен, что ты справишься.
        if(DIA_LARES_DI_UNDEADDRAGONDEAD_ONETIME == FALSE)
        {
                B_AI_Output(other,self,"DIA_Lares_DI_UndeadDragonDead_15_02");    //Что ты собираешься делать дальше?
                B_AI_Output(self,other,"DIA_Lares_DI_UndeadDragonDead_09_03");    //Пока не знаю. Пожалуй, я пока побуду с тобой.
