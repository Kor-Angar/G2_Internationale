...
{
        B_AI_Output(other,self,"DIA_Parlaf_HALLO_15_00"); //Привет, как дела?
        B_AI_Output(self,other,"DIA_Parlaf_HALLO_03_01"); //А как, по-твоему? Я провел целый день за точильным кругом, затачивая клинки.
        if(Npc_IsDead(engor) == FALSE)
        {
                B_AI_Output(self,other,"DIA_Parlaf_HALLO_03_02"); //А вчера Энгор опять урезал наш паек. Если он будет продолжать в том же духе, мы все помрем с голода.
                B_AI_Output(self,other,"DIA_Parlaf_HALLO_03_03"); //Или нас всех перебьют орки. Не самая радужная перспектива.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Parlaf_ENGOR_15_00"); //Кто такой Энгор?
        B_AI_Output(self,other,"DIA_Parlaf_ENGOR_03_01"); //Энгор заведует провизией и выдает пайки. А они становятся все меньше с каждой неделей.
        B_AI_Output(self,other,"DIA_Parlaf_ENGOR_03_02"); //Конечно, с ним можно договориться, но только за золото и весьма недешево.
        B_AI_Output(other,self,"DIA_Parlaf_ENGOR_15_03"); //А ты не можешь позволить себе это?
        B_AI_Output(self,other,"DIA_Parlaf_ENGOR_03_04"); //За те гроши, что нам платят, я могу купить только пару реп!
...
{
        B_AI_Output(other,self,"DIA_Parlaf_Wo_15_00");    //Где мне найти этого Энгора?
        B_AI_Output(self,other,"DIA_Parlaf_Wo_03_01");    //В доме рыцарей. Пройди через открытый вход рядом с кузницей.
};
...
{
        B_AI_Output(other,self,"DIA_Parlaf_HUNGRIG_15_00");       //Что новенького?
        B_AI_Output(self,other,"DIA_Parlaf_HUNGRIG_03_01");       //Этот чертов Энгор должен увеличить пайки!
};
...
{
        B_AI_Output(other,self,"DIA_Parlaf_SATT_15_00");  //Что новенького?
        if((DIA_PARLAF_SATT_ONETIME == FALSE) && (Npc_IsDead(engor) == FALSE))
        {
                B_AI_Output(self,other,"DIA_Parlaf_SATT_03_01");  //Энгор раздал мясные пайки. Ох, как долго я этого ждал.
                DIA_PARLAF_SATT_ONETIME = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Parlaf_SATT_03_02");  //Твое прибытие - это единственная действительно свежая новость.
        };
};
