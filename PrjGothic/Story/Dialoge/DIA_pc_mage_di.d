...
{
        B_AI_Output(other,self,"DIA_Milten_DI_Hello_15_00");      //Прямо как в старые времена.
        B_AI_Output(self,other,"DIA_Milten_DI_Hello_03_01");      //Ты сам сказал это. Мне даже интересно, сможешь ли ты выкарабкаться на этот раз.
        B_AI_Output(other,self,"DIA_Milten_DI_Hello_15_02");      //Ты о чем?
        B_AI_Output(self,other,"DIA_Milten_DI_Hello_03_03");      //Удастся ли тебе спасти свою задницу, когда на тебя опять обрушится весь этот ад.
};
...
{
        B_AI_Output(other,self,"DIA_Milten_DI_TRADE_15_00");      //Есть несколько лишних зелий?
        b_givetradeinv(self);
        ...
        B_AI_Output(self,other,"DIA_Milten_DI_TRADE_03_01");      //Пока есть запас.
};
...
{
        B_AI_Output(other,self,"DIA_Milten_DI_Rat_15_00");        //Можешь дать мне какой-нибудь совет?
        B_AI_Output(self,other,"DIA_Milten_DI_Rat_03_01");        //Ммм. Это большая честь, что ты спрашиваешь у меня совета. Но у меня в голове все это время вертится только одно.
        B_AI_Output(self,other,"DIA_Milten_DI_Rat_03_02");        //У тебя есть Глаз Инноса?
        Info_ClearChoices(dia_milten_di_rat);
        if(SC_INNOSEYEVERGESSEN_DI == TRUE)
        {
                Info_AddChoice(dia_milten_di_rat,dia_milten_di_rat_nein_description,dia_milten_di_rat_nein); // "Нет."
        }
        else
        {
                Info_AddChoice(dia_milten_di_rat,dia_milten_di_rat_ja_description,dia_milten_di_rat_ja); // "Конечно."
        };
};
func void dia_milten_di_rat_nein()
{
        B_AI_Output(other,self,"DIA_Milten_DI_Rat_nein_15_00");   //Нет.
        B_AI_Output(self,other,"DIA_Milten_DI_Rat_nein_03_01");   //(возмущенно) Ты такой...Что ты будешь делать, если ты встретишься с драконами здесь, на острове?
        B_AI_Output(self,other,"DIA_Milten_DI_Rat_nein_03_02");   //Ты так и не поумнел? Здесь у нас есть даже алхимический стол, мы могли бы спокойно перезарядить Глаз.
        B_AI_Output(self,other,"DIA_Milten_DI_Rat_nein_03_03");   //А ты о чем думаешь? Мне остается только надеяться, что твоя непредусмотрительность не будет стоить нам жизней.
        Info_ClearChoices(dia_milten_di_rat);
};
func void dia_milten_di_rat_ja()
{
        B_AI_Output(other,self,"DIA_Milten_DI_Rat_ja_15_00");     //Конечно.
        B_AI_Output(self,other,"DIA_Milten_DI_Rat_ja_03_01");     //Извини, что я задаю такой глупый вопрос. Я немного нервничаю.
...
{
        B_AI_Output(other,self,"DIA_Milten_DI_PEDROTOT_15_00");   //Я нашел Педро.
        B_AI_Output(self,other,"DIA_Milten_DI_PEDROTOT_03_01");   //(удивленно) Что? Где? Здесь, на острове? Черт, но это абсолютно невероятно.
        B_AI_Output(self,other,"DIA_Milten_DI_PEDROTOT_03_02");   //Я понятия не имел, что он такой подонок.
        b_giveplayerxp(XP_AMBIENT);
        if(Npc_IsDead(pedro_di))
        {
                B_AI_Output(other,self,"DIA_Milten_DI_PEDROTOT_15_03");   //Он мертв.
                B_AI_Output(self,other,"DIA_Milten_DI_PEDROTOT_03_04");   //Да? Хорошо. Да упокоится его прах с миром. Хотя я не могу сказать, что мне жаль его, я все же был бы не прочь задать ему парочку вопросов.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Milten_DI_PEDROTOT_03_05");   //У меня с ним давние счеты.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Milten_DI_TeachMagic_15_00"); //Я хочу повысить свои магические способности.
        if(ORKSTURMDI == FALSE)
        {
                B_AI_Output(self,other,"DIA_Milten_DI_TeachMagic_03_01"); //Я сделаю все, что смогу.
        }
        else if(DIA_MILTEN_DI_TEACHMAGIC_ONETIME == FALSE)
        {
                B_AI_Output(self,other,"DIA_Milten_DI_TeachMagic_03_02"); //Я помогу тебе, но только при условии, что ты позаботишься, чтобы орки оставались там, где они есть сейчас.
                DIA_MILTEN_DI_TEACHMAGIC_ONETIME = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Milten_DI_TeachMagic_03_03"); //Хорошо. Что ты хочешь?
        };
        ...
        {
                Info_AddChoice(dia_milten_di_teachmagic,dia_milten_di_teachmagic_runes_description,dia_milten_di_teachmagic_runes); // "Создание рун"
        };
};
func void dia_milten_di_teachmagic_mana_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Milten_DI_TeachMagic_MANA_1_03_00");  //Да ведет тебя рука Инноса.
        };
        Info_ClearChoices(dia_milten_di_teachmagic);
        Info_AddChoice(dia_milten_di_teachmagic,DIALOG_BACK,dia_milten_di_teachmagic_back);
        Info_AddChoice(dia_milten_di_teachmagic,b_buildlearnstring(PRINT_LEARNMANA1,b_getlearncostattribute(other,ATR_MANA_MAX)),dia_milten_di_teachmagic_mana_1);
...
func void dia_milten_di_teachmagic_mana_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Milten_DI_TeachMagic_MANA_5_03_00");  //Да осветит Иннос твой путь.
        };
        Info_ClearChoices(dia_milten_di_teachmagic);
        Info_AddChoice(dia_milten_di_teachmagic,DIALOG_BACK,dia_milten_di_teachmagic_back);
        Info_AddChoice(dia_milten_di_teachmagic,b_buildlearnstring(PRINT_LEARNMANA1,b_getlearncostattribute(other,ATR_MANA_MAX)),dia_milten_di_teachmagic_mana_1);
...
func void dia_milten_di_teachmagic_runes()
{
        Info_ClearChoices(dia_milten_di_teachmagic);
        Info_AddChoice(dia_milten_di_teachmagic,DIALOG_BACK,dia_milten_di_teachmagic_back);
        B_AI_Output(self,other,"DIA_Milten_DI_TeachMagic_RUNES_03_00");   //Ох, нет! Я не большой специалист в этом, но мы как-нибудь справимся.
        if(hero.guild == GIL_PAL)
        {
                Info_AddChoice(dia_milten_di_teachmagic,dia_milten_di_teachmagic_runen_paladin_description,dia_milten_di_teachmagic_runen_paladin); // "Руны паладинов"
        }
        else if(Npc_GetTalentSkill(hero,NPC_TALENT_MAGE) == 4)
        {
                Info_AddChoice(dia_milten_di_teachmagic,dia_milten_di_teachmagic_runen_circle_4_description,dia_milten_di_teachmagic_runen_circle_4); // " Четвертый круг"
        }
        else if(Npc_GetTalentSkill(hero,NPC_TALENT_MAGE) == 5)
        {
                Info_AddChoice(dia_milten_di_teachmagic,dia_milten_di_teachmagic_runen_circle_5_description,dia_milten_di_teachmagic_runen_circle_5); // " Пятый круг"
        }
        else if(Npc_GetTalentSkill(hero,NPC_TALENT_MAGE) == 6)
        {
                Info_AddChoice(dia_milten_di_teachmagic,dia_milten_di_teachmagic_runen_circle_6_description,dia_milten_di_teachmagic_runen_circle_6); // " Шестой круг"
        }
        else
        {
                B_AI_Output(self,other,"DIA_Milten_DI_TeachMagic_RUNES_03_01");   //Ох. Ты еще не вполне готов. Сначала ты должен изучить высшие круги, а затем я смогу помочь тебе.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Milten_DI_DementorObsessionBook_15_00");      //Эта книга, Альманах Одержимых, говорит тебе о чем-то?
        B_AI_Output(self,other,"DIA_Milten_DI_DementorObsessionBook_03_01");      //Пирокар эксперт по таким книгам.
        B_AI_Output(self,other,"DIA_Milten_DI_DementorObsessionBook_03_02");      //Извини. Я знаю слишком мало, чтобы сказать что-либо умное об этом.
        b_giveplayerxp(XP_AMBIENT);
};
...
{
        B_AI_Output(other,self,"DIA_Milten_DI_DragonEgg_15_00");  //Ты имел дело с драконьими яйцами?
        B_AI_Output(self,other,"DIA_Milten_DI_DragonEgg_03_01");  //Нет, не совсем. Я слышал, что искусный алхимик как-то смог сварить зелье из них.
        B_AI_Output(self,other,"DIA_Milten_DI_DragonEgg_03_02");  //Но, пожалуйста, не спрашивай меня о рецепте. Я понятия не имею, как оно готовится.
        b_giveplayerxp(XP_AMBIENT);
};
...
{
        B_AI_Output(other,self,"DIA_Milten_DI_UndeadDragonDead_15_00");   //Хорошо. Дело сделано! Храм теперь лишен своей силы.
        if(DIA_MILTEN_DI_UNDEADDRAGONDEAD_ONETIME == FALSE)
        {
                B_AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_01");   //Как тебе всегда удается выходись сухим из воды?
                B_AI_Output(other,self,"DIA_Milten_DI_UndeadDragonDead_15_02");   //Черт меня побери, если я знаю.
                B_AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_03");   //(смеется) Мы, когда-нибудь сможем пожить спокойно? Мы, определенно, заслужили это.
                if(hero.guild == GIL_KDF)
                {
                        B_AI_Output(other,self,"DIA_Milten_DI_UndeadDragonDead_15_04");   //Что ты собираешься делать сейчас?
                        B_AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_05");   //Я думаю об основании собственной академии, чтобы проповедовать нашу веру. Но из этого может ничего не получиться.
                        B_AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_06");   //Я никогда не жалел о том, что стал Магом Огня. А как тебе это?
                        B_AI_Output(other,self,"DIA_Milten_DI_UndeadDragonDead_15_07");   //Я даже не знаю.
                        B_AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_08");   //(смеется) Сухо, как всегда. Эй, парень. Ты только что спас мир. Разве это недостаточная причина для радости?
                        B_AI_Output(other,self,"DIA_Milten_DI_UndeadDragonDead_15_09");   //Ммм. Может быть.
                };
                B_AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_10");   //Да ладно, дружище, я думаю, что главное, что тебе сейчас нужно, - немного поспать.
        };
        B_AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_11");   //Тебе нужно пойти к капитану и сказать ему, чтобы он поднимал якорь.
        AI_StopProcessInfos(self);
        if(DIA_MILTEN_DI_UNDEADDRAGONDEAD_ONETIME == FALSE)
        {
