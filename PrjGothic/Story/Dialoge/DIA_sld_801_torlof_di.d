...
{
        B_AI_Output(other,self,"DIA_Torlof_DI_Hallo_15_00");      //Все спокойно?
        if(ORKSTURMDI == FALSE)
        {
                B_AI_Output(self,other,"DIA_Torlof_DI_Hallo_01_01");      //Пока да. Но все может измениться в одно мгновение. Будь осторожен.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Torlof_DI_Hallo_01_02");      //Если эти презренные орки будут оставаться там, где они есть сейчас, я не вижу никакой проблемы.
...
{
        B_AI_Output(other,self,"DIA_Torlof_DI_Teach_15_00");      //Я хочу улучшить свои способности!
...
{
        B_AI_Output(other,self,"DIA_Torlof_DI_UndeadDragonDead_15_00");   //Я уничтожил врагов.
        B_AI_Output(self,other,"DIA_Torlof_DI_UndeadDragonDead_01_01");   //Я ничего другого и не ожидал. Как все прошло? Мы можем отправляться назад?
        Info_ClearChoices(dia_torlof_di_undeaddragondead);
        Info_AddChoice(dia_torlof_di_undeaddragondead,dia_torlof_di_undeaddragondead_moment_description,dia_torlof_di_undeaddragondead_moment); // "Мне нужно еще пару минут."
        Info_AddChoice(dia_torlof_di_undeaddragondead,dia_torlof_di_undeaddragondead_over_description,dia_torlof_di_undeaddragondead_over); // "Да. Все кончено."
};
func void dia_torlof_di_undeaddragondead_moment()
{
        B_AI_Output(other,self,"DIA_Torlof_DI_UndeadDragonDead_moment_15_00");    //Мне нужно еще пару минут.
        B_AI_Output(self,other,"DIA_Torlof_DI_UndeadDragonDead_moment_01_01");    //Поторопись!
        AI_StopProcessInfos(self);
};
