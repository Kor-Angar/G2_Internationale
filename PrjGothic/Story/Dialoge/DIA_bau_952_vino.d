...
{
        B_AI_Output(other,self,"DIA_Vino_HALLO_15_00");   //Как идет работа?
        B_AI_Output(self,other,"DIA_Vino_HALLO_05_01");   //Так же, как и всегда. Работы много, денег мало, а если нам совсем не повезет, то завтра придут орки и сожгут нашу ферму дотла.
        if(hero.guild == GIL_NONE)
        {
                B_AI_Output(self,other,"DIA_Vino_HALLO_05_02");   //Королевские паладины заняли весь город. Но я очень сомневаюсь в том, что они поднимут свои задницы и придут сюда, если орки нападут на нас.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Vino_SeekWork_15_00");        //Могу я чем-нибудь помочь? Я ищу работу.
        if(hero.guild == GIL_NONE)
        {
                B_AI_Output(self,other,"DIA_Vino_SeekWork_05_01");        //А ты вообще что-нибудь знаешь о полевых работах?
                B_AI_Output(other,self,"DIA_Vino_SeekWork_15_02");        //А что там такого знать-то?
                B_AI_Output(self,other,"DIA_Vino_SeekWork_05_03");        //Ах! В таком случае... я думаю, мы и сами справимся, спасибо.
                if(!Npc_IsDead(lobart))
                {
                        B_AI_Output(self,other,"DIA_Vino_SeekWork_05_04");        //Но если ты хочешь работать на Лобарта в качестве поденного рабочего, я могу только предупредить тебя. Он платит людям вроде тебя сущие гроши!
                        ...
                        {
                                B_AI_Output(other,self,"DIA_Vino_SeekWork_15_05");        //Он предложил купить у него чистую одежду дешевле, если я буду помогать на ферме.
                                B_AI_Output(self,other,"DIA_Vino_SeekWork_05_06");        //Хмм. У меня нет для тебя никакой работы, но ты можешь принести мне и парням что-нибудь выпить.
                                B_AI_Output(self,other,"DIA_Vino_SeekWork_05_07");        //Принеси мне бутылку вина, и я скажу Лобарту, что ты очень помог нам (смеется издевательски).
                                MIS_VINO_WEIN = LOG_RUNNING;
                                Log_CreateTopic(TOPIC_VINO,LOG_MISSION);
                                Log_SetTopicStatus(TOPIC_VINO,LOG_RUNNING);
                                b_logentry(TOPIC_VINO,TOPIC_VINO_description_66); // "Если я принесу Вино бутылку вина он скажет Лобарту что я помог ему."
                        }
                        else
                        {
                                B_AI_Output(self,other,"DIA_Vino_SeekWork_05_08");        //Но ты не слышал этого от меня, понятно?
                        };
                };
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vino_SeekWork_05_09");        //Я не думаю, что тебе это понравится. Человек твоего положения вряд ли захочет делать такую грязную работу, что я могу предложить.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Vino_BringWine_15_00");       //Вот твое вино.
        b_giveinvitems(other,self,itfo_wine,1);
        B_AI_Output(self,other,"DIA_Vino_BringWine_05_01");       //Я, пожалуй, не буду спрашивать, где ты взял его, хорошо? (смеется) Кого это волнует?
        if(!Npc_IsDead(lobart))
        {
                B_AI_Output(self,other,"DIA_Vino_BringWine_05_02");       //Но все равно, спасибо.
                if(hero.guild == GIL_NONE)
                {
                        B_AI_Output(self,other,"DIA_Vino_BringWine_05_03");       //Лобарт услышит только хорошее о тебе.
                };
        };
...
{
        B_AI_Output(other,self,"DIA_Vino_ToTheCity_15_00");       //Я направляюсь в город.
        B_AI_Output(self,other,"DIA_Vino_ToTheCity_05_01");       //И?
        B_AI_Output(other,self,"DIA_Vino_ToTheCity_15_02");       //Ты можешь рассказать о городе что-нибудь интересное?
        B_AI_Output(self,other,"DIA_Vino_ToTheCity_05_03");       //Нет. Но Малет частенько ходит в город. Возможно, он что-нибудь знает... ну, что-нибудь, что может заинтересовать тебя.
};
...
{
        B_AI_Output(other,self,"DIA_Vino_PERM_15_00");    //Есть какие-нибудь интересные новости?
        if(VINO_GOSSIP_ORKS == FALSE)
        {
                B_AI_Output(self,other,"DIA_Vino_PERM_05_01");    //Я могу поклясться, что видел орка вон там, на краю леса, два дня назад.
                B_AI_Output(self,other,"DIA_Vino_PERM_05_02");    //С тех пор я сплю хотя бы с одним открытым глазом.
                VINO_GOSSIP_ORKS = TRUE;
                KNOWS_ORK = TRUE;
        }
        else if((VINO_GOSSIP_BUGS == FALSE) && (MIS_ANDREHELPLOBART == LOG_RUNNING))
        {
                B_AI_Output(self,other,"DIA_Vino_PERM_05_03");    //Эти огромные отвратительные насекомые уже всех достали. Они повсюду. Они сожрут все, что мы вырастили, если мы не сделаем что-нибудь.
                B_AI_Output(self,other,"DIA_Vino_PERM_05_04");    //Несколько дней назад я лежал в траве, размышляя о своем, и собирался вздремнуть, когда одна из этих тварей сожрала мой ботинок!
                B_AI_Output(self,other,"DIA_Vino_PERM_05_05");    //Ты бы видел, как я улепетывал. С тех пор я не могу спать спокойно.
                VINO_GOSSIP_BUGS = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vino_PERM_05_06");    //Кроме того, что я уже сказал тебе? Нет.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Vino_DMTAMSTART_15_00");      //А как у тебя дела?
        if((FOUNDVINOSKELLEREI == TRUE) && (DIA_VINO_DMTAMSTART_ONETIME == FALSE) && (hero.guild != GIL_MIL))
        {
                B_AI_Output(self,other,"DIA_Vino_DMTAMSTART_05_01");      //Дерьмово. Ополчение нашло мой тайный винокуренный заводик.
                B_AI_Output(self,other,"DIA_Vino_DMTAMSTART_05_02");      //Остается надеяться, что они никогда не узнают, что он принадлежал мне.
                b_giveplayerxp(XP_AMBIENTKAP3);
                DIA_VINO_DMTAMSTART_ONETIME = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vino_DMTAMSTART_05_03");      //Эти ублюдки в черных рясах повсюду. Кто-нибудь должен засунуть их капюшоны им в глотку.
        };
};
...
func void dia_vino_obesessed_info()
{
        ...
        {
                B_AI_Output(other,self,"DIA_Vino_Obesessed_15_00");       //Что с тобой?
                B_AI_Output(self,other,"DIA_Vino_Obesessed_05_01");       //(рычит) Черт, убей их. Иначе они убьют меня.
...
{
        B_AI_Output(other,self,"DIA_Vino_Heilung_15_00"); //Ты сам не свой.
        B_AI_Output(self,other,"DIA_Vino_Heilung_05_01"); //Моя голова... я не могу больше выносить это.
        if(DIA_VINO_HEILUNG_ONETIME == FALSE)
        {
                B_AI_Output(other,self,"DIA_Vino_Heilung_15_02"); //Ты должен сходить в монастырь. Пирокар, высший маг Огня, сможет помочь тебе.
                B_AI_Output(self,other,"DIA_Vino_Heilung_05_03"); //Ты думаешь? Хорошо. Я попытаюсь.
                b_npcclearobsessionbydmt(self);
                b_startotherroutine(vino,"Kloster");
                b_logentry(TOPIC_DEMENTOREN,TOPIC_DEMENTOREN_description_67); // "Вино одержим. Я отправил его в монастырь на лечение. Надеюсь он сможет добраться туда живым."
...
{
        B_AI_Output(other,self,"DIA_Vino_PERM4OBSESSED_15_00");   //Как ты?
        B_AI_Output(self,other,"DIA_Vino_PERM4OBSESSED_05_01");   //Да, да, все уже хорошо. Я думаю, парни смогут помочь мне. Но я все еще чувствую себя довольно паршиво.
        if(DIA_VINO_PERM4OBSESSED_XP_ONETIME == FALSE)
        {
                B_AI_Output(self,other,"DIA_Vino_PERM4OBSESSED_05_02");   //Но должен признать, их вино - лучшее из того, что я пробовал.
...
{
        B_AI_Output(other,self,"DIA_Vino_PERM45UND6_15_00");      //Есть новости?
        if(hero.guild == GIL_PAL)
        {
                B_AI_Output(self,other,"DIA_Vino_PERM45UND6_05_01");      //Здесь все просто кишит орками.
                B_AI_Output(self,other,"DIA_Vino_PERM45UND6_05_02");      //Вы, паладины, ведь прикончите их, да?
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vino_PERM45UND6_05_03");      //Боюсь, что скоро нам придется покинуть нашу ферму и присоединиться к наемникам на ферме Онара.
        };
        if((FOUNDVINOSKELLEREI == TRUE) && (hero.guild != GIL_MIL))
        {
                B_AI_Output(self,other,"DIA_Vino_PERM45UND6_05_04");      //К сожалению, ополчение нашло мой винокуренный заводик. Надеюсь, что они хотя бы меня не поймают.
        };
};
