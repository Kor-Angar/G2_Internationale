...
{
        B_AI_Output(other,self,"DIA_Khaled_Hallo_15_00"); //С тобой все в порядке?
        if(Npc_KnowsInfo(other,dia_lee_wannajoin))
        {
                B_AI_Output(self,other,"DIA_Khaled_Hallo_11_01"); //Хм - так ты хочешь присоединиться к нам, да? А у тебя есть хотя бы приличное оружие?
                KHALED_WEITER = TRUE;
                Log_CreateTopic(TOPIC_SOLDIERTRADER,LOG_NOTE);
                b_logentry(TOPIC_SOLDIERTRADER,TOPIC_SOLDIERTRADER_description_519); // "Халед - торговец оружием."
        }
        else
        {
                B_AI_Output(self,other,"DIA_Khaled_Hallo_11_02"); //Если у тебя что-то важное, поговори с Ли. А меня оставь в покое.
                AI_StopProcessInfos(self);
        };
};
...
{
        B_AI_Output(other,self,"DIA_Khaled_TRADE_15_00"); //Какое оружие ты можешь предложить мне?
        b_givetradeinv(self);
        B_AI_Output(self,other,"DIA_Khaled_TRADE_11_01"); //Только лучшее. Да ты сам взгляни на него.
};
...
{
        B_AI_Output(other,self,"DIA_Khaled_WannaJoin_15_00");     //Ты не возражаешь, если я присоединюсь к вам?
        B_AI_Output(self,other,"DIA_Khaled_WannaJoin_11_01");     //Когда ты пройдешь испытание, я проголосую за тебя.
        if((MIS_TORLOF_HOLPACHTVONSEKOB == LOG_SUCCESS) || (MIS_TORLOF_BENGARMILIZKLATSCHEN == LOG_SUCCESS))
        {
                B_AI_Output(other,self,"DIA_Khaled_WannaJoin_15_02");     //Я его уже прошел.
                B_AI_Output(self,other,"DIA_Khaled_WannaJoin_11_03");     //Ну, тогда все в порядке.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Khaled_Woher_15_00"); //Как ты оказался среди наемников?
        B_AI_Output(self,other,"DIA_Khaled_Woher_11_01"); //Я пришел вместе с Сильвио с юга. Мы состояли в армии наемников, которая сражалась с орками.
};
...
{
        B_AI_Output(other,self,"DIA_Khaled_AboutSylvio_15_00");   //Что ты думаешь о Сильвио?
        B_AI_Output(self,other,"DIA_Khaled_AboutSylvio_11_01");   //Это опасный тип! И многие наемники, что пришли с ним, прислушиваются к его мнению. Тебе лучше не связываться с ним.
};
...
{
        B_AI_Output(other,self,"DIA_Khaled_AboutLee_15_00");      //Что ты думаешь о Ли?
        B_AI_Output(self,other,"DIA_Khaled_AboutLee_11_01");      //Я думаю, единственное, что по-настоящему интересует Ли - как убраться с этого острова.
        B_AI_Output(self,other,"DIA_Khaled_AboutLee_11_02");      //Он хочет добраться до материка. Похоже, у него там счеты с кем-то...
        B_AI_Output(self,other,"DIA_Khaled_AboutLee_11_03");      //Но, как бы то ни было, он значительно лучше справляется со своей работой, чем на это способен Сильвио.
        B_AI_Output(self,other,"DIA_Khaled_AboutLee_11_04");      //Кроме того, он хороший лидер. Большую часть времени мы можем делать здесь что хотим.
};
