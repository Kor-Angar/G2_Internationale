...
{
        B_AI_Output(other,self,"DIA_Cipher_HALLO_15_00"); //Отличный лагерь.
        B_AI_Output(self,other,"DIA_Cipher_HALLO_07_01"); //Скажешь тоже. Здесь воняет, как на скотобойне. Повсюду трупы животных и сгнившие деревья.
};
...
{
        B_AI_Output(other,self,"DIA_CipherDJG_HELLOAGAIN_15_00"); //Что ты делаешь здесь?
        B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_07_01"); //Я давно уже здесь сижу. Выжидаю подходящий момент.
        B_AI_Output(other,self,"DIA_CipherDJG_HELLOAGAIN_15_02"); //Подходящий момент? Для чего?
        B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_07_03"); //Там где-то должен прятаться дракон. Это вонючее болото затопило все вокруг вскоре после того, как он появился здесь.
        B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_07_04"); //Я еще хорошо помню время, когда здесь невозможно было ноги замочить.
        B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_07_05"); //Но теперь я ни за что не пойду туда один.
        if(Npc_IsDead(djg_rod) == FALSE)
        {
                B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_07_06"); //Правда, со мной Род. Но он побежит сломя голову даже при виде мясного жука.
        };
        B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_07_07"); //А как насчет тебя? Ты ведь тоже пришел сюда не на прогулку, правда? Пойдем туда вместе.
        b_logentry(TOPIC_DRAGONHUNTER,TOPIC_DRAGONHUNTER_description_121); // "Сифер сказал мне что в болотах Долины рудников живет дракон."
        Info_AddChoice(dia_cipherdjg_helloagain,dia_cipherdjg_helloagain_goalone_description,dia_cipherdjg_helloagain_goalone); // "И что ты собираешься делать дальше?"
        Info_AddChoice(dia_cipherdjg_helloagain,dia_cipherdjg_helloagain_gotogether_description,dia_cipherdjg_helloagain_gotogether); // "Почему бы и нет. Помощь мне не помешает."
};
func void dia_cipherdjg_helloagain_goalone()
{
        B_AI_Output(other,self,"DIA_CipherDJG_HELLOAGAIN_GoAlone_15_00"); //Я предпочитаю действовать в одиночку.
        B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_GoAlone_07_01"); //Как знаешь. Значит, я ошибался в тебе!
        AI_StopProcessInfos(self);
};
func void dia_cipherdjg_helloagain_gotogether()
{
        B_AI_Output(other,self,"DIA_CipherDJG_HELLOAGAIN_GoTogether_15_00");      //Почему бы и нет. Помощь мне не помешает.
        B_AI_Output(self,other,"DIA_CipherDJG_HELLOAGAIN_GoTogether_07_01");      //Это хорошо. Тогда я, наконец, смогу выбраться отсюда. Эта вонь невыносима. Просто дай мне слово!
        DJG_SWAMPPARTY = TRUE;
...
{
        B_AI_Output(other,self,"DIA_CipherDJG_GOTOGETHERAGAIN_15_00");    //Я передумал! Пойдем вместе!
        B_AI_Output(self,other,"DIA_CipherDJG_GOTOGETHERAGAIN_07_01");    //Просто дай мне слово!
        DJG_SWAMPPARTY = TRUE;
};
...
{
        B_AI_Output(other,self,"DIA_CipherDJG_GO_15_00"); //Сейчас самый подходящий момент, чтобы идти туда!
        B_AI_Output(self,other,"DIA_CipherDJG_GO_07_01"); //Тогда пойдем!
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(self,other,"DIA_CipherDJG_SwampWait2_07_00"); //Болото начинается сразу за следующим поворотом. Я предлагаю идти прямо туда.
        B_AI_Output(self,other,"DIA_CipherDJG_SwampWait2_07_01"); //Ну, ты также можешь проверить вон тот проход, чтобы эти твари не напали на нас сзади. Что ты выбираешь?
        B_AI_Output(other,self,"DIA_CipherDJG_SwampWait2_15_02"); //Не будем терять времени!
...
func void dia_cipherdjg_goforswampdragon_info()
{
        ...
        {
                B_AI_Output(self,other,"DIA_CipherDJG_GoForSwampDragon_07_00");   //(рычит) В атаку!
                AI_StopProcessInfos(self);
        ...
        {
                B_AI_Output(self,other,"DIA_CipherDJG_GoForSwampDragon_07_01");   //Я уже слышу эту тварь. Мы должны быть осторожны!
                B_AI_Output(self,other,"DIA_CipherDJG_GoForSwampDragon_07_02");   //Иди вперед и посмотри, можем ли мы атаковать.
                AI_StopProcessInfos(self);
        };
        self.flags = 0;
        djg_rod.flags = 0;
...
{
        B_AI_Output(self,other,"DIA_CipherDJG_SWAMPDRAGONDEAD_07_00");    //Черт. Он мертв?
        B_AI_Output(other,self,"DIA_CipherDJG_SWAMPDRAGONDEAD_15_01");    //Успокойся. Все кончено!
        B_AI_Output(self,other,"DIA_CipherDJG_SWAMPDRAGONDEAD_07_02");    //Ух, ну и громадина.
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(other,self,"DIA_CipherDJG_WHATNEXT_15_00");   //Болотный дракон мертв! Что ты будешь делать дальше?
        B_AI_Output(self,other,"DIA_CipherDJG_WHATNEXT_07_01");   //Понятия не имею. Я еще об этом не думал. А ты теперь можешь вернуться в Хоринис как герой.
        B_AI_Output(self,other,"DIA_CipherDJG_WHATNEXT_07_02");   //Ручаюсь, на этом можно сделать состояние. Подумай над этим.
        b_logentry(TOPIC_DRAGONHUNTER,TOPIC_DRAGONHUNTER_description_122); // "Сифер думает что теперь когда болотный дракон мертв он может заработать кучу денег как великий 'герой'. Что ж посмотрим."
        AI_StopProcessInfos(self);
};
