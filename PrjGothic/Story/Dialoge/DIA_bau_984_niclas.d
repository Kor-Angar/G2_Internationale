...
{
        B_AI_Output(self,other,"DIA_Niclas_Hello_03_00"); //Ты тоже сбежал?
        B_AI_Output(other,self,"DIA_Niclas_Hello_15_01"); //С чего ты взял это?
        B_AI_Output(self,other,"DIA_Niclas_Hello_03_02"); //Зачем ты зашел в такую даль один? Здесь НИЧЕГО нет.
};
...
{
        B_AI_Output(other,self,"DIA_Niclas_HaveALook_15_00");     //Я просто брожу, любуюсь природой.
        B_AI_Output(self,other,"DIA_Niclas_HaveALook_03_01");     //Хорошо, будь моим гостем. Хочешь кусок мяса?
        CreateInvItems(self,itfomutton,1);
        Info_ClearChoices(dia_niclas_havealook);
        Info_AddChoice(dia_niclas_havealook,dia_niclas_havealook_no_description,dia_niclas_havealook_no); // "Нет спасибо."
        Info_AddChoice(dia_niclas_havealook,dia_niclas_havealook_yes_description,dia_niclas_havealook_yes); // "Да спасибо."
};
func void dia_niclas_havealook_no()
{
        B_AI_Output(other,self,"DIA_Niclas_HaveALook_No_15_00");  //Нет, спасибо.
        B_AI_Output(self,other,"DIA_Niclas_HaveALook_No_03_01");  //Как хочешь. Надеюсь, ты не будешь против, если я поем.
        b_useitem(self,itfomutton);
        Info_ClearChoices(dia_niclas_havealook);
};
func void dia_niclas_havealook_yes()
{
        B_AI_Output(other,self,"DIA_Niclas_HaveALook_Yes_15_00"); //Да, спасибо.
        B_AI_Output(self,other,"DIA_Niclas_HaveALook_Yes_03_01"); //Держи, вкуснейшая нога крысокрота. Приготовлена по моему собственному рецепту.
...
{
        B_AI_Output(other,self,"DIA_Niclas_WhyHere_15_00");       //А ты что здесь делаешь?
        B_AI_Output(self,other,"DIA_Niclas_WhyHere_03_01");       //Здесь мне спокойнее. Я всегда чувствую себя неуютно в городе.
};
...
{
        B_AI_Output(other,self,"DIA_Niclas_ShouldntWork_15_00");  //Разве тебе не нужно работать?
        B_AI_Output(self,other,"DIA_Niclas_ShouldntWork_03_01");  //Все, что мне нужно, я могу добыть сам. Я сам вырезаю стрелы, а природа снабжает меня мясом.
        B_AI_Output(other,self,"DIA_Niclas_ShouldntWork_15_02");  //Так ты охотник.
        B_AI_Output(self,other,"DIA_Niclas_ShouldntWork_03_03");  //Да, если тебе нравится это слово. Но я охочусь только для пропитания, а не чтобы заработать денег.
};
...
{
        B_AI_Output(other,self,"DIA_Niclas_CanTeachMe_15_00");    //В таком случае, я уверен, что ты можешь научить меня чему-нибудь.
        B_AI_Output(self,other,"DIA_Niclas_CanTeachMe_03_01");    //Почему бы и нет. Я могу показать тебе, как правильно держать лук.
        Log_CreateTopic(TOPIC_TEACHER,LOG_NOTE);
        b_logentry(TOPIC_TEACHER,TOPIC_TEACHER_description_98); // "Никлас может обучить меня искусству стрельбы из лука."
};
...
{
        B_AI_Output(other,self,"DIA_Niclas_Teach_15_00"); //Покажи мне, как правильно держать лук.
        if(other.hitchance[NPC_TALENT_BOW] >= 50)
        {
                B_AI_Output(self,other,"DIA_Niclas_Teach_03_01"); //Я научил тебя всему, что мог. Теперь тебе лучше поискать другого учителя.
        }
        else
        {
...
func void dia_niclas_teach_back()
{
        if(other.hitchance[NPC_TALENT_BOW] >= 50)
        {
                B_AI_Output(self,other,"DIA_Niclas_Teach_03_00"); //Мне нечему больше учить тебя.
        };
        Info_ClearChoices(dia_niclas_teach);
};
