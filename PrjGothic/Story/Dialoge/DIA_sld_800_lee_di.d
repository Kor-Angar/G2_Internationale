...
{
        B_AI_Output(other,self,"DIA_Lee_DI_Hallo_15_00"); //Что ты будешь делать?
        B_AI_Output(self,other,"DIA_Lee_DI_Hallo_04_01"); //Кто-то должен охранять корабль. Я останусь здесь и позабочусь, чтобы он был на месте, когда ты вернешься.
};
...
{
        B_AI_Output(other,self,"DIA_Lee_DI_PERM6_15_00"); //Как мой корабль?
        if(ORKSTURMDI == FALSE)
        {
                B_AI_Output(self,other,"DIA_Lee_DI_PERM6_04_01"); //Не волнуйся. Я держу все под контролем.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Lee_DI_PERM6_04_02"); //Все отлично. Эти жалкие орки могут возвращаться, когда хотят. Я опять устрою им взбучку.
        };
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Lee_DI_Teach_15_00"); //Я хочу потренироваться.
        B_AI_Output(self,other,"DIA_Lee_DI_Teach_04_01"); //Что именно ты хочешь улучшить?
...
func void dia_lee_di_teach_1h_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lee_DI_Teach_1H_1_04_00");    //Твоя защита ужасна, но мы что-нибудь сделаем с этим.
        };
        Info_ClearChoices(dia_lee_di_teach);
        Info_AddChoice(dia_lee_di_teach,DIALOG_BACK,dia_lee_di_teach_back);
        Info_AddChoice(dia_lee_di_teach,b_buildlearnstring(PRINT_LEARN2H1,b_getlearncosttalent(other,NPC_TALENT_2H)),dia_lee_di_teach_2h_1);
...
func void dia_lee_di_teach_1h_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lee_DI_Teach_1H_5_04_00");    //Твои кисти слишком напряжены. Ты должен держать оружие свободнее.
        };
        Info_ClearChoices(dia_lee_di_teach);
        Info_AddChoice(dia_lee_di_teach,DIALOG_BACK,dia_lee_di_teach_back);
        Info_AddChoice(dia_lee_di_teach,b_buildlearnstring(PRINT_LEARN2H1,b_getlearncosttalent(other,NPC_TALENT_2H)),dia_lee_di_teach_2h_1);
...
func void dia_lee_di_teach_2h_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_DIA_Lee_DI_Teach_2H_1_04_00");        //Всегда помни: боковой удар должен идти от бедра, а не от запястья.
        };
        Info_ClearChoices(dia_lee_di_teach);
        Info_AddChoice(dia_lee_di_teach,DIALOG_BACK,dia_lee_di_teach_back);
        Info_AddChoice(dia_lee_di_teach,b_buildlearnstring(PRINT_LEARN2H1,b_getlearncosttalent(other,NPC_TALENT_2H)),dia_lee_di_teach_2h_1);
...
func void dia_lee_di_teach_2h_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Lee_DI_Teach_2H_5_04_00");    //Сильнейший удар бесполезен, если он приходится в никуда. Так что старайся точно рассчитывать удары.
        };
        Info_ClearChoices(dia_lee_di_teach);
        Info_AddChoice(dia_lee_di_teach,DIALOG_BACK,dia_lee_di_teach_back);
        Info_AddChoice(dia_lee_di_teach,b_buildlearnstring(PRINT_LEARN2H1,b_getlearncosttalent(other,NPC_TALENT_2H)),dia_lee_di_teach_2h_1);
...
{
        B_AI_Output(other,self,"DIA_Lee_DI_UndeadDragonDead_15_00");      //Мы можем отчаливать. Враг мертв.
        B_AI_Output(self,other,"DIA_Lee_DI_UndeadDragonDead_04_01");      //Очень хорошо. Тогда скажи капитану, чтобы поднимал якорь.
        if((DIA_LEE_DI_UNDEADDRAGONDEAD_ONETIME == FALSE) && (hero.guild == GIL_DJG))
        {
                B_AI_Output(self,other,"DIA_Lee_DI_UndeadDragonDead_04_02");      //Ты ведь доставишь меня на материк, да?
                B_AI_Output(other,self,"DIA_Lee_DI_UndeadDragonDead_15_03");      //Да. Хоринис проживет и без тебя.
                B_AI_Output(self,other,"DIA_Lee_Add_04_26");      //Значит, я, наконец, смогу нанести королю так долго откладываемый визит вежливости.
                B_AI_Output(self,other,"DIA_Lee_Add_04_27");      //Я столько ждал этого...
                B_AI_Output(self,other,"DIA_Lee_DI_UndeadDragonDead_04_05");      //Как ты думаешь? Разве терпение не должно быть вознаграждено?
                B_AI_Output(other,self,"DIA_Lee_DI_UndeadDragonDead_15_06");      //Терпение и убийственные доводы.
                B_AI_Output(self,other,"DIA_Lee_DI_UndeadDragonDead_04_07");      //(смеется) Да. Без хорошего отряда ничего не получится. Для меня было большой честью биться с тобой бок о бок.
                DIA_LEE_DI_UNDEADDRAGONDEAD_ONETIME = TRUE;
        };
        B_AI_Output(self,other,"DIA_Lee_DI_UndeadDragonDead_04_08");      //Может быть, когда-нибудь наши пути пересекутся вновь...
        AI_StopProcessInfos(self);
