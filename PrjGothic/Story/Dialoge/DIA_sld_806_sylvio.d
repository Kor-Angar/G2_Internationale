...
{
        B_AI_Output(other,self,"DIA_Sylvio_Hallo_15_00"); //Как дела?
        B_AI_Output(self,other,"DIA_Sylvio_Hallo_09_01"); //Разве я давал тебе разрешение обратиться ко мне?
...
{
        B_AI_Output(other,self,"DIA_Sylvio_Thekla_15_00");        //Текла говорит, что у нее проблемы с тобой.
        B_AI_Output(self,other,"DIA_Sylvio_Thekla_09_01");        //Да? И что за проблемы у этой милой леди? И почему она не пришла сама?
        B_AI_Output(other,self,"DIA_Sylvio_Thekla_15_02");        //Ты прекрасно знаешь, черт побери...
        B_AI_Output(self,other,"DIA_Sylvio_Thekla_09_03");        //(цинично-дружелюбно) Почему бы тебе не обсудить этот вопрос с моим другом Булко?
        SYLVIO_ANGEQUATSCHT = SYLVIO_ANGEQUATSCHT + 1;
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Sylvio_Gossip_15_00");        //О тебе многие говорят...
        B_AI_Output(self,other,"DIA_Sylvio_Gossip_09_01");        //Эти многие говорят слишком много.
        SYLVIO_ANGEQUATSCHT = SYLVIO_ANGEQUATSCHT + 1;
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Sylvio_AboutLee_15_00");      //Что ты думаешь о Ли?
        B_AI_Output(self,other,"DIA_Sylvio_AboutLee_09_01");      //(слегка иронично) Ох, он хороший боец. Я бы не стал вступать в схватку с ним.
        B_AI_Output(self,other,"DIA_Sylvio_AboutLee_09_02");      //(холодно) Если конечно, ее можно было бы избежать.
        SYLVIO_ANGEQUATSCHT = SYLVIO_ANGEQUATSCHT + 1;
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(self,other,"DIA_Sylvio_MenDefeated_09_00");   //Мне кажется, ты имеешь зуб на моих парней!
        B_AI_Output(self,other,"DIA_Sylvio_MenDefeated_09_01");   //Насколько я знаю, ты один из протеже Ли.
        B_AI_Output(self,other,"DIA_Sylvio_MenDefeated_09_02");   //Я предупреждаю тебя! Скоро здесь многое переменится, и тогда мы вернемся к этому разговору!
...
{
        B_AI_Output(other,self,"DIA_Sylvio_Asshole_15_00");       //Эй, дерьмо...
        B_AI_Output(self,other,"DIA_Sylvio_Asshole_09_01");       //Ты что, действительно думаешь, что я вызову тебя на дуэль?
        B_AI_Output(self,other,"DIA_Sylvio_Asshole_09_02");       //Проваливай туда, откуда ты выполз.
        SYLVIO_ANGEQUATSCHT = SYLVIO_ANGEQUATSCHT + 1;
        AI_StopProcessInfos(self);
};
