...
{
        B_AI_Output(other,self,"DIA_Gorn_DI_Hallo_15_00");        //Все тип-топ?
        if(ORKSTURMDI == FALSE)
        {
                B_AI_Output(self,other,"DIA_Gorn_DI_Hallo_12_01");        //Пока да. Помни, я хочу действовать.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Gorn_DI_Hallo_12_02");        //Там еще есть орки? Пришли их ко мне. Я как раз немного разогрелся.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Gorn_DI_Teach_15_00");        //Я хочу потренироваться.
        B_AI_Output(self,other,"DIA_Gorn_DI_Teach_12_01");        //Это не помешает.
...
func void dia_gorn_di_teach_2h_1()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gorn_DI_Teach_2H_1_12_00");   //Да уж. Когда-то ты был лучше.
        };
        Info_ClearChoices(dia_gorn_di_teach);
        Info_AddChoice(dia_gorn_di_teach,DIALOG_BACK,dia_gorn_di_teach_back);
        Info_AddChoice(dia_gorn_di_teach,b_buildlearnstring(PRINT_LEARN2H5,b_getlearncosttalent(other,NPC_TALENT_2H) * 5),dia_gorn_di_teach_2h_5);
...
func void dia_gorn_di_teach_2h_5()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gorn_DI_Teach_2H_5_12_00");   //Оружие нужно держать выше. Даже слепой легко пробьет твою защиту своей тростью.
        };
        Info_ClearChoices(dia_gorn_di_teach);
        Info_AddChoice(dia_gorn_di_teach,DIALOG_BACK,dia_gorn_di_teach_back);
        Info_AddChoice(dia_gorn_di_teach,b_buildlearnstring(PRINT_LEARN2H5,b_getlearncosttalent(other,NPC_TALENT_2H) * 5),dia_gorn_di_teach_2h_5);
...
{
        B_AI_Output(other,self,"DIA_Gorn_DI_UndeadDragonDead_15_00");     //Ты все это время был на корабле?
        B_AI_Output(self,other,"DIA_Gorn_DI_UndeadDragonDead_12_01");     //Конечно. Только представь, чтоб мы делали, если бы он пропал.
        if(DIA_GORN_DI_UNDEADDRAGONDEAD_ONETIME == FALSE)
        {
                if(hero.guild == GIL_DJG)
                {
                        B_AI_Output(self,other,"DIA_Gorn_DI_UndeadDragonDead_12_02");     //А что случилось?
                        B_AI_Output(other,self,"DIA_Gorn_DI_UndeadDragonDead_15_03");     //Ничего. Я собираюсь отправляться домой.
                        B_AI_Output(self,other,"DIA_Gorn_DI_UndeadDragonDead_12_04");     //(смеется) Домой? А где это? Насколько я знаю, у тебя нет дома.
                        B_AI_Output(self,other,"DIA_Gorn_DI_UndeadDragonDead_12_05");     //Хочешь предложение? Мы ворвемся в первую же попавшуюся таверну и напьемся до бессознательного состояния.
                        B_AI_Output(other,self,"DIA_Gorn_DI_UndeadDragonDead_15_06");     //Ммм. Может быть.
                        B_AI_Output(self,other,"DIA_Gorn_DI_UndeadDragonDead_12_07");     //Эй, расслабься, дружище. Все кончено.
                };
                DIA_GORN_DI_UNDEADDRAGONDEAD_ONETIME = TRUE;
        };
        if(Npc_KnowsInfo(other,dia_biff_di_plunder))
        {
                B_AI_Output(self,other,"DIA_Gorn_DI_UndeadDragonDead_12_08");     //Кто-то должен сказать об этом Биффу, иначе он сгниет на этом острове.
                if(Npc_IsDead(biff_di) == FALSE)
                {
                        B_AI_Output(other,self,"DIA_Gorn_DI_UndeadDragonDead_15_09");     //Он будет здесь вовремя.
                };
        };
        B_AI_Output(self,other,"DIA_Gorn_DI_UndeadDragonDead_12_10");     //Здесь нам больше нечего делать. Скажи капитану, чтобы поднимал якорь.
        AI_StopProcessInfos(self);
