...
{
        B_AI_Output(self,other,"DIA_KDF_507_Talamon_FirstWarn_04_00");    //Тебе нельзя идти дальше. Поворачивай назад!
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(self,other,"DIA_KDF_507_Talamon_SecondWarn_04_00");   //Иннос обратит свой гнев против тебя, если ты не повернешь назад!
        other.aivar[AIV_LASTDISTTOWP] = Npc_GetDistToWP(other,KDF_507_CHECKPOINT);
...
{
        B_AI_Output(self,other,"DIA_Talamon_KAP5_Stop_04_00");    //Тебе нельзя идти дальше. Поворачивай назад!
        B_AI_Output(other,self,"DIA_Talamon_KAP5_Stop_15_01");    //Пирокар сказал, что мне можно взять книгу Ксардаса.
        B_AI_Output(self,other,"DIA_Talamon_KAP5_Stop_04_02");    //Ну, если он это сказал... Хорошо, ты можешь войти. Книга, которую ты ищешь, находится вон там, на алхимическом столе.
        b_logentry(TOPIC_BUCHHALLENVONIRDORATH,TOPIC_BUCHHALLENVONIRDORATH_description_217); // "Таламон держал книгу Ксардаса на алхимическом столе в подвале."
};
...
{
        B_AI_Output(other,self,"DIA_Talamon_FoundSecretDoor_15_00");      //Я нашел секретную дверь.
        B_AI_Output(self,other,"DIA_Talamon_FoundSecretDoor_04_01");      //(удивленно) Что? Где?
        B_AI_Output(other,self,"DIA_Talamon_FoundSecretDoor_15_02");      //За книжной полкой.
        B_AI_Output(self,other,"DIA_Talamon_FoundSecretDoor_04_03");      //А что за дверью?
        B_AI_Output(other,self,"DIA_Talamon_FoundSecretDoor_15_04");      //Похоже, там старый подвал.
        B_AI_Output(self,other,"DIA_Talamon_FoundSecretDoor_04_05");      //Это важная новость, я должен немедленно сообщить об этом Пирокару.
        B_AI_Output(self,other,"DIA_Talamon_FoundSecretDoor_04_06");      //А пока ты должен выяснить, что там в этом подвале.
...
{
        B_AI_Output(other,self,"DIA_Talamon_ScoutSecretLibrary_15_00");   //Я был в этом подвале.
        B_AI_Output(self,other,"DIA_Talamon_ScoutSecretLibrary_04_01");   //И что ты там нашел?
        if(Npc_IsDead(secretlibraryskeleton))
        {
                B_AI_Output(other,self,"DIA_Talamon_ScoutSecretLibrary_15_02");   //Там был скелет воина, охраняющий дверь. Я убил его.
                B_AI_Output(self,other,"DIA_Talamon_ScoutSecretLibrary_04_03");   //Отлично.
                B_AI_Output(self,other,"DIA_Talamon_ScoutSecretLibrary_04_04");   //Мы разберемся с этим подвалом, когда победим Зло.
                MIS_SCOUTLIBRARY = LOG_SUCCESS;
                b_giveplayerxp(XP_SCOUTSECRETLIBRARY);
        }
        else
        {
                B_AI_Output(other,self,"DIA_Talamon_ScoutSecretLibrary_15_05");   //Там, внизу, толпы монстров.
                B_AI_Output(self,other,"DIA_Talamon_ScoutSecretLibrary_04_06");   //Там должно что-то быть, продолжай искать. И уничтожай этих монстров без всякой жалости.
        };
};
