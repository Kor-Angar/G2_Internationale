...
{
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_FirstWarn_01_00");       //Стой! Куда это ты идешь?
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_FirstWarn_15_01");       //Я хотел...
        if((hero.guild == GIL_PAL) || (hero.guild == GIL_KDF))
        {
                B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_FirstWarn_01_02");       //Извини. Но тебе сюда нельзя.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_FirstWarn_01_03");       //Здесь не на что смотреть. Иди своей дорогой.
        };
...
func void dia_pal_213_schiffswache_secondwarn_info()
{
        if((hero.guild == GIL_PAL) || (hero.guild == GIL_KDF))
        {
                B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_SecondWarn_01_00");      //Ни шагу дальше. Исключений нет ни для кого.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_SecondWarn_01_01");      //Ты же не хочешь ЗАСТАВИТЬ меня ударить тебя, правда?
        };
        other.aivar[AIV_LASTDISTTOWP] = Npc_GetDistToWP(other,PAL_213_CHECKPOINT);
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_GoOnBoard_15_00");       //Я хочу попасть на борт корабля.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_GoOnBoard_01_01");       //Никому не позволено входить на корабль! У меня есть приказ!
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_GoOnBoard_01_02");       //Я должен убить всякого, кто войдет на корабль без соответствующего разрешения.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_GoOnBoard_01_03");       //Во имя Инноса, я буду защищать этот корабль даже ценой своей жизни.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF_15_00");  //Ты сомневаешься в намерениях Мага Огня?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF_01_01");  //(нервно) Нет, конечно же, нет. Да простит меня Иннос.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF_01_02");  //(нервно) Маги Огня - стражи мудрости Инноса...
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF_01_03");  //(нервно) ...тот, кто сомневается в них, сомневается в Инносе и не заслуживает пощады.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF2_15_00"); //Что будет, если я взойду на борт?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF2_01_01"); //(нервно) Я должен убить... я хочу сказать, остановить тебя.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF3_15_00"); //Ты осмелишься атаковать Мага Огня?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF3_01_01"); //(нервно) Я никогда не подниму руку на мага.
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF3_15_02"); //Так что ты все-таки будешь делать, если я взойду на борт?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF3_01_03"); //(робко) Ничего, господин.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF4_15_00"); //Тогда я сейчас взойду на борт.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF4_01_01"); //(нервно) Ты не должен делать этого, приказы лорда Хагена были вполне конкретными.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF5_15_00"); //Приказы лорда Хагена касаются и меня тоже?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF5_01_01"); //(нервно) Я не знаю.
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF5_15_02"); //Сам подумай - разве лорд Хаген осмелился бы подозревать мага в воровстве?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF5_01_03"); //Я в это не верю.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmKDF6_15_00"); //В последний раз: позволь мне взойти на корабль!
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmKDF6_01_01"); //(нервно) Понятно. Ты можешь взойти на корабль.
        MIS_SHIPISFREE = TRUE;
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmPAL_15_00");  //Ты не доверяешь мне, рыцарь?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmPAL_01_01");  //Никак нет, я просто выполняю приказ.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmPAL2_15_00"); //Тогда ты должен знать, кто из нас выше званием.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmPAL2_01_01"); //Так точно!
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmPAL3_15_00"); //И я приказываю тебе пропустить меня на этот корабль.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmPAL3_01_01"); //Есть, сэр, вы можете взойти на корабль.
        MIS_SHIPISFREE = TRUE;
        b_startotherroutine(girion,"WaitForShip");
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmDJG_15_00");  //А этот вопрос нельзя как-нибудь решить?
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmDJG_01_01");  //Я не понимаю.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmDJG2_15_00"); //Я могу дать тебе денег. За то, что ты будешь смотреть в другую сторону.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmDJG2_01_01"); //Меня нельзя подкупить, и если ты сейчас не исчезнешь, я сочту это оскорблением.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_213_Schiffswache_IAmDJG3_15_00"); //У меня письменное разрешение. Я уполномочен взойти на этот корабль.
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmDJG3_01_01"); //Дай я взгляну.
        b_giveinvitems(other,self,itwr_forgedshipletter_mis,1);
        b_usefakescroll();
        B_AI_Output(self,other,"DIA_Pal_213_Schiffswache_IAmDJG3_01_02"); //Все правильно, ты можешь пройти.
        MIS_SHIPISFREE = TRUE;
        b_startotherroutine(girion,"WaitForShip");
};
