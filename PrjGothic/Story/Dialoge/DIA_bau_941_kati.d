...
func void dia_kati_sldnochda_info()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Kati_SLDNOCHDA_16_00");       //Эти головорезы угрожают моему мужу! Мы простые граждане Хориниса, преданные королю, а эти наемники хотят ограбить нас!
                Kati_AskedForHelp = TRUE;
        };
        B_AI_Output(self,other,"DIA_Kati_SLDNOCHDA_16_01");       //Ну не стой же здесь. Сделай что-нибудь! Помоги нам!
        AKILS_SLDSTILLTHERE = TRUE;
        Log_CreateTopic(TOPIC_AKILSSLDSTILLTHERE,LOG_MISSION);
        Log_SetTopicStatus(TOPIC_AKILSSLDSTILLTHERE,LOG_RUNNING);
        b_logentry(TOPIC_AKILSSLDSTILLTHERE,TOPIC_AKILSSLDSTILLTHERE_description_39); // "Фермеру Акилу угрожают наемники."
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Kati_HALLO_15_00");   //С тобой все в порядке?
        if(Npc_IsDead(akil))
        {
                B_AI_Output(self,other,"DIA_Kati_HALLO_16_01");   //(рыдает) Мой любимый муж мертв! Ох, Иннос, за что ты меня так наказываешь?!
        ...
        {
                B_AI_Output(self,other,"DIA_Kati_HALLO_16_02");   //Да, я в порядке, спасибо.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Kati_ESSEN_15_00");   //Акил говорит, что ты можешь накормить меня.
        B_AI_Output(self,other,"DIA_Kati_ESSEN_16_01");   //С тех пор как рухнул Барьер, для нас настали тяжелые времена. Жить здесь стало небезопасно.
        B_AI_Output(self,other,"DIA_Kati_ESSEN_16_02");   //Вот, держи ломоть хлеба, немного молока и бутылку воды. Извини, но это все, чем мы можем поделиться.
...
{
        B_AI_Output(other,self,"DIA_Kati_Baltram_15_00"); //Меня прислал Бальтрам. Я должен забрать посылку для него.
        B_AI_Output(self,other,"DIA_Kati_Baltram_16_01"); //Да, конечно. Вот, я уже упаковала ее.
...
{
        B_AI_Output(other,self,"DIA_Kati_BAUERNAUFSTAND_15_00");  //Почему вы не противостоите тирании Онара?
        B_AI_Output(self,other,"DIA_Kati_BAUERNAUFSTAND_16_01");  //Для фермеров, живущих у города, это имеет смысл. Им лучше быть на стороне ополчения, чем полагаться на наемников Онара.
        B_AI_Output(self,other,"DIA_Kati_BAUERNAUFSTAND_16_02");  //С другой стороны, есть Бенгар и Секоб, которые скорее откажутся от своих ферм, чем будут работать на короля.
};
...
{
        B_AI_Output(other,self,"DIA_Kati_ANDEREHOEFE_15_00");     //Где находятся фермы Бенгара и Секоба?
        B_AI_Output(self,other,"DIA_Kati_ANDEREHOEFE_16_01");     //Неподалеку от фермы лендлорда. Иди на восток отсюда, и ты найдешь их.
};
...
{
        B_AI_Output(other,self,"DIA_Kati_HIERWEG_15_00"); //А вы не думали о том, чтобы уехать отсюда?
        B_AI_Output(self,other,"DIA_Kati_HIERWEG_16_01"); //Не так-то просто уехать из этой части страны. Вся эта земля окружена стеной высоких, непроходимых гор.
        B_AI_Output(self,other,"DIA_Kati_HIERWEG_16_02"); //Выбраться отсюда можно только лежит через Долину Рудников или через гавань города.
        B_AI_Output(self,other,"DIA_Kati_HIERWEG_16_03"); //Так как мы не можем позволить себе купить место на корабле, а Долина Рудников - это место, откуда не возвращаются, мы вынуждены оставаться здесь.
};
...
{
        B_AI_Output(other,self,"DIA_Kati_PASS_15_00");    //Что ты знаешь о Проходе?
        B_AI_Output(self,other,"DIA_Kati_PASS_16_01");    //Сама я там никогда не была. Но я знаю, что он где-то неподалеку от фермы Бенгара на высокогорных пастбищах.
};
...
{
        B_AI_Output(other,self,"DIA_Kati_PERMKAP1_15_00");        //Береги своего мужа.
        B_AI_Output(self,other,"DIA_Kati_PERMKAP1_16_01");        //Я стараюсь изо всех сил.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Kati_PERM_15_00");    //С тобой все в порядке?
        B_AI_Output(self,other,"DIA_Kati_PERM_16_01");    //Мы справимся. Вот только не знаю, сколько еще нам придется терпеть этих дьяволов в черном.
        B_AI_Output(self,other,"DIA_Kati_PERM_16_02");    //Я так долго не вынесу. Они шныряют вокруг дома и везде суют свой нос.
};
