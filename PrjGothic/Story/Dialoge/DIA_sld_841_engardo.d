...
func void dia_engardo_hallo_info()
{
        ...
        if(CHANCE == FALSE)
        {
                B_AI_Output(self,other,"DIA_Engardo_HALLO_13_00");        //Ёй, мужик, ты что, заблудилс€? ¬озвращайс€ на свое поле и дергай репу.
                CHANCE = 1;
                AI_StopProcessInfos(self);
        }
        else if(CHANCE == 1)
        {
                B_AI_Output(self,other,"DIA_Engardo_HALLO_13_01");        //“ы оказалс€ не в том месте не в то врем€...
                B_AI_Output(self,other,"DIA_Engardo_HALLO_13_02");        //...так что, если хочешь жить, лучше топай отсюда. ѕон€л?
                CHANCE = 2;
                AI_StopProcessInfos(self);
        }
        else if(CHANCE == 2)
        {
                B_AI_Output(self,other,"DIA_Engardo_HALLO_13_03");        //” теб€ что, со слухом не все в пор€дке, или тебе очень хочетс€ умереть? (грубо) Ћадно - все равно уже слишком поздно.
                AI_StopProcessInfos(self);
