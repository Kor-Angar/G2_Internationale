...
{
        B_AI_Output(other,self,"DIA_Grom_HALLO_15_00");   //Все в порядке?
        B_AI_Output(self,other,"DIA_Grom_HALLO_08_01");   //Ах, незнакомый странник. Я очень занят. Что ты хочешь?
        Info_ClearChoices(dia_grom_hallo);
        Info_AddChoice(dia_grom_hallo,dia_grom_hallo_waszusehen_description,dia_grom_hallo_waszusehen); // "Что здесь интересного?"
        Info_AddChoice(dia_grom_hallo,dia_grom_hallo_was_description,dia_grom_hallo_was); // "Что ты делаешь здесь?"
};
func void dia_grom_hallo_waszusehen()
{
        B_AI_Output(other,self,"DIA_Grom_HALLO_waszusehen_15_00");        //На что интересное стоит обратить внимание здесь?
        B_AI_Output(self,other,"DIA_Grom_HALLO_waszusehen_08_01");        //Интересное - это хорошо сказано. Если ты углубишься в здешний лес, то наткнешься на очень злобных парней.
        B_AI_Output(self,other,"DIA_Grom_HALLO_waszusehen_08_02");        //Они около десяти футов высотой, волосатые и в очень дурном расположении духа. Так что лучше не ходи туда, если не считаешь, что достаточно силен.
};
func void dia_grom_hallo_was()
{
        B_AI_Output(other,self,"DIA_Grom_HALLO_was_15_00");       //Чем ты занимаешься здесь?
        B_AI_Output(self,other,"DIA_Grom_HALLO_was_08_01");       //Ну, чем обычно занимаются дровосеки и охотники?
...
{
        B_AI_Output(other,self,"DIA_Grom_AskTeacher_15_00");      //Ты можешь научить меня охотиться?
        B_AI_Output(self,other,"DIA_Grom_AskTeacher_08_01");      //Конечно. Но сначала принеси мне приличной еды. Я умираю от голода.
        B_AI_Output(self,other,"DIA_Grom_AskTeacher_08_02");      //Я хочу бутыль молока, буханку хлеба и жирный окорок. И тогда я научу тебя всему, что сам знаю.
        Log_CreateTopic(TOPIC_GROMASKTEACHER,LOG_MISSION);
        Log_SetTopicStatus(TOPIC_GROMASKTEACHER,LOG_RUNNING);
        b_logentry(TOPIC_GROMASKTEACHER,TOPIC_GROMASKTEACHER_description_93); // "Охотник Гром научит меня всему что знает если я принесу ему бутылку молока буханку хлеба и окорок."
};
...
func void dia_grom_payteacher_info()
{
        ...
        B_AI_Output(other,self,"DIA_Grom_PayTeacher_15_00");      //Вот еда, как ты просил.
        B_AI_Output(self,other,"DIA_Grom_PayTeacher_08_01");      //Фантастика. И что теперь?
...
{
        B_AI_Output(other,self,"DIA_Grom_TEACHHUNTING_15_00");    //Научи меня охотиться.
        ...
        {
                B_AI_Output(self,other,"DIA_Grom_TEACHHUNTING_08_01");    //Что ты хочешь узнать?
        ...
        {
                B_AI_Output(self,other,"DIA_Grom_TEACHHUNTING_08_02");    //Мне нечему учить тебя. Ты и так уже все знаешь.
        };
};
...
func void dia_grom_teachhunting_fur()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Grom_TEACHHUNTING_Fur_08_00");        //Сделай разрез вдоль ног животного, чтобы ты мог снять с него шкуру. Снимай ее с зада наперед, а не наоборот.
        };
        Info_ClearChoices(dia_grom_teachhunting);
};
func void dia_grom_teachhunting_teeth()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Grom_TEACHHUNTING_Teeth_08_00");      //Челюсть животного нужно вынимать осторожно, чтобы не разломить ее, когда она отделяется от черепа.
        };
        Info_ClearChoices(dia_grom_teachhunting);
};
func void dia_grom_teachhunting_heart()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Grom_TEACHHUNTING_Heart_08_00");      //Ты можешь вырезать сердце животного, сделав глубокий разрез в его грудной клетке. Но обычно это имеет смысл только в случае особых животных или магических существ.
        };
        Info_ClearChoices(dia_grom_teachhunting);
};
func void dia_grom_teachhunting_mandibles()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Grom_TEACHHUNTING_Mandibles_08_00");  //Мандибулы краулера или полевого хищника нужно отделять при помощи очень прочного металлического рычага. Они очень глубоко сидят в черепе этих животных.
        };
        Info_ClearChoices(dia_grom_teachhunting);
};
func void dia_grom_teachhunting_shadowhorn()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Grom_TEACHHUNTING_ShadowHorn_08_00"); //Чтобы отделить рог мракориса, надави на него всем своим весом, подрезая при этом основание острым ножом.
        };
        Info_ClearChoices(dia_grom_teachhunting);
};
