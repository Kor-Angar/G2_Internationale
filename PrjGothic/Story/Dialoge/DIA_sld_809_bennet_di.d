...
{
        B_AI_Output(other,self,"DIA_Bennet_DI_Hello_15_00");      //У тебя все в порядке?
        if(ORKSTURMDI == FALSE)
        {
                B_AI_Output(self,other,"DIA_Bennet_DI_Hello_06_01");      //Кузница на корабле немного проржавела. Морская соль разъедает ее. Здесь будет непросто выковать что-нибудь приличное. Ну, а кроме этого...
        }
        else
        {
                B_AI_Output(self,other,"DIA_Bennet_DI_Hello_06_02");      //Да. Если только орки не вернутся.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Bennet_DI_TRADE_15_00");      //Какое оружие ты можешь продать мне?
        b_givetradeinv(self);
        B_AI_Output(self,other,"DIA_Bennet_DI_TRADE_06_01");      //Только лучшее. Ты же знаешь.
};
...
{
        B_AI_Output(other,self,"DIA_Bennet_DI_Smith_15_00");      //Ты можешь обучить меня своему мастерству?
        B_AI_Output(self,other,"DIA_Bennet_DI_Smith_06_01");      //Это зависит от того, что ты хочешь научиться делать.
...
{
        B_AI_Output(other,self,"DIA_Bennet_TeachSTR_15_00");      //Я хочу стать сильнее.
        B_AI_Output(self,other,"DIA_Bennet_TeachSTR_06_01");      //В наше время твердая рука никогда не помешает.
...
{
        B_AI_Output(other,self,"DIA_Bennet_DI_DragonEgg_15_00");  //У меня есть драконье яйцо.
        B_AI_Output(self,other,"DIA_Bennet_DI_DragonEgg_06_01");  //И?
        B_AI_Output(other,self,"DIA_Bennet_DI_DragonEgg_15_02");  //Ну. Я подумал...
        B_AI_Output(self,other,"DIA_Bennet_DI_DragonEgg_06_03");  //Я знаю, о чем ты думаешь. Забудь об этом и оставь яйцо себе. Мне оно не нужно.
...
{
        B_AI_Output(other,self,"DIA_Bennet_DI_UndeadDragonDead_15_00");   //Мы сделали все, что было нужно сделать.
        B_AI_Output(self,other,"DIA_Bennet_DI_UndeadDragonDead_06_01");   //Рад слышать это. Мне уже успела надоесть эта старая кузница. Я хочу опять стоять на твердой земле.
        AI_StopProcessInfos(self);
};
