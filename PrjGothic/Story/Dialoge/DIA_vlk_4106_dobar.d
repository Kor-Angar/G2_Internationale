...
{
        B_AI_Output(self,other,"DIA_Dobar_HALLO_08_00");  //(раздражительно) Что тебе нужно?
};
...
{
        B_AI_Output(other,self,"DIA_Dobar_Talent_15_00"); //Я немного знаю кузнечное дело.
        B_AI_Output(self,other,"DIA_Dobar_Talent_08_01"); //Хорошо... и что?
};
...
{
        B_AI_Output(other,self,"DIA_Dobar_Schmiede_15_00");       //Могу я воспользоваться твоей кузницей?
        B_AI_Output(self,other,"DIA_Dobar_Schmiede_08_01");       //У меня много работы. Мы будем только мешать друг другу. Лучше дождись темноты.
        B_AI_Output(self,other,"DIA_Dobar_Schmiede_08_02");       //Тогда Парлаф и я пойдем спать, и ты сможешь спокойно поработать.
};
...
{
        B_AI_Output(other,self,"DIA_Dobar_beibringen_15_00");     //Ты можешь научить меня чему-нибудь?
        B_AI_Output(self,other,"DIA_Dobar_beibringen_08_01");     //Ты уже знаешь основы. Я могу показать тебе, как улучшить твою работу.
        B_AI_Output(self,other,"DIA_Dobar_beibringen_08_02");     //И ты сможешь ковать более совершенное оружие.
        DOBAR_LEARNSMITH = TRUE;
        Log_CreateTopic(TOPIC_TEACHER_OC,LOG_NOTE);
        b_logentry(TOPIC_TEACHER_OC,TOPIC_TEACHER_OC_description_562); // "Добар может обучить меня ковать хорошее оружие."
};
...
{
        B_AI_Output(other,self,"DIA_Dobar_Teach_15_00");  //Покажи мне, как выковать хороший меч!
        if(b_teachplayertalentsmith(self,hero,WEAPON_1H_SPECIAL_01))
        {
                B_AI_Output(self,other,"DIA_Dobar_Teach_08_01");  //Разогрей сталь, чтобы она равномерно светилась по всей длине, без этого хороший меч не выковать.
                B_AI_Output(self,other,"DIA_Dobar_Teach_08_02");  //Если ты будешь помнить это, сделанные тобой клинки будут прочнее и острее.
                B_AI_Output(self,other,"DIA_Dobar_Teach_08_03");  //Это все, что тебе нужно знать. Если тебе нужна сырая сталь, поговори с Энгором.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Dobar_Waffe_15_00");  //Ты можешь сделать оружие для меня?
        B_AI_Output(self,other,"DIA_Dobar_Waffe_08_01");  //У меня нет времени на это. Тандор распоряжается всем оружием - я только кую его, а Парлаф затачивает.
        Log_CreateTopic(TOPIC_TRADER_OC,LOG_NOTE);
        b_logentry(TOPIC_TRADER_OC,TOPIC_TRADER_OC_description_563); // "Тандор торгует оружием в замке."
};
...
{
        B_AI_Output(other,self,"DIA_Dobar_NEWS_15_00");   //Как работа?
        if(DOBAR_EINMALIG == FALSE)
        {
                B_AI_Output(self,other,"DIA_Dobar_NEWS_08_01");   //Я кую оружие для рыцарей замка. С тех пор, как мы прибыли сюда, я не отхожу от кузницы.
                B_AI_Output(self,other,"DIA_Dobar_NEWS_08_02");   //Это оружие нам очень пригодится. Мы еще покажем этим проклятым оркам!
                if(Npc_IsDead(parlaf) == FALSE)
                {
                        b_turntonpc(self,parlaf);
                        B_AI_Output(self,other,"DIA_Dobar_NEWS_08_03");   //(зовет) Эй, Парлаф - затачивай эти мечи получше - орки чертовски сильные противники!
        ...
        {
                B_AI_Output(self,other,"DIA_Dobar_NEWS_08_04");   //Могло бы быть и лучше. Если бы ты постоянно не отвлекал меня, работа продвигалась бы быстрее.
        };
};
