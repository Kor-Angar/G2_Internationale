...
{
        B_AI_Output(self,other,"DIA_Biff_DI_HALLO_07_00");        //И? Где все те сокровища, что ты мне обещал?
        if(Npc_KnowsInfo(other,dia_biff_di_orks) == FALSE)
        {
                B_AI_Output(other,self,"DIA_Biff_DI_HALLO_15_01");        //Что я там говорил тебе раньше, в море?
        };
        B_AI_Output(other,self,"DIA_Biff_DI_HALLO_15_02");        //Сейчас твоя задача - охранять корабль.
        B_AI_Output(other,self,"DIA_Biff_DI_HALLO_15_03");        //Мне не улыбается плыть назад.
        B_AI_Output(self,other,"DIA_Biff_DI_HALLO_07_04");        //Черт. Если бы я знал это заранее, я бы лучше остался в Хоринисе.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Biff_DI_perm_15_00"); //На борту все спокойно?
        B_AI_Output(self,other,"DIA_Biff_DI_perm_07_01"); //(сердито) Да, да. Все тихо.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(self,other,"DIA_Biff_DI_ORKS_07_00"); //Эти мерзкие твари!
        B_AI_Output(other,self,"DIA_Biff_DI_ORKS_15_01"); //Черт, что ты делаешь здесь? Ты должен был охранять корабль.
        B_AI_Output(self,other,"DIA_Biff_DI_ORKS_07_02"); //Да никуда он не денется.
...
{
        B_AI_Output(self,other,"DIA_Biff_DI_UNDEADDRGDEAD_07_00");        //Что, все кончено?
        B_AI_Output(other,self,"DIA_Biff_DI_UNDEADDRGDEAD_15_01");        //Вроде бы да.
        B_AI_Output(self,other,"DIA_Biff_DI_UNDEADDRGDEAD_07_02");        //И теперь, могу я...
        B_AI_Output(other,self,"DIA_Biff_DI_UNDEADDRGDEAD_15_03");        //Ты можешь перевернуть весь остров, ели хочешь.
        B_AI_Output(self,other,"DIA_Biff_DI_UNDEADDRGDEAD_07_04");        //Отлично.
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(self,other,"DIA_Biff_DI_plunder_07_00");      //Черт. Не сейчас.
        AI_StopProcessInfos(self);
};
