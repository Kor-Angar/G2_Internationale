...
{
        B_AI_Output(self,other,"DIA_Cedric_Hallo_12_00"); //я —едрик, мастер меча и паладин корол€.
};
...
{
        B_AI_Output(other,self,"DIA_Cedric_CanTeach_15_00");      //“ы можешь обучить мен€?
        if(other.guild == GIL_PAL)
        {
                B_AI_Output(self,other,"DIA_Cedric_CanTeach_12_01");      //я могу научить теб€ сражатьс€ одноручным оружием.
                CEDRIC_TEACH1H = TRUE;
                b_logentry(TOPIC_CITYTEACHER,TOPIC_CITYTEACHER_description_336); // "ѕаладин —едрик может обучить мен€ искусству сражени€ одноручным оружием."
        }
        else
        {
                B_AI_Output(self,other,"DIA_Cedric_CanTeach_12_02");      //я тренирую только паладинов.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Cedric_Teach_15_00"); //я хочу потренироватьс€.
        if(other.hitchance[NPC_TALENT_1H] >= 90)
        {
                B_AI_Output(self,other,"DIA_DIA_Cedric_Teach_12_01");     //“ы уже владеешь мечом лучше мен€. ћне нечему учить теб€.
        }
        else
        {
