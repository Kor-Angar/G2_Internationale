...
{
        B_AI_Output(other,self,"DIA_Malak_HALLO_15_00");  //Все в порядке?
        if(hero.guild == GIL_NONE)
        {
                B_AI_Output(self,other,"DIA_Malak_HALLO_08_01");  //Еще один поденный рабочий, который не знает, чем заняться? Нет проблем. Поговори с нашим фермером Бенгаром.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Malak_HALLO_08_02");  //Конечно.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Malak_WASMACHSTDU_15_00");    //Что ты делаешь здесь?
        B_AI_Output(self,other,"DIA_Malak_WASMACHSTDU_08_01");    //Я пасу овец. Это не такая уж тяжелая работа.
        B_AI_Output(self,other,"DIA_Malak_WASMACHSTDU_08_02");    //Но иногда нужно быть очень внимательным. Особенно, если приближаешься к Проходу.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_PASS_15_00");   //Что ты знаешь о Проходе?
        B_AI_Output(self,other,"DIA_Malak_PASS_08_01");   //Не много. Только то, что он ведет в старую Долину Рудников, которая была окружена Барьером еще несколько недель назад.
        B_AI_Output(self,other,"DIA_Malak_PASS_08_02");   //Тогда единственное, чего нужно было опасаться нам, фермерам, это ежемесячного каравана, доставлявшего различные грузы в Долину Рудников.
        B_AI_Output(self,other,"DIA_Malak_PASS_08_03");   //Эти головорезы сильно усложняли нам жизнь.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_WOPASS_15_00"); //Где мне найти проход в старую Долину Рудников?
        B_AI_Output(self,other,"DIA_Malak_WOPASS_08_01"); //Около двух водопадов на другой стороне этого пастбища.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_MINENTAL_15_00");       //Что говорят о Долине Рудников?
        B_AI_Output(self,other,"DIA_Malak_MINENTAL_08_01");       //Разное. Некоторые говорят, что слышали крики по ночам, другие видели странный свет над горами.
        B_AI_Output(self,other,"DIA_Malak_MINENTAL_08_02");       //Но я думаю, что это все пустые сплетни.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_WARSTDUSCHONDA_15_00"); //А ты был в Долине Рудников?
        B_AI_Output(self,other,"DIA_Malak_WARSTDUSCHONDA_08_01"); //Нет. Но иногда я пасу овец рядом с Проходом. И я всегда вижу там двух паладинов, стоящих на страже.
        B_AI_Output(self,other,"DIA_Malak_WARSTDUSCHONDA_08_02"); //Похоже, они там, чтобы никого не выпускать из долины.
        B_AI_Output(self,other,"DIA_Malak_WARSTDUSCHONDA_08_03"); //Раньше Долина Рудников была естественной тюрьмой. Если ты попадал туда, ты уже никогда не мог выбраться назад.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_PALADINE_15_00");       //Расскажи мне об этих паладинах. Как давно они стоят здесь?
        B_AI_Output(self,other,"DIA_Malak_PALADINE_08_01");       //Неделю или две, по-моему. Я точно не помню.
        B_AI_Output(self,other,"DIA_Malak_PALADINE_08_02");       //Не так давно тут был даже целый отряд паладинов. Они пошли через Проход. И с тех пор я их больше не видел.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_KEINEFRAUEN_15_00");    //Я не вижу здесь женщин.
        B_AI_Output(self,other,"DIA_Malak_KEINEFRAUEN_08_01");    //Ты прав. Это мужская ферма, так сказать. И это даже к лучшему, мне кажется.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_PERMKAP1_15_00");       //Смотри, не перетрудись.
        B_AI_Output(self,other,"DIA_Malak_PERMKAP1_08_01");       //Я попробую.
        AI_StopProcessInfos(self);
};
...
func void dia_malak_fleefrompass_info()
{
        ...
        {
                B_AI_Output(other,self,"DIA_Malak_FLEEFROMPASS_15_00");   //Что ты делаешь здесь?
                B_AI_Output(self,other,"DIA_Malak_FLEEFROMPASS_08_01");   //Я сбежал с фермы Бенгара. Я не хочу, чтобы меня сожрали все эти твари, что толпами валят из Прохода.
                B_AI_Output(self,other,"DIA_Malak_FLEEFROMPASS_08_02");   //Ты представить себе не можешь, какие ужасы повылазили оттуда за последние несколько дней.
                B_AI_Output(other,self,"DIA_Malak_FLEEFROMPASS_15_03");   //Могу.
                b_logentry(TOPIC_BENGARALLEIN,TOPIC_BENGARALLEIN_description_82); // "Малак спрятался в укрепленном месте на юге потому что боится тварей приходящих на его пастбища из Прохода."
...
{
        B_AI_Output(other,self,"DIA_Malak_Heilung_15_00");        //Тебе нужна помощь.
        if(DIA_MALAK_HEILUNG_ONETIME == FALSE)
        {
                B_AI_Output(self,other,"DIA_Malak_Heilung_08_01");        //(плаксиво) Я просто хочу вернуться домой. Я вернусь к Бенгару. Надеюсь, он еще жив.
                b_npcclearobsessionbydmt(self);
        ...
        {
                B_AI_Output(self,other,"DIA_Malak_Heilung_08_02");        //Оставь меня в покое, маг. Я справлюсь.
                b_npcclearobsessionbydmt(self);
        };
};
...
{
        B_AI_Output(other,self,"DIA_Malak_PERMCASTLE_15_00");     //А как тебе здесь?
        B_AI_Output(self,other,"DIA_Malak_PERMCASTLE_08_01");     //Здесь мне приходится иметь дело только с бандитами. Это все же меньшее зло.
};
...
{
        B_AI_Output(other,self,"DIA_Malak_BACKTOBENGAR_15_00");   //Ты нужен Бенгару. Он хочет, чтобы ты вернулся на его ферму.
        B_AI_Output(self,other,"DIA_Malak_BACKTOBENGAR_08_01");   //Я не сумасшедший. Пока ферма беззащитна, я ни на шаг отсюда не сойду!
        b_logentry(TOPIC_BENGARALLEIN,TOPIC_BENGARALLEIN_description_83); // "Малак не вернется на ферму Бенгара пока она не будет хорошо защищена."
        if(MIS_BENGARSHELPINGSLD == LOG_SUCCESS)
        {
                B_AI_Output(other,self,"DIA_Malak_BACKTOBENGAR_15_02");   //Я нанял наемника. Он присмотрит за вашей фермой.
                B_AI_Output(self,other,"DIA_Malak_BACKTOBENGAR_08_03");   //Ну, это другое дело, конечно же. Но подожди минутку. А кто будет платить этому парню?
                B_AI_Output(other,self,"DIA_Malak_BACKTOBENGAR_15_04");   //Это моя проблема.
                B_AI_Output(self,other,"DIA_Malak_BACKTOBENGAR_08_05");   //(с любопытством) М-м-м. А может, скажешь, а?
                Info_AddChoice(dia_malak_backtobengar,dia_malak_backtobengar_los_description,dia_malak_backtobengar_los); // "Нет."
        };
};
func void dia_malak_backtobengar_los()
{
        B_AI_Output(other,self,"DIA_Malak_BACKTOBENGAR_los_15_00");       //Нет.
        B_AI_Output(self,other,"DIA_Malak_BACKTOBENGAR_los_08_01");       //Хорошо. Тогда я возвращаюсь. Надеюсь, этот парень знает свое дело.
...
{
        B_AI_Output(other,self,"DIA_Malak_BACK_15_00");   //Ты теперь останешься на ферме Бенгара?
        B_AI_Output(self,other,"DIA_Malak_BACK_08_01");   //Конечно. У меня появилась надежда. Мы как-нибудь справимся.
        b_npcclearobsessionbydmt(self);
};
