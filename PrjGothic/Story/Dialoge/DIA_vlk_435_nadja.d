...
{
        B_AI_Output(self,other,"DIA_Nadja_STANDARD_16_00");       //Эй, я не могу заняться тобой сейчас, детка. Если хочешь развлечься, поговори с Бромором.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(self,other,"DIA_Nadja_Danach_16_00"); //Заходи в другой раз.
        NADJA_NACHT = FALSE;
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Nadja_hochgehen_15_00");      //Пошли наверх.
        B_AI_Output(self,other,"DIA_Nadja_hochgehen_16_01");      //Сегодня тебе выпал счастливый билет, детка. Пойдем.
        BROMOR_PAY = 2;
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(self,other,"DIA_Nadja_Poppen_16_00"); //Следующая пара часов принадлежит только нам.
        B_AI_Output(self,other,"DIA_Nadja_Poppen_16_01"); //Расслабься. Ложись и получай удовольствие.
        Info_ClearChoices(dia_nadja_poppen);
        Info_AddChoice(dia_nadja_poppen,dia_nadja_poppen_start_description,dia_nadja_poppen_start); // "Хорошо ..."
};
...
func void dia_nadja_buyherb_info()
{
        ...
        B_AI_Output(other,self,"DIA_Nadja_BUYHERB_15_00");        //Могу я здесь купить травки?
        if(Npc_GetDistToWP(self,"NW_CITY_HABOUR_PUFF_NADJA") < 200)
        {
                if(Hlp_IsItem(heroarmor,itar_mil_l) == TRUE)
                {
                        B_AI_Output(self,other,"DIA_Nadja_BUYHERB_16_01");        //Откуда мне знать? Да и если бы знала, все равно не сказала бы городскому стражнику.
                        UNDERCOVER_FAILED = TRUE;
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Nadja_BUYHERB_16_02");        //Заплати несколько золотых, и я скажу, где ее достать.
                        B_AI_Output(other,self,"DIA_Nadja_BUYHERB_15_03");        //Сколько ты хочешь?
                        B_AI_Output(self,other,"DIA_Nadja_BUYHERB_16_04");        //50 золотых будет достаточно.
                        NADJA_MONEY = TRUE;
                };
        }
        else
...
func void dia_nadja_want_herb_info()
{
        var C_ITEM heroarmor;
        heroarmor = Npc_GetEquippedArmor(other);
        B_AI_Output(other,self,"DIA_Nadja_WANT_HERB_15_00");      //А теперь скажи мне, где можно купить травки.
        if(Hlp_IsItem(heroarmor,itar_mil_l) == TRUE)
        {
                B_AI_Output(self,other,"DIA_Nadja_WANT_HERB_16_01");      //Извини, я забыла.
        }
        else
        {
                b_giveinvitems(other,self,itmi_gold,50);
                B_AI_Output(self,other,"DIA_Nadja_WANT_HERB_16_02");      //Поговори с Боркой, детка. У него найдется травка для тебя.
