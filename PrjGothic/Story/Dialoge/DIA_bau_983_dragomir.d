...
{
        B_AI_Output(self,other,"DIA_Dragomir_Hello_12_00");       //Путешествуешь так далеко от дома?
        B_AI_Output(self,other,"DIA_Dragomir_Hello_12_01");       //Здесь тебе не помешает осторожность, особенно когда ты совсем один.
};
...
{
        B_AI_Output(other,self,"DIA_Dragomir_OutHere_15_00");     //Что ты делаешь здесь?
        B_AI_Output(self,other,"DIA_Dragomir_OutHere_12_01");     //Ну, ночлег в таверне слишком дорог для меня, вот почему я и живу здесь.
        B_AI_Output(self,other,"DIA_Dragomir_OutHere_12_02");     //Предыдущим хозяевам этого места, похоже, оно больше не понадобится.
};
...
{
        B_AI_Output(other,self,"DIA_Dragomir_Settlers_15_00");    //А кто жил в этом лагере?
        B_AI_Output(self,other,"DIA_Dragomir_Settlers_12_01");    //Понятия не имею. Наверное, какие-нибудь охотники. Я полагаю, они решили, что здесь стало слишком опасно.
};
...
{
        B_AI_Output(other,self,"DIA_Dragomir_Dangerous_15_00");   //А разве здесь не опасно?
        B_AI_Output(self,other,"DIA_Dragomir_Dangerous_12_01");   //Ну... Не очень, если ты можешь постоять за себя. Мой арбалет уже неоднократно спасал меня.
        B_AI_Output(other,self,"DIA_Dragomir_Dangerous_15_02");   //Хм. Он не такой уж большой.
        B_AI_Output(self,other,"DIA_Dragomir_Dangerous_12_03");   //Но смертоносный, если ты знаешь, как обращаться с ним. Да, у меня был арбалет побольше. Но, к сожалению, я потерял его.
        B_AI_Output(self,other,"DIA_Dragomir_Dangerous_12_04");   //Я забрел слишком далеко на север, в горы. Там находится большой каменный круг с жертвенным алтарем.
        B_AI_Output(self,other,"DIA_Dragomir_Dangerous_12_05");   //Пока я охотился там на падальщиков, из леса выбежали эти чертовы скелеты и напали на меня.
        B_AI_Output(self,other,"DIA_Dragomir_Dangerous_12_06");   //Мне пришлось бежать со всех ног.
        B_AI_Output(self,other,"DIA_Dragomir_Dangerous_12_07");   //А когда я бежал, арбалет выскользнул из моей руки. Я думаю, он все еще лежит там, у этого странного круга на севере.
        Log_CreateTopic(TOPIC_DRAGOMIRSARMBRUST,LOG_MISSION);
        Log_SetTopicStatus(TOPIC_DRAGOMIRSARMBRUST,LOG_RUNNING);
        b_logentry(TOPIC_DRAGOMIRSARMBRUST,TOPIC_DRAGOMIRSARMBRUST_description_96); // "Драгомир потерял свой арбалет в странном каменном круге находящемся высоко в северных горах."
...
{
        B_AI_Output(other,self,"DIA_Dragomir_Armbrust_15_00");    //Вот. Я нашел твой арбалет.
        b_giveinvitems(other,self,itrw_dragomirsarmbrust_mis,1);
        B_AI_Output(self,other,"DIA_Dragomir_Armbrust_12_01");    //Точно, это мой арбалет. Только не говори мне, что ты ходил к этому каменному кругу?
        B_AI_Output(other,self,"DIA_Dragomir_Armbrust_15_02");    //А что в этом такого?
        B_AI_Output(self,other,"DIA_Dragomir_Armbrust_12_03");    //Это было очень безрассудно с твоей стороны. Но все равно, спасибо.
        B_AI_Output(self,other,"DIA_Dragomir_Armbrust_12_04");    //Естественно, я готов заплатить за эту твою услугу. Надеюсь, этого будет достаточно.
...
{
        B_AI_Output(other,self,"DIA_Dragomir_Learn_15_00");       //Ты можешь научить меня чему-нибудь?
        if(other.attribute[ATR_DEXTERITY] < 30)
        {
                B_AI_Output(self,other,"DIA_Dragomir_Learn_12_01");       //Прежде, чем я смогу научить тебя чему-нибудь, ты должен повысить свою ловкость.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Dragomir_Learn_12_02");       //Если ты можешь заплатить. Как я уже говорил, я сейчас немного стеснен в средствах.
                B_AI_Output(other,self,"DIA_Dragomir_Learn_15_03");       //Сколько ты хочешь?
                b_say_gold(self,other,DRAGOMIR_TEACHINGCOST);
                Info_ClearChoices(dia_dragomir_learn);
                Info_AddChoice(dia_dragomir_learn,dia_dragomir_learn_later_description,dia_dragomir_learn_later); // "Может быть позже."
                if(Npc_HasItems(other,itmi_gold) >= DRAGOMIR_TEACHINGCOST)
                {
                        Info_AddChoice(dia_dragomir_learn,dia_dragomir_learn_here_description,dia_dragomir_learn_here); // "Вот твое золото."
                };
        };
};
func void dia_dragomir_learn_later()
{
        B_AI_Output(other,self,"DIA_Dragomir_Learn_Later_15_00"); //Возможно, позже.
        Info_ClearChoices(dia_dragomir_learn);
};
func void dia_dragomir_learn_here()
{
        B_AI_Output(other,self,"DIA_Dragomir_Learn_Here_15_00");  //Вот твое золото.
        b_giveinvitems(other,self,itmi_gold,DRAGOMIR_TEACHINGCOST);
        B_AI_Output(self,other,"DIA_Dragomir_Learn_Here_12_01");  //Хорошо, мы можем начать хоть сейчас.
...
{
        B_AI_Output(other,self,"DIA_Dragomir_Teach_15_00");       //Научи меня чему-нибудь.
...
func void dia_dragomir_teach_back()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Dragomir_Teach_12_00");       //Я больше ничему не могу научить тебя. Тебе стоит поискать другого учителя.
