...
{
        B_AI_Output(self,other,"DIA_NAME_BANDIT_SCHAFBLEIBTHIER_09_00");  //Эй ты, кретин! Ты пришел сюда случайно не за овцой, а?
        if(MIS_AKIL_SCHAFDIEBE == 0)
        {
                B_AI_Output(other,self,"DIA_NAME_BANDIT_SCHAFBLEIBTHIER_15_01");  //Овцой? Какой овцой?
                B_AI_Output(self,other,"DIA_NAME_BANDIT_SCHAFBLEIBTHIER_09_02");  //Это хорошо. А теперь проваливай. Здесь тебе нечего делать.
        };
};
...
{
        B_AI_Output(self,other,"DIA_NAME_BANDIT_SchafKlau_09_00");        //Эй! Я с тобой говорю.
...
{
        B_AI_Output(other,self,"DIA_NAME_BANDIT_RUECKDASSCHAFRAUS_15_00");        //Вы украли овцу у фермера Акила.
        B_AI_Output(self,other,"DIA_NAME_BANDIT_RUECKDASSCHAFRAUS_09_01");        //И что? Что ты будешь делать?
        Info_ClearChoices(dia_name_bandit_rueckdasschafraus);
        Info_AddChoice(dia_name_bandit_rueckdasschafraus,dia_name_bandit_rueckdasschafraus_nichts_description,dia_name_bandit_rueckdasschafraus_nichts); // "Ничего. И зачем мне эта тупая овца?"
        Info_AddChoice(dia_name_bandit_rueckdasschafraus,dia_name_bandit_rueckdasschafraus_schaf_description,dia_name_bandit_rueckdasschafraus_schaf); // "Отдай мне эту овцу или в глаз получишь."
};
func void dia_name_bandit_rueckdasschafraus_schaf()
{
        B_AI_Output(other,self,"DIA_NAME_BANDIT_RUECKDASSCHAFRAUS_schaf_15_00");  //Отдай мне эту овцу, или в глаз получишь.
        B_AI_Output(self,other,"DIA_NAME_BANDIT_RUECKDASSCHAFRAUS_schaf_09_01");  //Тогда берегись, приятель. Сейчас тебе придется несладко.
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(other,self,"DIA_NAME_BANDIT_RUECKDASSCHAFRAUS_nichts_15_00"); //Ничего. И зачем мне эта тупая овца?
        B_AI_Output(self,other,"DIA_NAME_BANDIT_RUECKDASSCHAFRAUS_nichts_09_01"); //Тогда не играй меня на нервах. Проваливай!
        hero.aivar[AIV_LASTDISTTOWP] = Npc_GetDistToWP(hero,"NW_FOREST_CAVE1_IN_01");
        AI_StopProcessInfos(self);
};
