...
{
        B_AI_Output(self,other,"DIA_Zuris_Sperre_14_00"); //Ты заключенный, сбежавший из горнодобывающей колонии. Проваливай!
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(self,other,"DIA_Zuris_GREET_14_00");  //Я Зурис, специалист по зельям.
        B_AI_Output(self,other,"DIA_Zuris_GREET_14_01");  //Тебе нужна лечебная эссенция или магический эликсир? Тогда ты обратился по адресу.
        Log_CreateTopic(TOPIC_CITYTRADER,LOG_NOTE);
        b_logentry(TOPIC_CITYTRADER,TOPIC_CITYTRADER_description_555); // "Зурис торгует зельями на рыночной площади."
};
...
{
        b_givetradeinv(self);
        B_AI_Output(other,self,"DIA_Zuris_WAREZ_15_00");  //Покажи мне свои товары.
        if((ZURIS_EINMAL == FALSE) && !Npc_KnowsInfo(other,dia_zuris_potions))
        {
                B_AI_Output(self,other,"DIA_Zuris_GREET_14_02");  //Я только что получил несколько новых зелий. Мой гость, Мастер Дарон, Маг Огня, принес мне их из монастыря.
                ZURIS_EINMAL = TRUE;
        };
};
...
{
        B_AI_Output(other,self,"DIA_Zuris_POTIONS_15_00");        //Ты сам готовишь свои зелья?
        B_AI_Output(self,other,"DIA_Zuris_POTIONS_14_01");        //Нет, я получаю их из монастыря или покупаю у Константино, алхимика.
        if(hero.guild == GIL_NONE)
        {
                B_AI_Output(self,other,"DIA_Zuris_POTIONS_14_02");        //Если тебя интересуют рецепты приготовления зелий, тебе нужно поговорить с ним. Ему как раз нужен помощник.
                B_AI_Output(self,other,"DIA_Zuris_POTIONS_14_03");        //Но он слишком туп, чтобы нанять кого-нибудь самому. И к тому же, упрям как мул.
        };
        B_AI_Output(self,other,"DIA_Zuris_Add_14_00");    //Он всегда проходит мимо меня, когда выходит через восточные ворота собирать растения для своих зелий.
};
...
{
        B_AI_Output(other,self,"DIA_Zuris_Add_15_01");    //А где именно Константино собирает травы?
        B_AI_Output(self,other,"DIA_Zuris_Add_14_02");    //Он всегда выходит через восточные ворота города, а затем исчезает в лесу налево.
        B_AI_Output(self,other,"DIA_Zuris_Add_14_03");    //Он как-то сказал мне, что почти все, что ему нужно, растет там, за исключением царского щавеля.
...
{
        B_AI_Output(other,self,"DIA_Zuris_Add_15_04");    //Константино говорил тебе, где можно найти царский щавель?
        B_AI_Output(self,other,"DIA_Zuris_Add_14_05");    //Он как-то упоминал, что он растет около каменных кругов.
        B_AI_Output(self,other,"DIA_Zuris_Add_14_06");    //Около фермы Лобарта, эта ферма находится неподалеку от других ворот, есть один из таких каменных кругов.
        B_AI_Output(self,other,"DIA_Zuris_Add_14_07");    //Насколько я помню, он стоит на холме. Там когда-то была могила, или что-то вроде этого.
...
{
        B_AI_Output(other,self,"DIA_Zuris_WHERE_15_00");  //Где я могу найти Константино?
        B_AI_Output(self,other,"DIA_Zuris_WHERE_14_01");  //Его дом находится в туннеле около кузницы Гарада. Его будет нетрудно найти.
};
...
{
        B_AI_Output(other,self,"DIA_Zuris_Kloster_14_00");        //Расскажи мне о монастыре.
        B_AI_Output(self,other,"DIA_Zuris_Kloster_14_01");        //Тебе лучше поговорить об этом с самим Мастером Дароном. Он знает больше меня.
};
...
{
        B_AI_Output(other,self,"DIA_Zuris_Minenanteil_15_00");    //Ты продаешь шахтерские акции?
        B_AI_Output(self,other,"DIA_Zuris_Minenanteil_14_01");    //Да. Это оказалось не самое выгодное дело. Это последний раз, когда я ввязываюсь в подобное сомнительное предприятие!
