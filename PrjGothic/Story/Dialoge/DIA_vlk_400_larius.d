...
{
        B_AI_Output(self,other,"DIA_Larius_Hello_01_00"); //Что ты делаешь здесь? Если тебе что-то нужно, запишись на прием.
};
...
{
        B_AI_Output(other,self,"DIA_Larius_WhoAreYou_15_00");     //Кто ты?
        B_AI_Output(self,other,"DIA_Larius_WhoAreYou_01_01");     //Я Лариус, губернатор.
        B_AI_Output(self,other,"DIA_Larius_WhoAreYou_01_02");     //Даже если я не кажусь тебе таковым в данный момент, я самый влиятельный человек в этом городе.
        B_AI_Output(other,self,"DIA_Larius_WhoAreYou_15_03");     //То есть ты всем здесь заправляешь?
        B_AI_Output(self,other,"DIA_Larius_WhoAreYou_01_04");     //Я... ну... в настоящий момент мои руки связаны.
        B_AI_Output(self,other,"DIA_Larius_WhoAreYou_01_05");     //Здесь командует этот лорд Хаген, по крайней мере, пока он в городе.
};
...
{
        B_AI_Output(other,self,"DIA_Larius_Disturb_15_00");       //Я не хотел помешать.
        B_AI_Output(self,other,"DIA_Larius_Disturb_01_01");       //Но, тем не менее, помешал! Убирайся!
};
...
{
        B_AI_Output(other,self,"DIA_Larius_DieLage_15_00");       //Как дела?
        B_AI_Output(self,other,"DIA_Larius_DieLage_01_01");       //Тебе до этого какое дело? Пока паладины здесь, лорд Хаген занимается всеми делами, касающимися жизни города.
};
...
{
        B_AI_Output(other,self,"DIA_Larius_Richterueberfall_15_00");      //Судья нанял бандитов, чтобы они убили тебя. Я могу доказать это.
        B_AI_Output(self,other,"DIA_Larius_Richterueberfall_01_01");      //Не пори чепухи, или ты хочешь, чтобы я заковал тебя в кандалы?
        if((hero.guild == GIL_MIL) || (hero.guild == GIL_PAL))
        {
                B_AI_Output(self,other,"DIA_Larius_Richterueberfall_01_02");      //Даже сели ты воин Инноса...
        };
        if(hero.guild == GIL_KDF)
        {
                B_AI_Output(self,other,"DIA_Larius_Richterueberfall_01_03");      //Даже если ты посвященный маг...
        };
        B_AI_Output(self,other,"DIA_Larius_Richterueberfall_01_04");      //Мое слово все еще имеет вес в этом городе. Не смей даже пытаться запятнать светлое имя нашего судьи!
