...
{
        B_AI_Output(self,other,"DIA_PAL_205_Torwache_FirstWarn_12_00");   //СТОЙ!
        if(b_getgreatestpetzcrime(self) >= CRIME_ATTACK)
        {
                if(b_getgreatestpetzcrime(self) == CRIME_MURDER)
                {
                        B_AI_Output(self,other,"DIA_PAL_205_Torwache_FirstWarn_12_01");   //Я нарушу свой самый священный долг, если пущу сюда убийцу.
                };
                if(b_getgreatestpetzcrime(self) == CRIME_THEFT)
                {
                        B_AI_Output(self,other,"DIA_PAL_205_Torwache_FirstWarn_12_02");   //Ты обвиняешься в воровстве. Пока не доказано обратное, ты не можешь войти сюда!
                };
                if(b_getgreatestpetzcrime(self) == CRIME_ATTACK)
                {
                        B_AI_Output(self,other,"DIA_PAL_205_Torwache_FirstWarn_12_03");   //Говорят, ты принимал участие в драке. Я не пропущу тебя.
                };
                B_AI_Output(self,other,"DIA_PAL_205_Torwache_FirstWarn_12_04");   //Иди к лорду Андрэ и урегулируй этот вопрос!
        }
        else
        {
                B_AI_Output(self,other,"DIA_PAL_205_Torwache_FirstWarn_12_05");   //Только люди, состоящие на службе у короля, могут войти в ратушу.
        };
...
{
        B_AI_Output(self,other,"DIA_PAL_205_Torwache_SecondWarn_12_00");  //Если ты сделаешь еще шаг вперед, клянусь Инносом, я применю оружие!
        other.aivar[AIV_LASTDISTTOWP] = Npc_GetDistToWP(other,PAL_205_CHECKPOINT);
...
{
        B_AI_Output(other,self,"DIA_PAL_205_Torwache_Hagen_15_00");       //Мне нужно поговорить с лордом Хагеном!
        B_AI_Output(self,other,"DIA_PAL_205_Torwache_Hagen_12_01");       //Да ты знаешь, как часто я слышу это? Я не пущу тебя. А если ты все же попытаешься войти, мне придется убить тебя.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_PAL_205_Torwache_PassAsMil_15_00");   //Я состою в ополчении.
        B_AI_Output(self,other,"DIA_PAL_205_Torwache_PassAsMil_12_01");   //Хорошо, ты можешь войти.
        self.aivar[AIV_PASSGATE] = TRUE;
...
{
        B_AI_Output(other,self,"DIA_PAL_205_Torwache_PassAsMage_15_00");  //Я маг Огня.
        B_AI_Output(self,other,"DIA_PAL_205_Torwache_PassAsMage_12_01");  //Да, конечно. Прошу простить меня, о, Избранный, я просто выполнял свой долг.
        self.aivar[AIV_PASSGATE] = TRUE;
        PAL_205_SCHONMALREINGELASSEN = TRUE;
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_PAL_205_Torwache_PassAsSld_15_00");   //Дай мне пойти, я несу послание от наемников.
        B_AI_Output(self,other,"DIA_PAL_205_Torwache_PassAsSld_12_01");   //Хорошо, но предупреждаю тебя. Если возникнут какие-нибудь проблемы, то ты не успеешь даже пожалеть об этом.
        self.aivar[AIV_PASSGATE] = TRUE;
        PAL_205_SCHONMALREINGELASSEN = TRUE;
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_PAL_205_Torwache_PERM_15_00");        //Как идет служба?
        if((other.guild == GIL_PAL) || (other.guild == GIL_MIL))
        {
                B_AI_Output(self,other,"DIA_PAL_205_Torwache_PERM_12_01");        //Мы все держим под контролем.
        }
        else if(other.guild == GIL_KDF)
        {
                B_AI_Output(self,other,"DIA_PAL_205_Torwache_PERM_12_02");        //Я полностью доволен службой, о, Избранный.
        }
        else if((other.guild == GIL_SLD) || (other.guild == GIL_DJG))
        {
                B_AI_Output(self,other,"DIA_PAL_205_Torwache_PERM_12_03");        //Проходи и заткнись.
        }
        else
        {
                B_AI_Output(self,other,"DIA_PAL_205_Torwache_PERM_12_04");        //Что тебе нужно?
        };
        AI_StopProcessInfos(self);
};
