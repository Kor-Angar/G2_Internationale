...
{
        B_AI_Output(other,self,"DIA_Egill_HALLO_15_00");  //В чем дело?
        B_AI_Output(self,other,"DIA_Egill_HALLO_08_01");  //Давно уже я не видел чужака на нашей земле, который не хотел бы ободрать нас, как липку.
        B_AI_Output(self,other,"DIA_Egill_HALLO_08_02");  //Надеюсь, я не ошибаюсь в тебе.
        B_AI_Output(self,other,"DIA_Egill_HALLO_08_03");  //Я Эгилл.
        if(Hlp_IsValidNpc(ehnim) && !c_npcisdown(ehnim))
        {
                B_AI_Output(self,other,"DIA_Egill_HALLO_08_04");  //А этот чудак вон там - мой брат Эним.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Egill_FELDARBEIT_15_00");     //Как идет работа в поле?
        B_AI_Output(self,other,"DIA_Egill_FELDARBEIT_08_01");     //Отлично. Если бы только эти мерзкие полевые хищники не портили наш урожай...
};
...
{
        B_AI_Output(other,self,"DIA_Egill_FELDRAEUBER_15_00");    //Почему вы ничего не делаете с этими полевыми хищниками?
        B_AI_Output(self,other,"DIA_Egill_FELDRAEUBER_08_01");    //Ты шутишь? С тех пор, как я работаю на этой ферме, я убил их столько, что уже сбился со счета.
        B_AI_Output(self,other,"DIA_Egill_FELDRAEUBER_08_02");    //Но на место убитых приходят новые.
};
...
{
        B_AI_Output(other,self,"DIA_Egill_STREIT2_15_00");        //Твой брат говорит то же самое.
        B_AI_Output(self,other,"DIA_Egill_STREIT2_08_01");        //(насмешливо) Ага. А что еще он говорит?
        B_AI_Output(other,self,"DIA_Egill_STREIT2_15_02");        //Что он тоже убил множество полевых хищников.
        B_AI_Output(self,other,"DIA_Egill_STREIT2_08_03");        //Что? Да этот тормоз даже не может самостоятельно найти путь домой.
        B_AI_Output(self,other,"DIA_Egill_STREIT2_08_04");        //Скажи ему, чтобы он больше не бахвалился.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Egill_STREIT4_15_00");        //Эним немного раздражен твоими словами.
        B_AI_Output(self,other,"DIA_Egill_STREIT4_08_01");        //Пусть не расхаживает как павлин, распустив хвост, а то я ему голову оторву. Скажи ему это.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(self,other,"DIA_Egill_PERMKAP1_08_00");       //Тебе мало того, что ты натворил? Я думаю, тебе лучше уйти.
        AI_StopProcessInfos(self);
};
