...
{
        B_AI_Output(self,other,"DIA_Salandril_PERM_13_00");       //Добро пожаловать, путник. Ищешь хорошее зелье?
        B_AI_Output(self,other,"DIA_Salandril_PERM_13_01");       //У меня большой выбор и умеренные цены. Мои зелья лучше, чем та отрава, что продает Зурис.
        Log_CreateTopic(TOPIC_CITYTRADER,LOG_NOTE);
        b_logentry(TOPIC_CITYTRADER,TOPIC_CITYTRADER_description_639); // "Саландрил торгует зельями. Его лавка находится в верхнем квартале."
};
...
{
        B_AI_Output(self,other,"DIA_Salandril_Trank_13_00");      //Я слышал, ты был с паладинами в Долине Рудников. Я восхищен.
        B_AI_Output(self,other,"DIA_Salandril_Trank_13_01");      //Тебе стоит задержаться и взглянуть на мои товары. Вот, например, сейчас у меня есть совершенно особенное зелье. Только для тебя.
...
{
        b_givetradeinv(self);
        B_AI_Output(other,self,"DIA_Salandril_Trade_15_00");      //Покажи мне свои товары.
        if(other.guild == GIL_KDF)
        {
                B_AI_Output(self,other,"DIA_Salandril_Trade_13_01");      //С удовольствием, преподобный брат.
        ...
        {
                B_AI_Output(self,other,"DIA_Salandril_Trade_13_02");      //С удовольствием, о, благородный воин.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Salandril_KLOSTER_15_00");    //Ты должен отправиться в монастырь, чтобы предстать перед судом.
        B_AI_Output(self,other,"DIA_Salandril_KLOSTER_13_01");    //Что? У тебя крыша поехала? Черта с два! У этих жалких магов нет никаких доказательств против меня.
        if((hero.guild == GIL_KDF) && (SC_KNOWSPROSPEKTORSALANDRIL == TRUE))
        {
                B_AI_Output(other,self,"DIA_Salandril_KLOSTER_15_02");    //А как насчет этих фальшивых акций, которыми ты наводнил всю страну? На них твоя подпись. Ты виновен.
        }
        else
        {
                B_AI_Output(other,self,"DIA_Salandril_KLOSTER_15_03");    //У меня есть приказ, и я выполню его. Так что, либо ты пойдешь сам, либо мне придется заставить тебя.
        };
        B_AI_Output(self,other,"DIA_Salandril_KLOSTER_13_04");    //Что? Да я протащу тебя через весь город за шиворот, как паршивого щенка, и вышвырну за ворота.
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(other,self,"DIA_Salandril_GehinsKloster_15_00");      //Так ты пойдешь в монастырь, или тебя еще раз проучить?..
        B_AI_Output(self,other,"DIA_Salandril_GehinsKloster_13_01");      //Ты еще пожалеешь об этом. Да, черт тебя побери, я пойду в этот монастырь, но тебе это просто так с рук не сойдет.
        AI_StopProcessInfos(self);
