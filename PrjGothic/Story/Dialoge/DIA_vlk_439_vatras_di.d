...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_HEAL_15_00");       //Вылечи меня.
        if(hero.attribute[ATR_HITPOINTS] < hero.attribute[ATR_HITPOINTS_MAX])
        {
                B_AI_Output(self,other,"DIA_Vatras_DI_HEAL_05_01");       //(благоговейно) Аданос, благослови это тело. Ему предназначено восстановить баланс мира.
                hero.attribute[ATR_HITPOINTS] = hero.attribute[ATR_HITPOINTS_MAX];
                PrintScreen(PRINT_FULLYHEALED,-1,-1,FONT_SCREEN,2);
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vatras_DI_HEAL_05_02");       //Твое тело сейчас не нуждается в лечении.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_TRADE_15_00");      //Ты можешь продать мне что-нибудь?
        b_givetradeinv(self);
        B_AI_Output(self,other,"DIA_Vatras_DI_TRADE_05_01");      //В чем ты нуждаешься?
};
...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_OBSESSION_15_00");  //Помоги мне. Я одержим!
        if(GOT_HEALOBSESSION_DAY <= (Wld_GetDay() - 2))
        {
                ...
                {
                        B_AI_Output(self,other,"DIA_Vatras_DI_OBSESSION_05_01");  //Возьми это Зелье Освобождения. Пирокар дал мне несколько этих лечебных эликсиров по моей просьбе.
                        B_AI_Output(self,other,"DIA_Vatras_DI_OBSESSION_05_02");  //Однако помни: мои возможности по избавлению тебя от ночных кошмаров ограничены.
                        b_giveinvitems(self,other,itpo_healobsession_mis,1);
                        GOT_HEALOBSESSION_DAY = Wld_GetDay();
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Vatras_DI_OBSESSION_05_03");  //Запасы Пирокара истощены. Мне очень жаль, друг мой. Я больше ничем не могу помочь тебе.
                };
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vatras_DI_OBSESSION_05_04");  //Я не возьму на себя риск дать тебе еще одну бутылку в столь короткое время. Возвращайся позже, друг мой.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_RAT_15_00");        //Какой совет ты можешь дать мне?
        B_AI_Output(self,other,"DIA_Vatras_DI_RAT_05_01");        //Держись подальше от Ищущих. Помни, что их черный взгляд может нанести тебе серьезный урон здесь, вдали от монастыря.
        if(Npc_HasItems(other,itam_prot_blackeye_mis))
        {
                B_AI_Output(other,self,"DIA_Vatras_DI_RAT_15_02");        //Не волнуйся, у меня есть амулет вызова душ.
        };
        B_AI_Output(self,other,"DIA_Vatras_DI_RAT_05_03");        //Если все же что-то произойдет с тобой, приходи ко мне. Я посмотрю, что можно сделать.
};
...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_PEDROTOT_15_00");   //Я нашел предателя Педро.
        b_giveplayerxp(XP_AMBIENT);
        if((MIS_GORAX_KILLPEDRO == LOG_SUCCESS) && Npc_IsDead(pedro_di))
        {
                B_AI_Output(other,self,"DIA_Vatras_DI_PEDROTOT_15_01");   //Он мертв.
                B_AI_Output(self,other,"DIA_Vatras_DI_PEDROTOT_05_02");   //Я крайне разочарован. Я не ожидал этого от тебя.
                B_AI_Output(other,self,"DIA_Vatras_DI_PEDROTOT_15_03");   //Что ты имеешь в виду?
                B_AI_Output(self,other,"DIA_Vatras_DI_PEDROTOT_05_04");   //Я знаю о твоем отвратительном соглашении с Серпентесом. Я глубоко потрясен тем, что так ошибался в тебе.
                DIA_VATRAS_DI_PEDROTOT_VATRASSUCKED = TRUE;
                AI_StopProcessInfos(self);
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vatras_DI_PEDROTOT_05_05");   //Тогда приведи его сюда, на корабль. Мы передадим его властям Хориниса.
                if(Npc_IsDead(pedro_di))
                {
                        B_AI_Output(other,self,"DIA_Vatras_DI_PEDROTOT_15_06");   //Для этого немного поздно. Он мертв.
                        B_AI_Output(self,other,"DIA_Vatras_DI_PEDROTOT_05_07");   //(удивленно) Ох. Какая жалость. Я буду молиться за его бедную душу.
                }
                else
                {
                        AI_StopProcessInfos(self);
...
{
        B_AI_Output(self,other,"DIA_Vatras_DI_VatrasSucked_05_00");       //Убирайся с глаз моих, убийца. Ты больше можешь не рассчитывать на мою помощь.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_Talente_15_00");    //Научи меня магии.
        B_AI_Output(self,other,"DIA_Vatras_DI_Talente_05_01");    //Я сделаю все, что в моих силах.
        Info_ClearChoices(dia_vatras_di_talente);
        Info_AddChoice(dia_vatras_di_talente,DIALOG_BACK,dia_vatras_di_talente_back);
        if(hero.guild == GIL_KDF)
        {
                Info_AddChoice(dia_vatras_di_talente,dia_vatras_di_talente_circles_description,dia_vatras_di_talente_circles); // "Круги магии"
        };
        Info_AddChoice(dia_vatras_di_talente,dia_vatras_di_talente_alchimie_description,dia_vatras_di_talente_alchimie); // "Алхимия - искусство приготовления зелий"
};
...
{
        B_AI_Output(self,other,"DIA_Vatras_DI_Talente_Circle_5_05_00");   //Теперь ты маг пятого Круга. Используй заклинания, изученные тобой, во благо.
        Info_ClearChoices(dia_vatras_di_talente);
        b_teachmagiccircle(self,other,5);
};
func void dia_vatras_di_talente_circle_6()
{
        B_AI_Output(self,other,"DIA_Vatras_DI_Talente_Circle_6_05_00");   //Ты достиг самых высоких вершин в магии.
        B_AI_Output(self,other,"DIA_Vatras_DI_Talente_Circle_6_05_01");   //Пусть руку твою направляет разум, а твои человеческие слабости будут под глубоким контролем. Они не смогут затмить твой взор.
        Info_ClearChoices(dia_vatras_di_talente);
...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_DementorObsessionBook_15_00");      //Я принес Альманах Одержимых
        if(DIA_VATRAS_DI_DEMENTOROBSESSIONBOOK_ONETIME == FALSE)
        {
                B_AI_Output(self,other,"DIA_Vatras_DI_DementorObsessionBook_05_01");      //Хм. Я думаю, будет лучше всего, если я отнесу его в монастырь к Пирокару. Если, конечно, нам удастся выбраться отсюда.
                DIA_VATRAS_DI_DEMENTOROBSESSIONBOOK_ONETIME = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Vatras_DI_DementorObsessionBook_05_02");      //У тебя есть еще? Принеси мне все, что найдешь.
        };
...
{
        B_AI_Output(other,self,"DIA_Vatras_DI_UndeadDragonDead_15_00");   //Я сделал это.
        if(DIA_VATRAS_DI_UNDEADDRAGONDEAD_ONETIME == FALSE)
        {
                B_AI_Output(self,other,"DIA_Vatras_DI_UndeadDragonDead_05_01");   //Я знаю, я чувствую это.
                B_AI_Output(self,other,"DIA_Vatras_DI_UndeadDragonDead_05_02");   //Ты нанес удар Белиару, от которого он не скоро оправится.
                if(hero.guild == GIL_DJG)
                {
                        B_AI_Output(other,self,"DIA_Vatras_DI_UndeadDragonDead_15_03");   //Могу я теперь успокоиться, или у вас, у магов, есть еще один скелет в шкафу, которого нужно изгнать из этого мира?
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Vatras_DI_UndeadDragonDead_05_04");   //Помни, что это был всего лишь эпизод в вечной битве Добра со Злом.
                };
                B_AI_Output(self,other,"DIA_Vatras_DI_UndeadDragonDead_05_05");   //Зло всегда находит способ проникнуть в этот мир. Эта война никогда не кончится.
                if(hero.guild == GIL_PAL)
                {
                        B_AI_Output(self,other,"DIA_Vatras_DI_UndeadDragonDead_05_06");   //Как воин Добра ты должен понимать это.
                };
                B_AI_Output(self,other,"DIA_Vatras_Add_05_15");   //Только один Аданос стоит между воюющими богами и хранит хрупкий баланс!
                B_AI_Output(other,self,"DIA_Vatras_Add_15_16");   //Лучше бы он помог мне.
                B_AI_Output(self,other,"DIA_Vatras_Add_05_17");   //(озорно) Он так и сделал - будь уверен.
                DIA_VATRAS_DI_UNDEADDRAGONDEAD_ONETIME = TRUE;
        };
        B_AI_Output(self,other,"DIA_Vatras_DI_UndeadDragonDead_05_09");   //Скажи капитану, чтобы он поднимал якорь как можно быстрее. Кратковременный мир может быть обманом.
};
