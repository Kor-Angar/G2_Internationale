...
{
        B_AI_Output(other,self,"DIA_Angar_NW_AllDragonsDead_15_00");      //Как дела?
        B_AI_Output(self,other,"DIA_Angar_NW_AllDragonsDead_04_01");      //Хорошо. Но меня все равно мучают головные боли, хотя они уменьшились с тех пор, как я покинул Долину Рудников.
        if(ANGAR_ISONBOARD != LOG_SUCCESS)
        {
                B_AI_Output(self,other,"DIA_Angar_NW_AllDragonsDead_04_02");      //Я останусь здесь. Ты будешь знать, где найти меня, если я тебе понадоблюсь.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Angar_NW_KnowWhereEnemy_15_00");      //Я собираюсь покинуть Хоринис. Ты хочешь присоединиться ко мне?
        B_AI_Output(self,other,"DIA_Angar_NW_KnowWhereEnemy_04_01");      //Чем дальше я буду от Долины Рудников, тем лучше. Когда мы отчаливаем?
        Log_CreateTopic(TOPIC_CREW,LOG_MISSION);
        Log_SetTopicStatus(TOPIC_CREW,LOG_RUNNING);
        b_logentry(TOPIC_CREW,TOPIC_CREW_description_133); // "Ангар будет рад каждой миле что разделяет его и Долину рудников. Он предложил присоединиться ко мне и путешествовать вместе."
        if(CREWMEMBER_COUNT >= MAX_CREW)
        {
                B_AI_Output(other,self,"DIA_Angar_NW_KnowWhereEnemy_15_02");      //У меня сейчас достаточно людей, чтобы управлять кораблем. Возможно, я обращусь к тебе позже.
                B_AI_Output(self,other,"DIA_Angar_NW_KnowWhereEnemy_04_03");      //Хорошо. Ты знаешь, где найти меня.
        }
        else
        {
                Info_ClearChoices(dia_angar_nw_knowwhereenemy);
                Info_AddChoice(dia_angar_nw_knowwhereenemy,dia_angar_nw_knowwhereenemy_no_description,dia_angar_nw_knowwhereenemy_no); // "Я дам тебе знать когда придет время."
                Info_AddChoice(dia_angar_nw_knowwhereenemy,dia_angar_nw_knowwhereenemy_yes_description,dia_angar_nw_knowwhereenemy_yes); // "Иди в гавань. Встретимся там."
        };
};
func void dia_angar_nw_knowwhereenemy_yes()
{
        B_AI_Output(other,self,"DIA_Angar_NW_KnowWhereEnemy_Yes_15_00");  //Иди в гавань. Встретимся там.
        B_AI_Output(self,other,"DIA_Angar_NW_KnowWhereEnemy_Yes_04_01");  //Уже иду.
...
{
        B_AI_Output(other,self,"DIA_Angar_NW_KnowWhereEnemy_No_15_00");   //Я дам тебе знать, когда придет время.
        B_AI_Output(self,other,"DIA_Angar_NW_KnowWhereEnemy_No_04_01");   //Хорошо.
...
{
        B_AI_Output(other,self,"DIA_Angar_NW_LeaveMyShip_15_00"); //Оставайся здесь и лечись от своей головной боли.
        B_AI_Output(self,other,"DIA_Angar_NW_LeaveMyShip_04_01"); //Хорошо. Я пойду назад. Может, так даже будет лучше.
        ANGAR_ISONBOARD = LOG_OBSOLETE;
        CREWMEMBER_COUNT = CREWMEMBER_COUNT - 1;
...
{
        B_AI_Output(other,self,"DIA_Angar_NW_StillNeedYou_15_00");        //Возвращайся на борт.
        B_AI_Output(self,other,"DIA_Angar_NW_StillNeedYou_04_01");        //Ты даже хуже чем я. Немного определенности тебе совсем бы не помешало. Увидимся позже.
        self.flags = NPC_FLAG_IMMORTAL;
        ANGAR_ISONBOARD = LOG_SUCCESS;
        CREWMEMBER_COUNT = CREWMEMBER_COUNT + 1;
        AI_StopProcessInfos(self);
