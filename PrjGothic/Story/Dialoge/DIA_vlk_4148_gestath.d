...
func void dia_gestath_exit_info()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_EXIT_09_00"); //(сухо) Хорошие доспехи!
        };
...
{
        B_AI_Output(other,self,"DIA_Gestath_HALLO_15_00");        //Как дела?
        B_AI_Output(self,other,"DIA_Gestath_HALLO_09_01");        //(сухо) Это мужественный поступок - прийти сюда. А ты не заблудился, часом?
        GESTATH_TEACHANIMALTROPHY = TRUE;
        Info_ClearChoices(dia_gestath_hallo);
        Info_AddChoice(dia_gestath_hallo,dia_gestath_hallo_waszusehen_description,dia_gestath_hallo_waszusehen); // "Что здесь есть интересного?"
        Info_AddChoice(dia_gestath_hallo,dia_gestath_hallo_was_description,dia_gestath_hallo_was); // "А что ты делаешь здесь?"
};
func void dia_gestath_hallo_plate()
{
        B_AI_Output(other,self,"DIA_Gestath_HALLO_plate_15_00");  //У тебя хорошие доспехи.
        B_AI_Output(self,other,"DIA_Gestath_HALLO_plate_09_01");  //Да. Такие нелегко найти. Они из панцирей краулеров. Парень по имени Вольф сделал их для меня.
        B_AI_Output(self,other,"DIA_Gestath_HALLO_plate_09_02");  //Я был каторжником здесь, в колонии. Дал ему пару панцирей краулеров, и пару дней спустя доспехи были готовы. Этот парень - мастер.
        WOLF_PRODUCECRAWLERARMOR = TRUE;
        Info_AddChoice(dia_gestath_hallo,DIALOG_BACK,dia_gestath_hallo_back);
        Info_AddChoice(dia_gestath_hallo,dia_gestath_hallo_plate_wowolf_description,dia_gestath_hallo_plate_wowolf); // "А где сейчас Вольф?"
};
func void dia_gestath_hallo_plate_wowolf()
{
        B_AI_Output(other,self,"DIA_Gestath_HALLO_plate_woWolf_15_00");   //А где сейчас Вольф?
        B_AI_Output(self,other,"DIA_Gestath_HALLO_plate_woWolf_09_01");   //Я давно уже не видел его. Раньше он был наемником здесь, в колонии.
        B_AI_Output(self,other,"DIA_Gestath_HALLO_plate_woWolf_09_02");   //Я думаю, он все еще со своими парнями.
};
func void dia_gestath_hallo_was()
{
        B_AI_Output(other,self,"DIA_Gestath_HALLO_was_15_00");    //А что ты делаешь здесь?
        B_AI_Output(self,other,"DIA_Gestath_HALLO_was_09_01");    //Зарабатываю деньги.
        B_AI_Output(other,self,"DIA_Gestath_HALLO_was_15_02");    //Здесь, где нет ничего?
        B_AI_Output(self,other,"DIA_Gestath_HALLO_was_09_03");    //Я охотник. Специализируюсь на сложных случаях.
        B_AI_Output(self,other,"DIA_Gestath_HALLO_was_09_04");    //Огненные ящеры, краулеры, драконьи снепперы... Другие не охотятся на таких зверей. Это приносит неплохие деньги.
        Info_AddChoice(dia_gestath_hallo,dia_gestath_hallo_plate_description,dia_gestath_hallo_plate); // "У тебя хорошие доспехи."
};
func void dia_gestath_hallo_waszusehen()
{
        B_AI_Output(other,self,"DIA_Gestath_HALLO_waszusehen_15_00");     //Что здесь есть интересного?
        B_AI_Output(self,other,"DIA_Gestath_HALLO_waszusehen_09_01");     //Куча огненных ящеров, орков и еще не знаю чего. Наверх я не ходил.
        B_AI_Output(self,other,"DIA_Gestath_HALLO_waszusehen_09_02");     //И тебе бы не советовал. Твари, что живут там, не особенно дружелюбны.
};
...
{
        B_AI_Output(other,self,"DIA_Gestath_Drachen_15_00");      //Ты умеешь потрошить драконов?
        if(KAPITEL < 4)
        {
                B_AI_Output(self,other,"DIA_Gestath_Drachen_09_01");      //(смеется) Спроси меня об этом попозже, когда будешь готов к чему-нибудь подобному, хорошо?
        }
        else
        {
                B_AI_Output(self,other,"DIA_Gestath_Drachen_09_02");      //Конечно. Почему нет?
...
{
        B_AI_Output(other,self,"DIA_Gestath_TEACHHUNTING_15_00"); //Научи меня потрошить животных.
        if((PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_FIRETONGUE] == FALSE) || (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_CRAWLERPLATE] == FALSE) || (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_MANDIBLES] == FALSE) || (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_DRGSNAPPERHORN] == FALSE) || ((GESTATH_DRAGONTROPHY == TRUE) && ((PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_DRAGONSCALE] == FALSE) || (PLAYER_TALENT_TAKEANIMALTROPHY[TROPHY_DRAGONBLOOD] == FALSE))))
        {
                ...
                {
                        B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_09_01"); //Почему нет? У меня есть немного свободного времени.
                        DIA_GESTATH_TEACHHUNTING_ONETIME = TRUE;
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_09_02"); //Что ты хочешь узнать?
                };
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_09_03"); //Сейчас я даже не представляю, чему еще можно обучить тебя.
        };
};
...
func void dia_gestath_teachhunting_firetongue()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_FireTongue_09_00");      //Язык огненного ящера нужно вырезать одним точным движением ножа, удерживая его при этом другой рукой.
        };
        Info_ClearChoices(dia_gestath_teachhunting);
};
func void dia_gestath_teachhunting_crawlerplate()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_CrawlerPlate_09_00");    //Чтобы снять панцирь с краулера, нужен хороший прочный нож.
        };
        Info_ClearChoices(dia_gestath_teachhunting);
};
func void dia_gestath_teachhunting_mandibles()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_Mandibles_09_00");       //Мандибулы сидят глубоко в черепе полевых хищников и краулеров. Чтобы вытащить их, нужно приложить некоторое усилие.
        };
        Info_ClearChoices(dia_gestath_teachhunting);
};
func void dia_gestath_teachhunting_drgsnapperhorn()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_DrgSnapperHorn_09_00");  //Лучше всего отделять рог драконьего снеппера от черепа при помощи толстого ножа.
        };
        Info_ClearChoices(dia_gestath_teachhunting);
};
func void dia_gestath_teachhunting_dragonscale()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_DragonScale_09_00");     //Чешую дракона очень сложно оторвать. Но когда ты уже думаешь, что у тебя ничего не получится, раз - и она все же отделяется.
        };
        Info_ClearChoices(dia_gestath_teachhunting);
};
func void dia_gestath_teachhunting_dragonblood()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Gestath_TEACHHUNTING_DragonBlood_09_00");     //Лучше всего брать кровь из брюха. Найди слабое место и вонзи туда острый нож.
        };
        Info_ClearChoices(dia_gestath_teachhunting);
};
