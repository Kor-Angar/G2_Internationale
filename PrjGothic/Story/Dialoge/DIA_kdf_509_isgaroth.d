...
{
        B_AI_Output(self,other,"DIA_Isgaroth_EXIT_01_00");        //Пусть Иннос всегда освещает твой путь.
...
{
        B_AI_Output(self,other,"DIA_Isgaroth_Hello_01_00");       //Да пребудет с тобой Иннос. Что я могу сделать для тебя, странник?
};
...
{
        B_AI_Output(other,self,"DIA_Isgaroth_Segen_15_00");       //Благослови меня!
        B_AI_Output(self,other,"DIA_Isgaroth_Segen_01_01");       //Благословляю тебя во имя Инноса. Пусть огонь Всевышнего горит в твоем сердце и дает тебе силы жить согласно его заветам.
        ISGAROTH_SEGEN = TRUE;
};
...
{
        B_AI_Output(other,self,"DIA_Isgaroth_Wolf_15_00");        //Меня послал Сержио. Он поручил мне свои обязанности. Что нужно сделать?
        B_AI_Output(self,other,"DIA_Isgaroth_Wolf_01_01");        //Здесь недавно появился черный волк. Найди его и убей.
        MIS_ISGAROTHWOLF = LOG_RUNNING;
        b_logentry(TOPIC_ISGAROTHWOLF,TOPIC_ISGAROTHWOLF_description_224); // "Около алтаря бродит черный волк. Я должен найти его и убить."
};
...
{
        B_AI_Output(other,self,"DIA_Isgaroth_tot_15_00"); //Я убил волка.
        B_AI_Output(self,other,"DIA_Isgaroth_tot_01_01"); //Хорошая работа, послушник. Ты мужественный человек. А теперь возвращайся в монастырь и принимайся за свои обязанности.
...
{
        B_AI_Output(other,self,"DIA_Isgaroth_Job_15_00"); //Что ты делаешь здесь?
        if((other.guild != GIL_KDF) && (other.guild != GIL_NOV))
        {
                B_AI_Output(self,other,"DIA_Isgaroth_Job_01_01"); //Я Маг Огня. Жрец нашего бога Инноса.
                B_AI_Output(self,other,"DIA_Isgaroth_Job_01_02"); //Этот алтарь посвящен ЕМУ, высшему богу, создателю огня и верховному судье.
        };
        B_AI_Output(self,other,"DIA_Isgaroth_Job_01_03"); //Ко мне приходят люди, чтобы помолиться Инносу и получить благословение.
        B_AI_Output(self,other,"DIA_Isgaroth_Job_01_04"); //А за небольшое пожертвование ты можешь получить от меня много полезного.
        Log_CreateTopic(TOPIC_KLOSTERTRADER,LOG_NOTE);
        b_logentry(TOPIC_KLOSTERTRADER,TOPIC_KLOSTERTRADER_description_225); // "Мастер Исгарот продает полезные магические предметы в часовне неподалеку от монастыря."
};
...
{
        b_givetradeinv(self);
        B_AI_Output(other,self,"DIA_Isgaroth_Trade_15_00");       //Покажи мне свои товары.
};
...
{
        B_AI_Output(other,self,"DIA_Isgaroth_Kloster_15_00");     //Куда ведет эта дорога?
        B_AI_Output(self,other,"DIA_Isgaroth_Kloster_01_01");     //Эта дорога ведет в монастырь Магов Огня. Однако вход в него разрешен только слугам Инноса.
        if(other.guild == GIL_NONE)
        {
                B_AI_Output(self,other,"DIA_Isgaroth_Kloster_01_02");     //Если ты хочешь стать послушником монастыря, ты должен принести овцу и...
                b_say_gold(self,other,SUMME_KLOSTER);
                Log_CreateTopic(TOPIC_KLOSTER,LOG_MISSION);
                Log_SetTopicStatus(TOPIC_KLOSTER,LOG_RUNNING);
                b_logentry(TOPIC_KLOSTER,TOPIC_KLOSTER_description_226); // "Чтобы стать послушником монастыря Инноса мне нужна овца и 1000 золотых монет."
        }
        else
        {
                B_AI_Output(self,other,"DIA_Isgaroth_Kloster_01_03");     //Так как ты уже решил присоединиться к другой группе, вход для тебя закрыт.
        };
};
...
{
        B_AI_Output(other,self,"DIA_ISgaroth_Vatras_15_00");      //Я несу сообщение от Ватраса.
        B_AI_Output(self,other,"DIA_Isgaroth_Vatras_01_01");      //Что это за сообщение?
        B_AI_Output(other,self,"DIA_Isgaroth_Vatras_15_02");      //Это письмо, вот.
        if(Npc_HasItems(other,itwr_vatrasmessage) == 1)
        {
                ...
                b_usefakescroll();
                B_AI_Output(self,other,"DIA_Isgaroth_Vatras_01_03");      //Хорошо, ты можешь сказать Ватрасу, что я получил его сообщение.
                B_AI_Output(self,other,"DIA_Isgaroth_Vatras_01_04");      //Возьми эти зелья в качестве награды за свое служение, они наверняка пригодятся тебе.
        ...
        else if(Npc_HasItems(other,itwr_vatrasmessage_open) == 1)
        {
                ...
                b_usefakescroll();
                B_AI_Output(self,other,"DIA_Isgaroth_Vatras_01_05");      //Печать сломана. О чем ты думал, идиот!
                B_AI_Output(self,other,"DIA_Isgaroth_Vatras_01_06");      //Иди и скажи Ватрасу, что я получил его сообщение.
                AI_StopProcessInfos(self);
        };
        VATRAS_RETURN = TRUE;
};
