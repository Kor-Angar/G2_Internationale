...
{
        B_AI_Output(other,self,"DIA_Engrom_HALLO_15_00"); //Как дела?
        B_AI_Output(self,other,"DIA_Engrom_HALLO_12_01"); //Паршиво!
        B_AI_Output(other,self,"DIA_Engrom_HALLO_15_02"); //Ммм!
        B_AI_Output(self,other,"DIA_Engrom_HALLO_12_03"); //А что еще я могу сказать? Изо дня в день я вижу только эту чертову реку.
        B_AI_Output(self,other,"DIA_Engrom_HALLO_12_04"); //Орки шныряют на левом берегу, бандиты - на правом, и каждый день я ем только мясо луркеров. Я уже не могу выносить все это!
};
...
{
        B_AI_Output(other,self,"DIA_Engrom_WhatAboutLeaving_15_00");      //А ты не думал о том, чтобы выбраться отсюда?
        B_AI_Output(self,other,"DIA_Engrom_WhatAboutLeaving_12_01");      //Да, конечно. С этим нет никаких проблем.
        B_AI_Output(self,other,"DIA_Engrom_WhatAboutLeaving_12_02");      //Сначала мне нужно прорубить себе дорогу через орды орков, поприветствовать всех остальных монстров, которых там тоже немало, а затем прошмыгнуть через Проход!
        B_AI_Output(self,other,"DIA_Engrom_WhatAboutLeaving_12_03");      //Что может быть легче!
        B_AI_Output(other,self,"DIA_Engrom_WhatAboutLeaving_15_04");      //Я же пришел сюда.
        B_AI_Output(self,other,"DIA_Engrom_WhatAboutLeaving_12_05");      //Ты хочешь сказать мне, что ты только что прошел через Проход?
        B_AI_Output(other,self,"DIA_Engrom_WhatAboutLeaving_15_06");      //Ну, в общем да!
        B_AI_Output(self,other,"DIA_Engrom_WhatAboutLeaving_12_07");      //Значит, тебе повезло. Пока вокруг творится такой бардак, я с места не сдвинусь.
};
...
{
        b_wasmachtjagd();
        B_AI_Output(self,other,"DIA_Engrom_Jagd_12_01");  //Охота - моя единственная отрада. Но мне бы хотелось хоть иногда видеть нормальную цель, а не этих отвратительных луркеров.
};
...
{
        B_AI_Output(other,self,"DIA_Engrom_BESSEN_15_00");        //Ты одержим Злом.
        B_AI_Output(other,self,"DIA_Engrom_BESSEN_15_01");        //Давай, я помогу тебе.
        B_AI_Output(self,other,"DIA_Engrom_BESSEN_12_02");        //(вопит) Не трогай меня!
        b_npcclearobsessionbydmt(self);
