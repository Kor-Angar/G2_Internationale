...
{
        B_AI_Output(other,self,"DIA_Angar_DI_HALLO_15_00");       //Как ты?
        if(Npc_IsDead(undeaddragon) == FALSE)
        {
                B_AI_Output(self,other,"DIA_Angar_DI_HALLO_04_01");       //Головные боли стали совсем невыносимыми.
                B_AI_Output(self,other,"DIA_Angar_DI_HALLO_04_02");       //Черт. Это когда-нибудь должно кончиться.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Angar_DI_HALLO_04_03");       //Не волнуйся обо мне. Нужно выбираться отсюда.
        };
};
...
{
        B_AI_Output(self,other,"DIA_Angar_DI_ORKS_04_00");        //Тебе, черт возьми, лучше поторопиться. Следующая атака орков может стоить нам жизней.
        B_AI_Output(other,self,"DIA_Angar_DI_ORKS_15_01");        //Я делаю, что могу.
        B_AI_Output(self,other,"DIA_Angar_DI_ORKS_04_02");        //Этого недостаточно.
        Info_ClearChoices(dia_angar_di_orks);
        Info_AddChoice(dia_angar_di_orks,dia_angar_di_orks_no_description,dia_angar_di_orks_no); // "Расслабься."
        Info_AddChoice(dia_angar_di_orks,dia_angar_di_orks_follow_description,dia_angar_di_orks_follow); // "Так помоги мне и прекрати скулить."
};
func void dia_angar_di_orks_follow()
{
        B_AI_Output(other,self,"DIA_Angar_DI_ORKS_follow_15_00"); //Так помоги мне и прекрати скулить.
        B_AI_Output(self,other,"DIA_Angar_DI_ORKS_follow_04_01"); //Хорошо. Но ты пойдешь первым. Вперед!
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(other,self,"DIA_Angar_DI_ORKS_no_15_00");     //Расслабься.
        B_AI_Output(self,other,"DIA_Angar_DI_ORKS_no_04_01");     //Тебе легко говорить. А я здесь вот-вот сойду с ума.
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Angar_DI_FOLLOW_15_00");      //Оставайся на месте.
        if(Npc_GetDistToWP(self,"SKELETTE") < 4000)
        {
                B_AI_Output(other,self,"DIA_Angar_DI_FOLLOW_15_01");      //Об остальном я сам позабочусь.
                B_AI_Output(self,other,"DIA_Angar_DI_FOLLOW_04_02");      //Удачи.
                AI_StopProcessInfos(self);
        ...
        {
                B_AI_Output(self,other,"DIA_Angar_DI_FOLLOW_04_03");      //Конечно.
                AI_StopProcessInfos(self);
...
{
        B_AI_Output(other,self,"DIA_Angar_DI_FOLLOWAGAIN_15_00"); //Иди за мной.
        B_AI_Output(self,other,"DIA_Angar_DI_FOLLOWAGAIN_04_01"); //Ты идешь первым.
        ANGAR_DI_PARTY = LOG_RUNNING;
        AI_StopProcessInfos(self);
        Npc_ExchangeRoutine(self,"FollowDI");
};
...
{
        B_AI_Output(self,other,"DIA_Angar_DI_FOLLOWSTOP_04_00");  //Аааа! Это головная боль сведет меня с ума!
};
...
{
        B_AI_Output(other,self,"DIA_Angar_DI_UNDEADDRGDEAD_15_00");       //Все кончено.
        B_AI_Output(self,other,"DIA_Angar_DI_UNDEADDRGDEAD_04_01");       //Можем мы, наконец, выбираться отсюда?
        B_AI_Output(other,self,"DIA_Angar_DI_UNDEADDRGDEAD_15_02");       //Да. Враг повержен.
        B_AI_Output(self,other,"DIA_Angar_DI_UNDEADDRGDEAD_04_03");       //Тогда не будем терять времени. Иди к капитану и скажи ему, чтобы он поднимал якорь.
        if(SC_KNOWSMADPSI == TRUE)
        {
                B_AI_Output(other,self,"DIA_Angar_DI_UNDEADDRGDEAD_15_04");       //Надеюсь, больше никого из членов твоей секты не осталось в живых.
                B_AI_Output(self,other,"DIA_Angar_DI_UNDEADDRGDEAD_04_05");       //Все может быть. Ищущие были очень сильны. Они даже меня чуть не поработили. Кто знает?
        };
        B_AI_Output(other,self,"DIA_Angar_DI_UNDEADDRGDEAD_15_06");       //Что ты будешь делать дальше?
        B_AI_Output(self,other,"DIA_Angar_DI_UNDEADDRGDEAD_04_07");       //Может быть, я осяду где-нибудь и стану фермером. Я устал от сражений.
        AI_StopProcessInfos(self);
        Npc_ExchangeRoutine(self,"Start");
};
