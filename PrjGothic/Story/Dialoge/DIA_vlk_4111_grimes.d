...
{
        B_AI_Output(self,other,"DIA_Grimes_Hallo_05_00"); //Ты? Кто бы мог подумать, что мы опять свидимся!
        B_AI_Output(self,other,"DIA_Grimes_Hallo_05_01"); //Похоже, мы оба умеем выживать, но, как видишь, я застрял в этой шахте.
...
{
        B_AI_Output(other,self,"DIA_Grimes_Erz_15_00");   //Сколько руды вам удалось добыть?
        B_AI_Output(self,other,"DIA_Grimes_Erz_05_01");   //Несколько ящиков. Но их здесь уже нет.
        B_AI_Output(self,other,"DIA_Grimes_Erz_05_02");   //Паладин Маркос решил отправить руду в замок, так как нас все время атаковали орки.
};
...
{
        B_AI_Output(other,self,"DIA_Grimes_Weg_15_00");   //Ты знаешь, в каком направлении пошел Маркос?
        B_AI_Output(self,other,"DIA_Grimes_Weg_05_01");   //Он не пошел прямой дорогой. Вместо этого он хотел пересечь реку там, где она замерзла.
        B_AI_Output(self,other,"DIA_Grimes_Weg_05_02");   //Он хотел пойти в обход и подойти к замку с запада.
};
...
{
        B_AI_Output(other,self,"DIA_Grimes_Grimes_15_00");        //Ты хочешь покинуть Долину?
        B_AI_Output(self,other,"DIA_Grimes_Grimes_05_01");        //Нет, я лучше продолжу добывать руду.
        B_AI_Output(self,other,"DIA_Grimes_Grimes_05_02");        //Я не позволю каким-то оркам остановить меня - я и не такое видал.
        B_AI_Output(self,other,"DIA_Grimes_Grimes_05_03");        //Как тогда, когда мы работали в старой шахте и неожиданно прорвалась вода...
        B_AI_Output(self,other,"DIA_Grimes_Grimes_05_04");        //Но это старая история.
};
...
{
        B_AI_Output(other,self,"DIA_Grimes_PERM_15_00");  //Как твоя работа?
        B_AI_Output(self,other,"DIA_Grimes_PERM_05_01");  //Эта скала ужасно твердая - она как будто не хочет отдавать нам руду.
};
