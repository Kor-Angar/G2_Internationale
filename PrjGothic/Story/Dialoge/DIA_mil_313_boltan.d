...
{
        B_AI_Output(self,other,"DIA_Boltan_Add_05_00");   //Что тебе нужно?
};
...
{
        B_AI_Output(other,self,"DIA_Boltan_Add_15_01");   //Я хочу увидеть заключенных.
        if((KAPITEL == 3) && (MIS_RESCUEBENNET != LOG_SUCCESS))
        {
                if(other.guild == GIL_SLD)
                {
                        B_AI_Output(self,other,"DIA_Boltan_Add_05_07");   //Да, иди попрощайся со своим приятелем.
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Boltan_Add_05_06");   //Мы отпустили эту свинью, что убила паладина Лотара.
                };
        }
        else if((CANTHAR_AUSGELIEFERT == FALSE) && (SARAH_AUSGELIEFERT == FALSE) && (RENGARU_AUSGELIEFERT == FALSE) && (NAGUR_AUSGELIEFERT == FALSE))
        {
                B_AI_Output(self,other,"DIA_Boltan_Add_05_02");   //Все камеры сейчас пусты.
        }
        else if(other.guild == GIL_MIL)
        {
                B_AI_Output(self,other,"DIA_Boltan_Add_05_04");   //Хорошо, приятель.
        }
        else if((other.guild == GIL_PAL) || (other.guild == GIL_KDF))
        {
                B_AI_Output(self,other,"DIA_Boltan_Add_05_05");   //Конечно.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Boltan_Add_05_03");   //Иди, но не задерживайся там слишком долго, понятно?
        };
};
...
{
        B_AI_Output(self,other,"DIA_Boltan_Add_05_08");   //Ты ведь пришел не затем, чтобы вызволить отсюда своего приятеля, нет?
        B_AI_Output(self,other,"DIA_Boltan_Add_05_09");   //Забудь об этом! Как только я подниму тревогу, парни будут здесь в мгновение ока!
        B_AI_Output(self,other,"DIA_Boltan_Add_05_10");   //И тогда мы сделаем из тебя котлету! (грязный смех)
};
...
{
        B_AI_Output(self,other,"DIA_Boltan_Add_05_11");   //Эй, ты тот парень, что написал жалобу на Кантара, тогровца.
        B_AI_Output(self,other,"DIA_Boltan_Add_05_12");   //Много людей приходило, чтобы похлопотать за него. Уважаемых людей.
        B_AI_Output(self,other,"DIA_Boltan_Add_05_13");   //Должно быть, произошло какое-то недоразумение. Такое бывает.
        B_AI_Output(self,other,"DIA_Boltan_Add_05_14");   //В будущем тебе стоит быть тщательнее выбирать, кого ты хочешь упечь в тюрьму.
};
