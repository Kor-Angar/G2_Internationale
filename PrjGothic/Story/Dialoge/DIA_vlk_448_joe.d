...
{
        B_AI_Output(self,other,"DIA_Joe_Hallo_10_00");    //Эй, спасибо, парень. Действительно, огромное спасибо. Я уж думал, мне никогда не выбраться отсюда...
        b_giveplayerxp(XP_AMBIENT * 4);
        B_AI_Output(other,self,"DIA_Joe_Hallo_15_01");    //Что ты делаешь ЗДЕСЬ?
        B_AI_Output(self,other,"DIA_Joe_Hallo_10_02");    //Я оказался взаперти. Дверь была открыта, и я хотел просто пошарить там - но как только я зашел внутрь, пришел стражник и запер эту чертову дверь.
        B_AI_Output(self,other,"DIA_Joe_Hallo_10_03");    //Я попал в дурацкое положение - и был бы очень благодарен, если бы все это осталось между нами.
        B_AI_Output(other,self,"DIA_Joe_Hallo_15_04");    //Понимаю. Да уж, тут действительно нечем гордиться.
        B_AI_Output(self,other,"DIA_Joe_Hallo_10_05");    //Я сейчас был бы не прочь что-нибудь выпить.
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(other,self,"DIA_Joe_Perm_15_00");     //Все в порядке?
        B_AI_Output(self,other,"DIA_Joe_Perm_10_01");     //Спасибо, что освободил меня.
        AI_StopProcessInfos(self);
};
...
func void dia_joe_sign_info()
{
        ...
        B_AI_Output(self,other,"DIA_Joe_Sign_10_00");     //Эй, привет - у нас есть общие друзья. И я хотел бы выразить свою благодарность за спасение.
        B_AI_Output(self,other,"DIA_Joe_Sign_10_01");     //Вот, возьми эти отмычки - я уверен, они пригодятся тебе.
