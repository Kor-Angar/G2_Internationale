...
{
        B_AI_Output(self,other,"DIA_Engor_HALLO_13_00");  //А, ты тот парень, которому удалось пройти через Проход?
        B_AI_Output(other,self,"DIA_Engor_HALLO_15_01");  //Да.
        B_AI_Output(self,other,"DIA_Engor_HALLO_13_02");  //Отлично. Я Энгор - я обеспечиваю эту экспедицию.
        B_AI_Output(self,other,"DIA_Engor_HALLO_13_03");  //Только не думай, что можешь получить от меня что-нибудь бесплатно!
        B_AI_Output(self,other,"DIA_Engor_HALLO_13_04");  //Но если в твоих карманах позванивает золото, мы всегда договоримся.
        Log_CreateTopic(TOPIC_TRADER_OC,LOG_NOTE);
        b_logentry(TOPIC_TRADER_OC,TOPIC_TRADER_OC_description_566); // "Энгор заведует припасами замка и ведет небольшой бизнес на стороне."
};
...
func void dia_engor_handeln_info()
{
        ...
        B_AI_Output(other,self,"DIA_Engor_HANDELN_15_00");        //Покажи мне свои товары.
};
...
{
        B_AI_Output(other,self,"DIA_Engor_ABOUTPARLAF_15_00");    //Ты распределяешь пайки, как я слышал?
        B_AI_Output(self,other,"DIA_Engor_ABOUTPARLAF_13_01");    //Это так. А что? Мне теперь и тебя кормить придется?
        B_AI_Output(self,other,"DIA_Engor_ABOUTPARLAF_13_02");    //Если тебе что-то нужно, тебе придется платить за это - как и всем остальным.
};
...
{
        B_AI_Output(other,self,"DIA_Engor_Ruestung_15_00");       //У тебя есть что-нибудь интересное для меня?
        B_AI_Output(self,other,"DIA_Engor_Ruestung_13_01");       //Я могу продать тебе хорошие доспехи - тяжелые доспехи ополчения. Если, конечно, тебе это интересно.
        B_AI_Output(self,other,"DIA_Engor_Ruestung_13_02");       //Они недешевы, конечно же. Но если у тебя есть золото, ты получишь их.
};
...
func void dia_engor_rskaufen_info()
{
        ...
        {
                B_AI_Output(other,self,"DIA_Engor_RSkaufen_15_00");       //Дай мне доспехи.
                B_AI_Output(self,other,"DIA_Engor_RSkaufen_13_01");       //Вот, держи, они надежно защитят тебя - это чертовски хорошие доспехи.
                b_giveinvitems(self,other,itar_mil_m,1);
                DIA_ENGOR_RSKAUFEN_PERM = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Engor_RSkaufen_13_02");       //Сначала принеси золото.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Engor_HELP_15_00");   //Может, я смогу помочь тебе в твоей работе?
        B_AI_Output(self,other,"DIA_Engor_HELP_13_01");   //Хмм... Конечно, почему нет? Мне не помешала бы помощь.
        B_AI_Output(other,self,"DIA_Engor_HELP_15_02");   //Так что нужно сделать?
        B_AI_Output(self,other,"DIA_Engor_HELP_13_03");   //Наши запасы продовольствия на исходе. Хуже всего, что у нас почти не осталось мяса.
        B_AI_Output(self,other,"DIA_Engor_HELP_13_04");   //Так что если сможешь раздобыть мясо, сырое или приготовленное, окорока или колбасу, я был бы очень благодарен. Ну, как? Ты поможешь нам?
        Info_ClearChoices(dia_engor_help);
        Info_AddChoice(dia_engor_help,dia_engor_help_no_description,dia_engor_help_no); // "У меня нет времени на это."
        Info_AddChoice(dia_engor_help,dia_engor_help_yes_description,dia_engor_help_yes); // "Не волнуйся я принесу тебе мясо."
};
func void dia_engor_help_no()
{
        B_AI_Output(other,self,"DIA_Engor_HELP_NO_15_00");        //У меня нет времени на это.
        B_AI_Output(self,other,"DIA_Engor_HELP_NO_13_01");        //Тогда зачем ты тратишь мое время попусту?
        MIS_ENGOR_BRINGMEAT = LOG_OBSOLETE;
        AI_StopProcessInfos(self);
};
func void dia_engor_help_yes()
{
        B_AI_Output(other,self,"DIA_Engor_HELP_YES_15_00");       //Не волнуйся, я принесу тебе мясо.
        B_AI_Output(self,other,"DIA_Engor_HELP_YES_13_01");       //Две дюжины кусков хватило бы, чтобы накормить все эти голодные рты. Возвращайся, когда у тебя будет это мясо. А мне нужно работать.
        Log_CreateTopic(TOPIC_BRINGMEAT,LOG_MISSION);
        Log_SetTopicStatus(TOPIC_BRINGMEAT,LOG_RUNNING);
        b_logentry(TOPIC_BRINGMEAT,TOPIC_BRINGMEAT_description_567); // "Энгору нужно две дюжины кусков мяса чтобы накормить людей в замке."
        b_logentry(TOPIC_BRINGMEAT,TOPIC_BRINGMEAT_description_568); // "Неважно что это будет - колбаса окорок сырое или жареное мясо. Что угодно лишь бы это можно было жевать."
...
{
        B_AI_Output(other,self,"DIA_Engor_BRINGMEAT_15_00");      //Вот, я принес тебе кое-что.
        ...
        {
                B_AI_Output(self,other,"DIA_Engor_BRINGMEAT_13_01");      //Для начала и это неплохо, но мне нужно больше.
        };
        if(MEAT_COUNTER == MEAT_AMOUNT)
        {
                B_AI_Output(self,other,"DIA_Engor_BRINGMEAT_13_02");      //Отлично, ты принес достаточно мяса. Этого хватит на некоторое время.
                B_AI_Output(self,other,"DIA_Engor_BRINGMEAT_13_03");      //Но даже не надейся, что я теперь буду давать тебе что-нибудь бесплатно!
...
{
        B_AI_Output(other,self,"DIA_Engor_Business_15_00");       //Как бизнес?
        if((hero.guild == GIL_MIL) || (hero.guild == GIL_PAL) || (hero.guild == GIL_KDF))
        {
                B_AI_Output(self,other,"DIA_Engor_Business_13_01");       //Неважно. Я надеюсь, что хотя бы у этих охотников на драконов есть какое-нибудь золото.
        };
        if((hero.guild == GIL_SLD) || (hero.guild == GIL_DJG))
        {
                B_AI_Output(self,other,"DIA_Engor_Business_13_02");       //Паршиво! Паладины ничего не покупают.
        };
        B_AI_Output(self,other,"DIA_Engor_Business_13_03");       //А как насчет тебя? Ты хочешь что-нибудь купить?
};
