...
func void dia_hilda_hallo_info()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Hilda_Hallo_17_00");  //Эй, тебя сюда звали? Немедленно убирайся из моего дома. Здесь не подают милостыню.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Hilda_Hallo_17_01");  //Да, что еще?
        };
        AI_StopProcessInfos(self);
};
...
func void dia_hilda_waszuessen_info()
{
        ...
        if(hero.guild == GIL_NONE)
        {
                B_AI_Output(other,self,"DIA_Hilda_WasZuEssen_15_00");     //Ты дашь мне что-нибудь поесть?
                if(MIS_LOBART_RUEBEN == LOG_SUCCESS)
                {
                        if(!Npc_KnowsInfo(other,dia_hilda_pfannetoolate))
                        {
                                if(HILDA_STEW_DAY != Wld_GetDay())
                                {
                                        ...
                                        B_AI_Output(self,other,"DIA_Hilda_WasZuEssen_17_01");     //Вот, возьми. Ты хороший парень.
                                        HILDA_STEW_DAY = Wld_GetDay();
                                }
                                else if((Wld_GetDay() == 0) && (rueben_tagnull == FALSE))
                                {
                                        B_AI_Output(self,other,"DIA_Hilda_WasZuEssen_17_02");     //За добавкой можешь зайти завтра.
                                        b_giveinvitems(self,other,itfo_stew,1);
                                        HILDA_STEW_DAY = Wld_GetDay();
                                        rueben_tagnull = TRUE;
                                        Log_CreateTopic(TOPIC_BONUS,LOG_NOTE);
                                        b_logentry(TOPIC_BONUS,TOPIC_BONUS_description_62); // "Я могу получать тарелку тушеной репы у Хильды каждый день."
                                }
                                else
                                {
                                        B_AI_Output(self,other,"DIA_Hilda_WasZuEssen_17_03");     //Ты сегодня уже получил свою порцию! Заходи попозже.
                                };
                        }
                        else
                        {
                                B_AI_Output(self,other,"DIA_Hilda_WasZuEssen_17_04");     //Мы не собираемся кормить здесь всяких лодырей. Убирайся!
                                AI_StopProcessInfos(self);
                        };
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Hilda_WasZuEssen_17_05");     //Я кормлю только тех, кто работает здесь!
                };
        }
        else
        {
                B_AI_Output(self,other,"DIA_Hilda_WasZuEssen_17_06");     //Я уверена, у тебя достаточно денег, чтобы купить себе еды в городе.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Hilda_BringBeet_15_00");      //Я принес тебе репу...
        if(Npc_HasItems(other,itpl_beet) >= 20)
        {
                b_giveinvitems(other,self,itpl_beet,Npc_HasItems(other,itpl_beet));
                B_AI_Output(self,other,"DIA_Hilda_BringBeet_17_01");      //Отлично! (смеется) Этого должно хватить, чтобы накормить наших работников до отвала!
                B_AI_Output(self,other,"DIA_Hilda_BringBeet_17_02");      //Раз уж ты все равно здесь... Я видела, как мимо прошел странствующий торговец. Это было несколько минут назад.
                B_AI_Output(self,other,"DIA_Hilda_BringBeet_17_03");      //Я думаю, он где-нибудь остановился по пути в город. Сходи к нему. Может у него найдется сковородка для меня.
        ...
        else
        {
                if(hero.guild == GIL_NONE)
                {
                        B_AI_Output(self,other,"DIA_Hilda_BringBeet_17_04");      //Что?! Это все?
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Hilda_BringBeet_17_05");      //Этого недостаточно.
                };
                B_AI_Output(self,other,"DIA_Hilda_BringBeet_17_06");      //Иди в поле и собери еще. Мои люди голодны!
                AI_StopProcessInfos(self);
        };
};
...
{
        B_AI_Output(other,self,"DIA_Hilda_Einkaufen_15_00");      //Дай мне золота, и я схожу к этому торговцу для тебя...
        if(hero.guild == GIL_NONE)
        {
                B_AI_Output(self,other,"DIA_Hilda_Einkaufen_17_01");      //А тебе можно доверять? Только попробуй потратить эти деньги на выпивку, слышишь?!
        };
        ...
        b_logentry(TOPIC_HILDA,TOPIC_HILDA_description_63); // "Хильда жена фермера Лобарта чтобы я купил для нее у странствующего торговца сковороду."
};
...
{
        B_AI_Output(other,self,"DIA_Hilda_PfanneGeholt_15_00");   //Вот твоя сковородка.
        b_giveinvitems(other,self,itmi_pan,1);
        B_AI_Output(self,other,"DIA_Hilda_PfanneGeholt_17_01");   //Отлично. Посмотрим, хорошая ли она...
...
func void dia_hilda_pfannetoolate_info()
{
        if(hero.guild == GIL_NONE)
        {
                B_AI_Output(self,other,"DIA_Hilda_PfanneTooLate_17_00");  //У тебя еще хватает наглости появляться здесь? Что ты сделал с моими деньгами, ты, бездельник?!
        }
        else
        {
                B_AI_Output(self,other,"DIA_Hilda_PfanneTooLate_17_01");  //Где ты шлялся все это время? И где мои деньги, что я дала тебе на сковороду?
        };
        if(Npc_HasItems(other,itmi_pan) > 0)
        {
                B_AI_Output(other,self,"DIA_Hilda_PfanneTooLate_15_02");  //Извини, я немного подзадержался. Вот твоя сковородка!
                b_giveinvitems(other,self,itmi_pan,1);
                B_AI_Output(self,other,"DIA_Hilda_PfanneTooLate_17_03");  //Ах-х, давай же ее сюда! Ну и нахал же ты - просто невероятно!
                MIS_HILDA_PFANNEKAUFEN = LOG_SUCCESS;
        ...
        else
        {
                ...
                {
                        B_AI_Output(self,other,"DIA_Hilda_PfanneTooLate_17_04");  //Ах! Они все еще у тебя! Давай их назад. Живее!
                        b_giveinvitems(other,self,itmi_gold,20);
                        B_AI_Output(self,other,"DIA_Hilda_PfanneTooLate_17_05");  //Ну и нахал же ты - просто невероятно!
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Hilda_PfanneTooLate_17_06");  //У тебя нет денег?! Я дала тебе 20 золотых!
                        B_AI_Output(self,other,"DIA_Hilda_PfanneTooLate_17_07");  //Прочь с глаз моих, грязный вор!
...
{
        B_AI_Output(other,self,"DIA_Hilda_KRANK_15_00");  //Как ты себя чувствуешь?
        B_AI_Output(self,other,"DIA_Hilda_KRANK_17_01");  //Отвратительно. У меня опять эта ужасная лихорадка.
        B_AI_Output(self,other,"DIA_Hilda_KRANK_17_02");  //Мне бы нужно сходить к городскому лекарю, но я слишком слаба для этого.
        if(DIA_HILDA_KRANK_ONTIME == FALSE)
        {
                Info_ClearChoices(dia_hilda_krank);
                Info_AddChoice(dia_hilda_krank,dia_hilda_krank_besserung_description,dia_hilda_krank_besserung); // "Поправляйся. А мне нужно идти."
                Info_AddChoice(dia_hilda_krank,dia_hilda_krank_helfen_description,dia_hilda_krank_helfen); // "Могу я чем-нибудь помочь?"
...
{
        B_AI_Output(other,self,"DIA_Hilda_KRANK_besserung_15_00");        //Поправляйся. А мне нужно идти.
        B_AI_Output(self,other,"DIA_Hilda_KRANK_besserung_17_01");        //Остается надеяться, что вскоре мне станет лучше.
        AI_StopProcessInfos(self);
};
func void dia_hilda_krank_helfen()
{
        B_AI_Output(other,self,"DIA_Hilda_KRANK_helfen_15_00");   //Могу я чем-нибудь помочь?
        B_AI_Output(self,other,"DIA_Hilda_KRANK_helfen_17_01");   //Было бы великолепно, если бы ты мог сходить к Ватрасу и принести мне лекарство от него.
        B_AI_Output(self,other,"DIA_Hilda_KRANK_helfen_17_02");   //Он знает, что мне нужно! Самой мне не дойти.
        Log_CreateTopic(TOPIC_HEALHILDA,LOG_MISSION);
        Log_SetTopicStatus(TOPIC_HEALHILDA,LOG_RUNNING);
        b_logentry(TOPIC_HEALHILDA,TOPIC_HEALHILDA_description_64); // "Жена Лобарта Хильда больна но у Ватраса есть лекарство которое может вылечить ее."
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(other,self,"DIA_Hilda_HEILUNGBRINGEN_15_00"); //Я принес тебе лекарство.
        b_giveinvitems(other,self,itpo_healhilda_mis,1);
        B_AI_Output(self,other,"DIA_Hilda_HEILUNGBRINGEN_17_01"); //Ох. Если бы только таких людей, как ты, было побольше. Огромное спасибо.
        b_useitem(self,itpo_healhilda_mis);
        B_AI_Output(self,other,"DIA_Hilda_HEILUNGBRINGEN_17_02"); //Надеюсь, этих нескольких монет будет достаточно.
...
func void dia_hilda_disturb_info()
{
        ...
        {
                B_AI_Output(other,self,"DIA_Hilda_DISTURB_15_00");        //Как твое здоровье?
                B_AI_Output(self,other,"DIA_Hilda_DISTURB_17_01");        //Уже лучше, спасибо тебе.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Hilda_DISTURB_17_02");        //Все еще плохо.
        };
};
