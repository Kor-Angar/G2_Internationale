...
{
        B_AI_Output(self,other,"DIA_Mortis_Hallo_13_00"); //Что тебе нужно? Пека здесь нет. А без него ты ничего не получишь. Заходи позже.
};
...
{
        B_AI_Output(other,self,"DIA_Mortis_Waffe_15_00"); //А где Пек?
        B_AI_Output(self,other,"DIA_Mortis_Waffe_13_01"); //Ты только что вступил в наши ряды? Ну, тогда добро пожаловать.
        B_AI_Output(self,other,"DIA_Mortis_Waffe_13_02"); //Пек ушел в город уже давно. Готов поспорить, что он обхаживает Ваню в Красном Фонаре.
};
...
{
        B_AI_Output(other,self,"DIA_Mortis_Paket_15_00"); //Ты что-нибудь знаешь о тюке травки?
        B_AI_Output(self,other,"DIA_Mortis_Paket_13_01"); //Хм... последний раз, когда я был в портовом кабаке, я слышал, как Кардиф говорил об этом с каким-то парнем.
        B_AI_Output(other,self,"DIA_Mortis_Paket_15_02"); //Что это за парень?
        B_AI_Output(self,other,"DIA_Mortis_Paket_13_03"); //Понятия не имею. Но он сказал, что нашел отличное место для хранения травы - а затем они долго ржали как пьяные орки.
        b_logentry(TOPIC_WAREHOUSE,TOPIC_WAREHOUSE_description_255); // "Мортис слышал как трактирщик Кардиф и кто-то еще разговаривали о тюке в портовой таверне. Они обсуждали где лучше хранить его."
};
...
{
        B_AI_Output(other,self,"DIA_Mortis_Redlight_15_00");      //Ты хорошо знаешь портовый квартал? Я хочу найти того, кто продает болотную траву.
        B_AI_Output(self,other,"DIA_Mortis_Redlight_13_01");      //Ну-у... Народ там не особенно разговорчивый, и они уж точно ничего не скажут городскому стражнику.
        B_AI_Output(self,other,"DIA_Mortis_Redlight_13_02");      //Если ты хочешь разузнать что-либо там, тебе лучше... нет, ты ДОЛЖЕН снять свои доспехи.
        B_AI_Output(other,self,"DIA_Mortis_Redlight_15_03");      //Хорошо, а что дальше?
        B_AI_Output(self,other,"DIA_Mortis_Redlight_13_04");      //Кабак и бордель - вероятно, самые лучшие места для поисков. Если тебе вообще удастся что-либо узнать, то ты узнаешь это там.
        b_logentry(TOPIC_REDLIGHT,TOPIC_REDLIGHT_description_256); // "Мортис полагает что если я хочу купить болотную траву в портовом квартале мне лучше снять все доспехи. Лучше всего попытать счасться в таверне или борделе."
};
...
{
        B_AI_Output(other,self,"DIA_Mortis_CanTeach_15_00");      //Я хочу стать сильнее.
        if((other.guild == GIL_MIL) || (other.guild == GIL_PAL))
        {
                B_AI_Output(self,other,"DIA_Mortis_CanTeach_13_01");      //Понятно. Если у тебя достаточно опыта, я могу потренировать тебя.
                Log_CreateTopic(TOPIC_CITYTEACHER,LOG_NOTE);
                b_logentry(TOPIC_CITYTEACHER,TOPIC_CITYTEACHER_description_257); // "Ополченец Мортис может помочь мне повысить мою силу."
                MORTIS_TEACHSTR = TRUE;
        }
        else
        {
                B_AI_Output(self,other,"DIA_Mortis_CanTeach_13_02");      //Конечно, хочешь. Но пока ты не станешь одним из нас или паладином, я не буду помогать тебе.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Mortis_Teach_15_00"); //Я хочу стать сильнее.
...
func void dia_mortis_teach_back()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Mortis_Teach_13_00"); //Ты и так достаточно силен. Если же ты стремишься к большему, найди себе другого учителя.
