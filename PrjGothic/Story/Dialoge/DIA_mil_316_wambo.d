...
{
        B_AI_Output(other,self,"DIA_Wambo_Job_15_00");    //Что ты делаешь здесь?
        B_AI_Output(self,other,"DIA_Wambo_Job_03_01");    //Я слежу за тем, чтобы никто не входил в дома.
        B_AI_Output(self,other,"DIA_Wambo_Job_03_02");    //Лорд Хаген лично отдал этот приказ. И даже не думай нарушить его!
};
...
{
        B_AI_Output(other,self,"DIA_Wambo_Situation_15_00");      //Как дела?
        B_AI_Output(self,other,"DIA_Wambo_Situation_03_01");      //Сейчас все спокойно.
};
...
{
        B_AI_Output(other,self,"DIA_Wambo_Ramirez_15_00");        //Я пришел от нашего общего друга, Рамиреза.
        B_AI_Output(self,other,"DIA_Wambo_Ramirez_03_01");        //Ох? Этот Рамирез тот еще подлец. И он мне не друг. Да, ладно. Ты принес мне что-нибудь?
        B_AI_Output(other,self,"DIA_Wambo_Ramirez_15_02");        //Это зависит от того, что ты хочешь.
        AI_PlayAni(self,"T_SEARCH");
        B_AI_Output(self,other,"DIA_Wambo_Ramirez_03_03");        //(тихо) Хорошо, поступим так: ты платишь мне 250 золотых монет. И меня не будет здесь всю ночь.
        B_AI_Output(self,other,"DIA_Wambo_Ramirez_03_04");        //И, если ты уж такой подозрительный, давай договоримся так: если я появлюсь здесь, то нашу сделку можно будет считать расторгнутой.
};
...
{
        B_AI_Output(other,self,"DIA_Wambo_Deal_15_00");   //У меня есть деньги...
        if(b_giveinvitems(other,self,itmi_gold,250))
        {
                B_AI_Output(self,other,"DIA_Wambo_Deal_03_01");   //Хорошо, тогда я исчезаю на всю ночь.
                B_AI_Output(self,other,"DIA_Wambo_Deal_03_02");   //И помни: если у тебя возникнут проблемы, я не знаю тебя.
        ...
        {
                B_AI_Output(other,self,"DIA_Wambo_Deal_15_03");   //...но с собой их не захватил.
                B_AI_Output(self,other,"DIA_Wambo_Deal_03_04");   //Я не выношу, когда кто-нибудь пытается одурачить меня. Так что давай сюда золото.
                AI_StopProcessInfos(self);
        };
};
