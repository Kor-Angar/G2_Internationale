...
{
        B_AI_Output(other,self,"DIA_Pal_212_Schiffswache_WERSEIDIHR_15_00");      //Кто вы?
        B_AI_Output(self,other,"DIA_Pal_212_Schiffswache_WERSEIDIHR_08_01");      //Мы эмиссары короля Робара и слуги Инноса. Также нас еще называют паладинами.
};
...
{
        B_AI_Output(other,self,"DIA_Pal_212_Schiffswache_WASMACHSTDU2_15_00");    //Что вы делаете здесь?
        if(MIS_SHIPISFREE == FALSE)
        {
                if((hero.guild != GIL_PAL) && (hero.guild != GIL_KDF))
                {
                        B_AI_Output(self,other,"DIA_Pal_212_Schiffswache_WASMACHSTDU2_08_01");    //Тебе не попасть на этот корабль. Это все, что тебе нужно знать.
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Pal_212_Schiffswache_WASMACHSTDU2_08_02");    //Мы стоим здесь на страже. Никто не может взойти на этот корабль. Ты в том числе. Извини.
                };
        }
        else
        {
                B_AI_Output(self,other,"DIA_Pal_212_Schiffswache_WASMACHSTDU2_08_03");    //Из-за таких, как ты, мне приходится тратить мое время попусту. Я бы уж лучше отправился с нашими братьями в Долину Рудников.
        };
        AI_StopProcessInfos(self);
};
