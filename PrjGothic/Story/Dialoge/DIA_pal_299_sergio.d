...
func void dia_sergio_exit_info()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Sergio_EXIT_04_00");  //Да осветит Иннос твой путь.
        };
...
{
        B_AI_Output(self,other,"DIA_Sergio_WELCOME_04_00");       //Да пребудет с тобой Иннос, чем я могу помочь тебе?
};
...
{
        B_AI_Output(self,other,"DIA_Sergio_Isgaroth_04_00");      //Ты молился за моих товарищей. Я благодарен тебе за это. Скажи мне, что я могу сделать для тебя.
        Info_ClearChoices(dia_sergio_isgaroth);
        Info_AddChoice(dia_sergio_isgaroth,dia_sergio_isgaroth_spende_description,dia_sergio_isgaroth_spende); // "Как насчет небольшого пожертвования?"
        Info_AddChoice(dia_sergio_isgaroth,dia_sergio_isgaroth_xp_description,dia_sergio_isgaroth_xp); // "Ты не мог бы поделиться своим боевым опытом?"
};
func void dia_sergio_isgaroth_spende()
{
        B_AI_Output(other,self,"DIA_Sergio_Isgaroth_Spende_15_00");       //Как насчет небольшого пожертвования?
        B_AI_Output(self,other,"DIA_Sergio_Isgaroth_Spende_04_01");       //Пусть это золото сослужит тебе добрую службу.
        b_giveinvitems(self,other,itmi_gold,100);
        Info_ClearChoices(dia_sergio_isgaroth);
};
func void dia_sergio_isgaroth_xp()
{
        B_AI_Output(other,self,"DIA_Sergio_Isgaroth_XP_15_00");   //Ты не мог бы поделиться своим боевым опытом?
        B_AI_Output(self,other,"DIA_Sergio_Isgaroth_XP_04_01");   //Когда ты сражаешься, постарайся, чтобы никто не мог атаковать тебя сзади.
...
{
        B_AI_Output(other,self,"DIA_Sergio_Aufgabe_15_00");       //Мне нужен доступ в библиотеку.
        B_AI_Output(self,other,"DIA_Sergio_Aufgabe_04_01");       //Ну, я не могу обеспечить тебе доступ. Для этого ты должен сначала выполнить свои задания.
        B_AI_Output(self,other,"DIA_Sergio_Aufgabe_04_02");       //Но я могу помочь тебе. Иди к Мастеру Исгароту и поговори с ним. Я слышал, ему нужна помощь и собирался сам помочь ему, но я поручаю эту задачу тебе.
        SERGIO_SENDS = TRUE;
        ...
        b_logentry(TOPIC_ISGAROTHWOLF,TOPIC_ISGAROTHWOLF_description_395); // "Мастеру Исгароту необходима помощь в часовне. Я должен найти его."
};
...
{
        B_AI_Output(other,self,"DIA_Sergio_WHAT_15_00");  //Что ты делаешь здесь?
        B_AI_Output(self,other,"DIA_Sergio_WHAT_04_01");  //Я молюсь Инносу, чтобы он укрепил мою руку и мою волю.
        B_AI_Output(self,other,"DIA_Sergio_WHAT_04_02");  //Тогда я буду готов к любым опасностям и уничтожу всех его врагов с его именем на устах.
        B_AI_Output(other,self,"DIA_Sergio_WHAT_15_03");  //Каких врагов?
        B_AI_Output(self,other,"DIA_Sergio_WHAT_04_04");  //Всех тех, кто противится воле Инноса. Не важно, человек это или вызванное существо.
};
...
{
        B_AI_Output(other,self,"DIA_Sergio_Babo_15_00");  //Не мог бы ты немного потренировать Бабо?
        B_AI_Output(self,other,"DIA_Sergio_Babo_04_01");  //А почему он не попросит сам?
        B_AI_Output(other,self,"DIA_Sergio_Babo_15_02");  //Я думаю, он робеет.
        B_AI_Output(self,other,"DIA_Sergio_Babo_04_03");  //Понимаю. Хорошо, если это так много значит для него, я буду тренировать его каждое утро в течение двух часов. Мы будем начинать в 5 утра. Можешь передать ему это.
        Npc_ExchangeRoutine(self,"TRAIN");
        b_startotherroutine(babo,"TRAIN");
        b_logentry(TOPIC_BABOTRAIN,TOPIC_BABOTRAIN_description_396); // "Сержио согласился тренироваться с Бабо по два часа каждое утро."
};
...
{
        B_AI_Output(other,self,"DIA_Sergio_WHY_15_00");   //Почему ты не с другими паладинами?
        B_AI_Output(self,other,"DIA_Sergio_WHY_04_01");   //Может показаться немного странным, что я здесь, однако не надо забывать, что мы, паладины, также служим магам, так как они проповедуют волю Инноса.
        B_AI_Output(self,other,"DIA_Sergio_WHY_04_02");   //Мы, паладины, - воины Инноса. Его воля - закон для нас. В настоящий момент я жду новых приказов от магов.
};
...
{
        B_AI_Output(other,self,"DIA_Sergio_ORDERS_15_00");        //Ты уже получил новые приказы?
        B_AI_Output(self,other,"DIA_Sergio_ORDERS_04_01");        //Пока нет, и у меня есть время найти силу в молитвах.
};
...
{
        B_AI_Output(other,self,"DIA_Sergio_Start_15_00"); //Ты должен сопровождать меня к Проходу.
        B_AI_Output(self,other,"DIA_Sergio_Start_04_01"); //Хорошо, я сделаю это. Я знаю дорогу, иди за мной.
        AI_StopProcessInfos(self);
...
{
        B_AI_Output(other,self,"DIA_Sergio_Guide_15_00"); //Как дела?
        B_AI_Output(self,other,"DIA_Sergio_Guide_04_01"); //Я должен сопроводить тебя к Проходу. Но самая опасная часть путешествия только начинается там.
        B_AI_Output(self,other,"DIA_Sergio_Guide_04_02"); //"Но не будем терять времени
        AI_StopProcessInfos(self);
};
...
{
        B_AI_Output(self,other,"DIA_Sergio_Ende_04_00");  //Мы пришли. Что бы ни ждало тебя в Долине Рудников, я надеюсь, что ты найдешь дорогу назад.
        B_AI_Output(other,self,"DIA_Sergio_Ende_15_01");  //Не бойся - я вернусь.
        B_AI_Output(self,other,"DIA_Sergio_Ende_04_02");  //Иди с Инносом. Да не оставит он тебя без защиты.
...
func void dia_sergio_perm_info()
{
        ...
        {
                B_AI_Output(self,other,"DIA_Sergio_Perm_04_00");  //Хвала Инносу, брат. Если ты пришел узнать что-нибудь об Освящении Меча, поговори с Мардуком.
        }
        else
        {
                B_AI_Output(self,other,"DIA_Sergio_Perm_04_01");  //Я слышал о тебе. Ты парень с фермы, который был в Долине Рудников. Мое почтение.
        };
        AI_StopProcessInfos(self);
};
