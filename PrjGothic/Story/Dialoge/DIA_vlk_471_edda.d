...
{
        B_AI_Output(other,self,"DIA_Edda_Hallo_15_00");   //Что ты варишь?
        B_AI_Output(self,other,"DIA_Edda_Hallo_17_01");   //Уху. Может, она не особенно вкусная, но, по крайней мере, это горячая еда.
        B_AI_Output(self,other,"DIA_Edda_Hallo_17_02");   //Могу налить тебе тарелку, если хочешь.
};
...
{
        B_AI_Output(other,self,"DIA_Edda_Stadt_15_00");   //Что ты можешь рассказать мне о городе?
        B_AI_Output(self,other,"DIA_Edda_Stadt_17_01");   //Большинство горожан опасаются воров. Поэтому лучше не входить в чужие дома.
        B_AI_Output(self,other,"DIA_Edda_Stadt_17_02");   //Но если ты ищешь, где остановиться на ночь, можешь поспать в моей хижине. Там есть еще одна кровать.
        B_AI_Output(other,self,"DIA_Edda_Stadt_15_03");   //А ты не боишься воров?
        B_AI_Output(self,other,"DIA_Edda_Stadt_17_04");   //Единственная ценная вещь, что была у меня, уже пропала.
        B_AI_Output(self,other,"DIA_Edda_Stadt_17_05");   //Кто-то украл мою статую Инноса.
...
{
        B_AI_Output(other,self,"DIA_Edda_Kochen_15_00");  //Ты можешь сварить суп для меня?
        B_AI_Output(self,other,"DIA_Edda_Kochen_17_01");  //Я готовлю для всех. Для тебя тоже, если захочешь. Все, что мне нужно - это чтобы ты принес мне рыбу.
};
...
{
        B_AI_Output(other,self,"DIA_Edda_Suppe_15_00");   //Ты можешь сварить для меня суп?
        if(Wld_GetDay() == 0)
        {
                B_AI_Output(self,other,"DIA_Edda_Suppe_17_02");   //С завтрашнего дня ты можешь приходить и получать суп каждый день.
        }
        else if(EDDA_DAY != Wld_GetDay())
        {
                if(b_giveinvitems(other,self,itfo_fish,1))
                {
                        B_AI_Output(self,other,"DIA_Edda_Suppe_17_01");   //Нет ничего проще. Вот, держи тарелку.
                        b_giveinvitems(self,other,itfo_fishsoup,1);
                        EDDA_DAY = Wld_GetDay();
                }
                else
                {
                        B_AI_Output(self,other,"DIA_Edda_Suppe_17_04");   //Принеси мне рыбу, и я сварю тебе вкусный суп.
                };
        }
        else
        {
                B_AI_Output(self,other,"DIA_Edda_Suppe_17_03");   //Нет, не сегодня. Приходи завтра.
        };
};
...
{
        B_AI_Output(other,self,"DIA_Edda_Statue_15_00");  //Посмотри, я принес статую Инноса для тебя.
        B_AI_Output(self,other,"DIA_Edda_Statue_17_01");  //Ох - огромное тебе спасибо. Да не оставит тебя свет Инноса...
        B_AI_Output(other,self,"DIA_Edda_Statue_15_02");  //Не стоит благодарностей.
